        ...
        <div className={`relative w-full ${className}`} ref={dropdownRef}>
            ...
            <div 
                className={`
                    ...
                    ${showDropdown ? 'border-indigo-500 ring-4 ring-indigo-500/10 shadow-2xl' : 'border-slate-100 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 shadow-sm'}
                    cursor-pointer
                `}
                onClick={() => setShowDropdown(!showDropdown)}
            >
                <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-indigo-500 transition-colors">
                ...
                <div className="flex-grow min-w-0">
                    {selectedSupplier ? (
                        ...
                        <span className="text-sm font-bold text-slate-400">Select a strategic supplier...</span>
                    )}
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <ChevronsUpDown className={`w-4 h-4 text-slate-400 transition-transform duration-500 ${showDropdown ? 'rotate-180' : ''}`} />
                </div>
            </div>

            {/* Dropdown Menu */}
            {showDropdown && (
                <div className="absolute top-full left-0 right-0 mt-3 p-2 bg-white/90 dark:bg-slate-900/95 backdrop-blur-2xl border-2 border-slate-100 dark:border-slate-800 rounded-2xl shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-200">
                    <div className="relative mb-2">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-indigo-500/20 transition-all placeholder:text-slate-400"
                            value={searchQuery}
