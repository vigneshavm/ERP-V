    ...
    return (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 w-full bg-sidebar/90 backdrop-blur-xl border-t border-default z-[100] flex justify-around items-center h-16 pb-[env(safe-area-inset-bottom,0px)] expanager-glass">
            {navItems.map(({ id, path, icon: Icon, label }) => {
                const isActive = pathname === path || pathname.startsWith(path === '/' ? '/_' : path + '/');
                return (
                    <button
                        key={id}
                        onClick={() => navigate(id)}
                        className={`flex flex-col items-center justify-center flex-1 h-full gap-0.5 transition-colors ${isActive ? 'text-primary' : 'text-secondary/60'}`}
                    >
