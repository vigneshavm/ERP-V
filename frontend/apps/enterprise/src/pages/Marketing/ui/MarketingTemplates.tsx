        ...
        <Layout>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div className="space-y-4">
                    ...
                    <h1 className="text-4xl lg:text-5xl font-black text-main tracking-tighter italic">
                        Marketing <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-500">Templates</span>
                    </h1>
                    <p className="text-lg text-muted dark:text-muted font-medium max-w-xl">
                ...
                <div className="flex items-center gap-3">
                    <div className="relative group">
                        <Search className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-indigo-500 transition-colors" />
                        <input
                            type="text"
                            placeholder="Search templates..."
                            className="pl-11 pr-6 py-4 bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default rounded-2xl text-xs font-bold w-full md:w-80 outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-sm"
                        />
                    </div>
                    <button
                        onClick={() => navigate('DASHBOARD' as any)}
                        className="flex items-center gap-2 px-6 py-4 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] hover:bg-slate-200 dark:hover:bg-slate-700 text-secondary dark:text-slate-200 rounded-2xl transition-all text-sm font-black shadow-sm"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-[var(--erp-card)] rounded-[2.5rem] p-8 border border-default dark:border-default shadow-sm relative group">
                    <Users className="w-8 h-8 mb-4 text-emerald-500" />
                    <h4 className="text-sm font-black uppercase tracking-widest mb-1 text-muted">Total Utilization</h4>
                    <p className="text-2xl font-black mb-2 text-main">12,450 Campaigns</p>
                    <p className="text-xs font-bold text-emerald-500">+12% from last month</p>
                </div>
                <div className="bg-white dark:bg-[var(--erp-card)] rounded-[2.5rem] p-8 border border-default dark:border-default shadow-sm relative group">
                    <Calendar className="w-8 h-8 mb-4 text-amber-500" />
                    <h4 className="text-sm font-black uppercase tracking-widest mb-1 text-muted">Upcoming Festival</h4>
                    <p className="text-2xl font-black mb-2 text-main">Republic Day 2026</p>
                    <p className="text-xs font-bold text-muted">4 templates available</p>
                </div>
            </div>
                ...
                {categories.map((cat) => (
                    <button
                        ...
                        className={`px-8 py-3.5 rounded-2xl text-xs font-black uppercase tracking-[0.2em] transition-all ${selectedCategory === cat
                            ? 'bg-[var(--erp-bg)] dark:bg-white text-main dark:text-black shadow-lg scale-105'
                            : 'bg-white dark:bg-[var(--erp-card)] text-muted dark:text-muted border border-default dark:border-default hover:border-indigo-500'
                            }`}
                    >
                        {cat}
                    </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredTemplates.map((template) => (
                    <div
                        key={template.id}
                        className="group bg-white dark:bg-[var(--erp-card)] rounded-[3rem] border border-default dark:border-default p-6 hover:shadow-[0_20px_50px_rgba(79,70,229,0.15)] hover:border-indigo-500 transition-all duration-500 cursor-pointer relative"
                    >
                        {/* Status Tags */}
                        <div className="absolute top-10 right-10 z-10 flex flex-col gap-2 scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100 transition-all">
                            <span className="bg-white/90 dark:bg-[var(--erp-bg)]/90 backdrop-blur-md p-2 rounded-xl text-muted hover:text-indigo-600 shadow-lg">
                        ...
                        <div className="px-2">
                            ...
                            <h3 className="text-xl font-black text-main mb-2 leading-tight group-hover:text-indigo-600 transition-colors">{template.name}</h3>
                            <div className="flex items-center justify-between mt-6 pt-6 border-t border-default dark:border-default">
                                <div className="flex items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-indigo-500" />
                                    <span className="text-[10px] font-black uppercase tracking-widest text-muted">{template.usage.toLocaleString()} Used</span>
                                </div>
                                <button
                                    onClick={() => navigate('DASHBOARD' as any)}
                                    className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-main hover:text-indigo-500 transition-colors"
                                >
            ...
            <div className="bg-[var(--erp-bg)] dark:bg-[var(--erp-bg)] rounded-[4rem] p-12 text-main relative overflow-hidden group">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
                    <div className="max-w-xl text-center md:text-left">
                        ...
                        <p className="text-muted font-medium">Our design intelligence engine can generate brand-locked templates for your specific promotions in seconds.</p>
                    </div>
