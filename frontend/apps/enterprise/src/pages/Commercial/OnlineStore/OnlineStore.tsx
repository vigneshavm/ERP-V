                ...
                <div className="relative z-10 max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-16">
                    <div className="max-w-3xl">
                        ...
                        <button
                            onClick={onActivate}
                            disabled={isLoading}
                            className="w-full sm:w-auto px-10 py-5 bg-[#4F46E5] text-main rounded-xl font-black uppercase tracking-widest text-sm hover:scale-105 active:scale-95 transition-all duration-200 focus:ring-4 focus:ring-[#4F46E5]/40 focus:outline-none flex items-center justify-center gap-4 group"
                        >
...
const OnlineStore: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-[var(--erp-bg-sunken)] dark:bg-slate-950 animate-fade-in relative rounded-3xl overflow-hidden border border-default dark:border-default">
            {/* --- Top Navigation Bar --- */}
            <div className="sticky top-0 z-30 bg-white/80 dark:bg-[var(--erp-bg)]/80 backdrop-blur-md border-b border-default dark:border-default px-6 py-4 flex items-center gap-4">
                <div className="flex-1 max-w-2xl relative flex items-center">
                    <input
                        ...
                        className="w-full pl-10 pr-12 py-3 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] border-none rounded-2xl text-main outline-none focus:ring-2 focus:ring-indigo-500 shadow-inner"
                    />
                ...
                <div className="flex items-center gap-2 ml-auto">
                    <button
                        ...
                        className={`hidden md:flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all border ${showAi ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-[var(--erp-card)] text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-900'}`}
                    >
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <aside className={`w-72 bg-white dark:bg-[var(--erp-bg)] border-r border-default dark:border-default overflow-y-auto hidden md:block p-8`}>
                    <div className="flex items-center gap-2 mb-8 text-main font-black text-lg">
                        <Filter className="w-5 h-5" /> Filters
                    </div>
                    {/* Filter Controls (Categories, Price, etc) - Copied from Storefront.tsx */}
                    <div className="space-y-8">
                        <div>
                            <h4 className="text-xs font-black text-muted uppercase tracking-widest mb-4">Categories</h4>
                            <div className="space-y-3">
                                {availableCategories.map(cat => (
                                    <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                                        ...
                                        <span className={`text-sm ${selectedCategories.includes(cat) ? 'text-main font-bold' : 'text-secondary dark:text-muted'}`}>{cat}</span>
                                    </label>
                                ))}
                            </div>
                        </div>
                ...
                <main className="flex-1 overflow-y-auto p-6 md:p-10">
                    ...
                    {(showAi || aiResult || visualSearchImage) && (
                        <div className="mb-8 bg-white dark:bg-[var(--erp-bg)] rounded-[2rem] border border-indigo-100 dark:border-indigo-900 shadow-xl overflow-hidden animate-in slide-in-from-top-2">
                            {/* ... Content ... */}
                            <div className="p-6">
                                {showAi && !visualSearchImage && !aiResult && !aiThinking && (
                                    <div className="relative">
                                        <textarea
                                            ...
                                            className="w-full bg-[var(--erp-bg-sunken)] dark:bg-slate-950 border border-default dark:border-default rounded-2xl p-6 pr-14 text-sm focus:ring-2 focus:ring-indigo-500 outline-none resize-none h-24 text-main placeholder:text-muted font-medium"
                                ...
                                {aiResult && <div className="p-4 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] rounded-2xl text-sm font-medium leading-relaxed">{aiResult.text}</div>}
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                        {filteredProducts.map(product => (
                            <div key={product.id} className="group bg-white dark:bg-[var(--erp-card)] rounded-[2rem] border border-default dark:border-default overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col h-full">
                                ...
                                <div className="p-6 flex flex-col flex-1">
                                    ...
                                    <div className="mt-auto flex items-center justify-between">
                                        <span className="block text-2xl font-black text-main">₹{product.sellingPrice.toLocaleString()}</span>
                                        <button onClick={() => dispatch(addToCart({ ...product, qty: 1, price: product.sellingPrice }))} disabled={product.stockQty <= 0} className="p-4 bg-[var(--erp-bg)] dark:bg-[#4F46E5] text-main rounded-2xl hover:scale-110 active:scale-95 transition-all shadow-lg hover:shadow-xl">
