...
export const StandardPOSTemplate: React.FC<POSTemplateProps> = ({ logic }) => {
    ...
    return (
        <div
            ref={posContainerRef}
            className={`flex flex-col relative transition-all duration-300 premium-bg ${isFullScreen ? 'h-screen fixed inset-0 z-50 p-4 pb-20 lg:pb-4' : 'h-[calc(100vh-4rem)] pb-20 lg:pb-0'}`}
        >
            {/* Modals */}
            <POSHeldBillsModal
            ...
            <div className="flex flex-col gap-2 mb-2">
                <div className="flex justify-between items-center gap-2">
                    ...
                    <div className="flex gap-2 mr-2">
                        {/* Sync Engine Status */}
                        <div className="hidden lg:flex items-center gap-3 px-4 py-2 premium-card mr-2">
                        ...
                        <button
                            ...
                            className="relative p-2 premium-card hover:border-indigo-500/50 text-slate-300 rounded-xl flex items-center gap-2 transition-all group hover:-translate-y-0.5"
                            title="Held Bills (F6)"
                        >
                            ...
                            {(heldBills?.length || 0) > 0 && (
                                <span className="absolute -top-1.5 -right-1.5 bg-indigo-600 text-white text-[9px] font-black w-5 h-5 flex items-center justify-center rounded-full border border-white/10 shadow-lg group-hover:scale-110 transition-transform">
                                    {heldBills?.length}
                                </span>
                            )}
                        </button>
                        <button
                            ...
                            className="p-2 premium-card hover:border-indigo-500/50 text-slate-300 rounded-xl flex items-center gap-2 transition-all hover:-translate-y-0.5"
                            title="Customer Display"
                        >
                            <Monitor className="w-5 h-5 text-indigo-400" />
                        </button>

                        <div className="premium-card p-1 flex gap-1 ml-2">
                            <div className="flex bg-white/5 p-1 rounded-lg mr-1">
                                <button
                                    onClick={() => setIsReturnMode(false)}
                                    className={`px-4 py-1 text-[10px] font-black uppercase tracking-widest rounded-md transition-all ${!isReturnMode ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-300'}`}
                                >
                                    Sale
                                </button>
                                <button
                                    onClick={() => setIsReturnMode(true)}
                                    className={`px-4 py-1 text-[10px] font-black uppercase tracking-widest rounded-md transition-all ${isReturnMode ? 'bg-rose-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-300'}`}
                                >
                                    Return
                                </button>
                            </div>
                            ...
                            <button
                                ...
                                className={`px-4 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${viewMode === 'SCANNER' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-300'}`}
                            >
                            ...
                            <button
                                ...
                                className={`px-4 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${viewMode === 'VISUAL' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-300'}`}
                            >
            ...
            <div className="grid grid-cols-12 gap-6 flex-1 min-h-0 bg-white/5 rounded-xl overflow-hidden border border-white/10">
                ...
                <div className={`col-span-12 lg:col-span-4 flex flex-col min-h-0 border-l border-white/10 ${mobileTab === 'CART' ? 'flex' : 'hidden lg:flex'}`}>
                    {viewMode === 'VISUAL' ? (
                        ...
                        <div className="flex flex-col h-full premium-card shadow-2xl z-20">
                            <POSTerminalInfo
                                cashierName={user?.name}
                                counterName={activeCounterName}
                                counterId={activeCounterId}
                            />
                            <div className="shrink-0 p-2 border-b border-white/10 bg-white/5">
                                <POSCustomerPanel
                                    activeCustomer={activeCustomer}
                                    customers={customers}
                                    onSetCustomer={onSetCustomer}
                            ...
                            {/* Last Bill Card */}
                            {lastBill && (
                                <div className="shrink-0 px-2 pb-1 bg-white/5">
                                    <div className="bg-white/5 border border-white/10 rounded-lg p-2 shadow-sm">
                                        ...
                                        <div className="flex flex-col items-center justify-center pb-0.5">
                                            <span className="text-xl font-black text-primary tracking-tight leading-none mb-1">
                                             ...
                                             <div className="flex items-center gap-1.5 text-[10px] text-slate-400 leading-none">
                                                <span className="font-medium text-slate-300">#{lastBill.id.split('-').pop()}</span>
            ...
            {/* Mobile Tab Nav */}
            <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-neutral-950 border-t border-white/10 pb-safe z-50">
