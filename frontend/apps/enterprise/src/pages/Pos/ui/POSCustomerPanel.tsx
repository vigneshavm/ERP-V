...
export const POSCustomerPanel: React.FC<POSCustomerPanelProps> = ({
    ...
    return (
        <div className="bg-white dark:bg-[var(--erp-card)] p-2 border-b border-default dark:border-default shadow-sm shrink-0 transition-colors">
            ...
            <div className="relative mb-2 flex flex-col gap-2">
                    ...
                    <div className="flex gap-2 animate-in fade-in slide-in-from-top-1 duration-200">
                        <div className="relative flex-1">
                            <input
                                ...
                                className="w-full pl-8 pr-2 py-1.5 text-xs bg-primary/5 dark:bg-[var(--erp-bg)] border border-primary/20 dark:border-primary/40 rounded focus:outline-none focus:ring-2 focus:ring-primary transition-all"
                                value={nameQuery}
                ...
                {phoneSuggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default rounded-lg shadow-xl z-20 max-h-60 overflow-y-auto">
                        {phoneSuggestions.map((cust, idx) => (
                            <button
                                ...
                                className={`w-full text-left px-3 py-2.5 text-sm border-b border-default dark:border-default/50 block ${idx === selectedPhoneIndex ? 'bg-primary text-white' : 'text-neutral-700 dark:text-neutral-300 hover:bg-[var(--erp-bg-sunken)] dark:hover:bg-neutral-700'}`}
                            >
                                <span className="font-bold">{cust.name}</span>
                                <div className="flex items-center gap-2 mt-0.5">
                                    <span className={`text-xs ${idx === selectedPhoneIndex ? 'text-secondary' : 'text-neutral-500'}`}>{cust.phone}</span>
                                    <span className={`text-[10px] px-1 rounded font-bold ${idx === selectedPhoneIndex ? 'bg-white/20 text-main' : 'bg-[var(--erp-bg-sunken)] dark:bg-neutral-700 text-neutral-600 dark:text-neutral-400'}`}>
                                        {cust.tier || 'General'}
                                    </span>
                                    <span className={`text-xs ${idx === selectedPhoneIndex ? 'text-secondary' : 'text-neutral-500'}`}>&bull; {cust.points} pts</span>
                                </div>
                            </button>
                        ))}
                    </div>
            ...
            <div className="flex flex-col gap-2 bg-white dark:bg-[var(--erp-bg)] rounded-3xl p-4 border border-default dark:border-default shadow-sm relative overflow-hidden group">
                ...
                <div className="flex justify-between items-center relative z-10">
                    <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-black text-[10px] ${activeCustomer.id !== 'c1' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] text-muted'}`}>
                            {activeCustomer.name.charAt(0)}
                        </div>
                        <div>
                            <span
                                onClick={() => setIs360ModalOpen(true)}
                                className={`text-xs font-black truncate max-w-[120px] italic block leading-none cursor-pointer hover:underline ${activeCustomer.id !== 'c1' ? 'text-main' : 'text-muted'}`}
                            >
                                {activeCustomer.id !== 'c1' ? activeCustomer.name : 'Unidentified Customer'}
                            </span>
                            {activeCustomer.id !== 'c1' && (
                                <p className="text-[9px] font-bold text-muted uppercase tracking-widest mt-0.5">{activeCustomer.phone}</p>
                            )}
                        </div>
                    </div>
                    {activeCustomer.id !== 'c1' && (
                        <button onClick={() => onSetCustomer('c1')} className="p-1.5 bg-rose-50 dark:bg-rose-900/20 text-rose-500 hover:bg-rose-500 hover:text-main rounded-lg transition-all">
                ...
                {/* Loyalty Tier & Points */}
                {activeCustomer.id !== 'c1' && (
                    <div className="flex items-center justify-between mt-2 pt-3 border-t border-default dark:border-default relative z-10">
                        <div className="flex items-center gap-2">
                            <div className="flex flex-col">
                                <span className="text-[8px] font-black text-muted uppercase tracking-widest">Customer Tier</span>
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-sm font-black text-main tracking-tight">{activeCustomer.points.toLocaleString()} <span className="text-[10px] text-muted uppercase ml-0.5">Pts</span></p>
                        </div>
                    </div>
                )}
                ...
                {activeCustomer.id !== 'c1' && (
                    <div className="flex items-center gap-4 pt-3 mt-1 border-t border-default dark:border-default relative z-10">
                        {/* Email Actions */}
                        <div className="flex items-center gap-1.5">
                            <span className="text-[9px] font-black text-muted uppercase tracking-widest mr-1">Reach:</span>
                            <button
                                onClick={() => alert("Invoice emailed!")}
                                className="p-2 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] hover:bg-indigo-600 hover:text-main rounded-xl transition-all shadow-sm border border-default dark:border-default"
                                title="Email Invoice"
                            >
                            ...
                            <button
                                ...
                                className="p-2 bg-emerald-50 dark:bg-emerald-900/10 hover:bg-emerald-600 hover:text-main rounded-xl text-emerald-600 transition-all shadow-sm border border-emerald-100 dark:border-emerald-900/30"
                        ...
                        <div className="flex items-center gap-1.5 ml-auto">
                            <button
                                ...
                                className="p-2 bg-amber-50 dark:bg-amber-900/10 hover:bg-amber-500 hover:text-main rounded-xl text-amber-600 transition-all shadow-sm border border-amber-100 dark:border-amber-900/30"
                        ...
                        {/* CX Feedback Trigger */}
                        <div className="flex items-center gap-1.5 ml-2 border-l border-default dark:border-default pl-4">
                            <button
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-[7px] font-black text-muted">92% CSAT</span>
                            </div>
                        </div>
                    </div>
                )}
