        ...
        return (
            <Layout>
                ...
                <div className="flex flex-col justify-center items-center h-64 gap-4">
                    ...
                    <p className="text-[10px] font-black text-muted uppercase tracking-[0.2em]">Indexing Financial Data</p>
                </div>
                      </div>
                ...
                actions={
                    <div className="flex gap-2">
                        <button
                            ...
                            className="px-5 py-2.5 bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default text-secondary dark:text-muted rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[var(--erp-bg-sunken)] dark:hover:bg-slate-700 transition-all flex items-center gap-2"
                        >
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {['Savings', 'Current', 'Overdraft', 'Loan'].map((type) => {
                            ...
                            return (
                                <div key={type} className={`${config.bg} border border-default dark:border-default rounded-[2rem] p-6 transition-all hover:shadow-lg hover:scale-[1.02] group`}>
                                    ...
                                    <h4 className="text-sm font-black text-main uppercase tracking-tight">{type}</h4>
                                    <p className="text-xs font-bold text-muted mt-1">₹{typeBalance.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</p>
                                    <div className="mt-4 pt-4 border-t border-default dark:border-default">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[9px] font-black text-muted uppercase tracking-widest">Portfolio Share</span>
            ...
            <div className="mb-8">
                ...
                {accounts.length === 0 ? (
                    <div className="py-20 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)]/20 border-2 border-dashed border-default dark:border-default rounded-[3rem] text-center flex flex-col items-center">
                        <div className="w-20 h-20 bg-white dark:bg-[var(--erp-card)] rounded-[2rem] shadow-xl flex items-center justify-center text-slate-200 mb-6">
                            <Building2 className="w-10 h-10" />
                        </div>
                        <h4 className="text-lg font-black text-main uppercase tracking-tight">No Institutions Registered</h4>
                        <p className="text-xs font-medium text-muted mt-2 mb-6">Initialize your banking portfolio by adding your first institution.</p>
                        <button
                            onClick={() => navigate('/cashbank/bank-accounts')}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {accounts.map((account) => {
                            ...
                            return (
                                <div key={account._id} className="bg-white dark:bg-[var(--erp-bg)] border border-default dark:border-default rounded-[2rem] p-6 transition-all hover:shadow-xl hover:border-indigo-100 dark:hover:border-indigo-900 group">
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex items-center gap-4">
                                            <div className="w-14 h-14 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-700 rounded-2xl flex items-center justify-center text-xl font-black text-secondary dark:text-muted group-hover:scale-110 transition-transform">
                                                {account.bankName?.charAt(0) || 'B'}
                                            </div>
                                            <div>
                                                <h4 className="text-sm font-black text-main uppercase tracking-tight">{account.bankName}</h4>
                                                <p className="text-[10px] font-bold text-muted mt-0.5">****{account.accountNumber?.slice(-4) || '0000'}</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex items-end justify-between pt-4 border-t border-default dark:border-default">
                                        <div>
                                            <p className="text-[9px] font-black text-muted uppercase tracking-widest mb-1">Current Balance</p>
                                        ...
                                        <button
                                            ...
                                            className="px-4 py-2 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600 rounded-xl text-[9px] font-black text-muted uppercase tracking-widest transition-all flex items-center gap-1.5"
                                        >
