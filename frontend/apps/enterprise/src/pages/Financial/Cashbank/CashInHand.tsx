    ...
    return (
        <Layout>
            ...
            <CashBankHero
                ...
                stats={
                    <>
                        ...
                        <button onClick={() => navigate('/cashbank/ledger/cash')} className="text-[10px] font-black uppercase tracking-widest text-indigo-200 hover:text-main transition-all underline underline-offset-8">Audit Ledger</button>
                    </>
                }
                extraActions={
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-[var(--erp-bg-sunken)] border border-default p-6 rounded-[2rem] backdrop-blur-sm">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-1.5 p-1.5 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] rounded-2xl w-fit">
                ...
                <div className="flex items-center gap-3 bg-white dark:bg-[var(--erp-bg)] px-4 py-2 rounded-2xl border border-default dark:border-default shadow-sm">
                    <Calendar className="w-4 h-4 text-muted" />
                    <input
                        type="date"
                        value={dateRange.from}
                        onChange={(e) => setDateRange({ ...dateRange, from: e.target.value })}
                        className="bg-transparent text-xs font-bold text-secondary dark:text-muted focus:outline-none"
                    />
            ...
            {showAddTransaction && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--erp-bg)]/60 backdrop-blur-sm animate-fade-in">
                    <form onSubmit={handleSubmit} className="bg-white dark:bg-[var(--erp-bg)] w-full max-w-3xl rounded-[3rem] shadow-2xl border border-default dark:border-default overflow-hidden animate-slide-up">
                        <div className="p-8 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)]/50 border-b border-default dark:border-default flex items-center justify-between">
                            <div>
                                <h2 className="text-xl font-black text-main uppercase tracking-tight">Vault Transfer Terminal</h2>
                                <p className="text-xs font-medium text-muted">Recording manual {formData.type === 'in' ? 'Inflow' : 'Outflow'} protocol</p>
                            </div>
                            ...
                            <CashBankFormSection title="Clearing Matrix">
                                <CashBankInput label="Execution Date" icon={Calendar}>
                                    <input
                                        ...
                                        className="w-full pl-11 pr-4 py-3.5 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)]/50 border border-default dark:border-default rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-sm"
                                        required
                                    />
                                </CashBankInput>
                            ...
                            <CashBankFormSection title="Routing Information">
                                <CashBankInput label="Category / Origin Unit" icon={Navigation}>
                                    <select
                                        ...
                                        className="w-full pl-11 pr-4 py-3.5 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)]/50 border border-default dark:border-default rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-sm bg-no-repeat"
                                        required
                                    >
                            ...
                            <div className="md:col-span-2">
                                <CashBankInput label="Transactional Narrative" icon={History}>
                                    <textarea
                                        ...
                                        rows={2}
                                        className="w-full pl-11 pr-4 py-3.5 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)]/50 border border-default dark:border-default rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-sm leading-relaxed"
                            ...
                            {(formData.amount || 0) > 0 && (formData.fromAccount || formData.toAccount) && (
                                <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 p-6 rounded-[2rem] flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="p-3 bg-white dark:bg-[var(--erp-card)] rounded-2xl shadow-sm">
                                            <Zap className="w-5 h-5 text-indigo-600" />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-muted uppercase tracking-widest">Anticipated Impact</p>
                                    ...
                                    <div className="text-right">
                                        <p className="text-[10px] font-black text-muted uppercase tracking-widest">New Vault Index</p>
                        ...
                        <div className="p-8 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)]/50 border-t border-default dark:border-default flex justify-end gap-3">
                            <button type="button" onClick={() => setShowAddTransaction(false)} className="px-8 py-3.5 border border-default dark:border-default text-muted rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all">Abort</button>
            ...
            <div className="grid grid-cols-1 gap-4 pb-20">
                {filteredTransactions.map(t => {
                    ...
                    return (
                        <div key={t._id} className="bg-white dark:bg-[var(--erp-bg)] p-6 rounded-[2rem] border border-default dark:border-default shadow-sm flex items-center justify-between hover:shadow-lg hover:border-indigo-100 dark:hover:border-indigo-900 transition-all group">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-muted uppercase tracking-widest flex items-center gap-2 mb-1">
                                        {new Date(t.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })} • {t.reference || 'NO-REF'}
                                    </p>
                                    <h4 className="text-sm font-black text-main uppercase tracking-tight">{t.description}</h4>
                                    <p className="text-[10px] font-bold text-muted mt-1 italic">Channel: {t.type === 'transfer' ? 'Internal Transfer' : 'Direct Manual'}</p>
                                </div>
                            </div>
                            <div className="text-right">
                                ...
                                <button className="p-1 px-3 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] rounded-lg text-[9px] font-black text-muted uppercase tracking-widest mt-2 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600 transition-all">Audit Detalle</button>
                            </div>
                        </div>
                    );
                })}
                ...
                {filteredTransactions.length === 0 && (
                    <div className="py-20 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)]/20 border-2 border-dashed border-default dark:border-default rounded-[3rem] text-center flex flex-col items-center">
                        <div className="w-20 h-20 bg-white dark:bg-[var(--erp-card)] rounded-[2rem] shadow-xl flex items-center justify-center text-slate-200 mb-6 font-black text-4xl">
                            ?
                        </div>
                        <h4 className="text-lg font-black text-main uppercase tracking-tight">No Temporal Ledger Data</h4>
                        <p className="text-xs font-medium text-muted mt-2">Initialize a capital pulse through the inflow/outflow controls above.</p>
                    </div>
                )}
            </div>
                  </div>
