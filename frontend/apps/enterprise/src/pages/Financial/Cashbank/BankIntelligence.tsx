            ...
            <div className="space-y-6 animate-fade-in text-neutral-900 dark:text-neutral-100 pb-16">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                        <button className="px-4 py-2 bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-[var(--erp-bg-sunken)] shadow-sm">
                ...
                {/* Navigation Tabs */}
                <div className="flex border-b border-default dark:border-default pb-0.5 mt-4">
                            ...
                            <>
                                <div className="relative mb-6">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary/20"
                                        value={searchTerm}
                                ...
                                {filteredAccounts.map(acc => (
                                    <div key={acc.id} className={`group bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default rounded-2xl p-6 shadow-sm transition-all hover:border-primary/30 ${acc.status === 'MISMATCH' ? 'border-l-4 border-l-error' : ''
                                        }`}>
                                        <div className="flex flex-col sm:flex-row justify-between gap-6">
                                            <div className="flex gap-4">
                                                <div className="p-4 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-bg)] rounded-2xl text-primary shrink-0">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center gap-2 mt-2">
                                                        <span className="px-2 py-0.5 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-bg)] rounded text-[9px] font-black text-neutral-500 uppercase">{acc.branch}</span>
                                                        <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${acc.status === 'OK' ? 'bg-success/10 text-success' : 'bg-error text-main shadow-sm shadow-error/20'
                                                            }`}>
                                                            {acc.status}
                                                        </span>
                                                    </div>
                                        ...
                                        {acc.status === 'MISMATCH' && (
                                            <div className="mt-6 p-4 bg-error/5 border border-error/10 rounded-2xl flex items-center justify-between gap-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-error text-main rounded-full">
                                                        <AlertTriangle className="w-4 h-4" />
                                                    </div>
                                                    <div>
                                                ...
                                                <button className="px-4 py-2 bg-error text-main rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-lg shadow-error/20">
                                                    Audit Gap
                                                </button>
                                            </div>
                                        )}

                                        <div className="mt-6 pt-4 border-t border-default dark:border-default flex items-center justify-between">
                                            <div className="flex gap-4">
                                            ...
                                            <button className="px-4 py-2 bg-[var(--erp-bg-sunken)] dark:bg-neutral-700 hover:bg-neutral-200 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
                                                Sync Statement
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div className="bg-white dark:bg-[var(--erp-card)] rounded-[2.5rem] border border-default dark:border-default p-8 shadow-sm">
                                ...
                                <div className="space-y-6">
                                    {[
                                        ...
                                        <div key={i} className="flex flex-col sm:flex-row items-center gap-6 p-6 border border-default dark:border-default rounded-3xl group hover:border-primary/30 transition-all">
                                            ...
                                            <div className="flex flex-1 items-center justify-between gap-8 border-l border-default px-8">
                                                <div>
                                            ...
                                            <div className="text-right">
                                                <span className={`px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${row.status === 'MATCHED' ? 'bg-success/10 text-success' :
                                                    row.status === 'MISMATCH' ? 'bg-error/10 text-error' : 'bg-[var(--erp-bg-sunken)] text-neutral-500'
                                                    }`}>
                                                    {row.status}
                                                </span>
                                                {row.status !== 'MATCHED' && (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white dark:bg-[var(--erp-card)] rounded-3xl border border-default dark:border-default p-6 shadow-sm">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <span className="px-2 py-0.5 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-bg)] rounded text-[9px] font-bold text-neutral-500 italic">Audit Ready</span>
                            </div>
                            ...
                            <div className="space-y-4">
                                {obligations.map(o => (
                                    <div key={o.id} className="flex gap-4 group/item">
                                        ...
                                        <button className="opacity-0 group-hover/item:opacity-100 transition-opacity p-2 hover:bg-[var(--erp-bg-sunken)] rounded-lg">
                            ...
                            <div className="mt-8 pt-6 border-t border-default dark:border-default">
                        ...
                        <div className="bg-neutral-950 text-main p-8 rounded-[2.5rem] border border-default relative overflow-hidden group shadow-2xl">
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="space-y-4 mb-8">
                                    <div className="p-3 bg-[var(--erp-bg-sunken)] border border-default rounded-2xl">
                                        <p className="text-[10px] font-medium text-neutral-400 leading-relaxed">
                                            Detected <span className="text-main font-bold">₹8.5L idle</span> at Madurai Savings.
                        ...
                        <div className="p-4 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-bg)]/50 rounded-2xl border border-default dark:border-default flex items-start gap-3">
...
// eslint-disable-next-line @typescript-eslint/no-empty-object-type -- TODO(TS-FIX): Phase 2/3 fix
