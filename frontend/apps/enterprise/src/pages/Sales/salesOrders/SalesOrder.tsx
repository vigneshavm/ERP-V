...
const SalesOrderPage = () => {
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            logger.error('Error saving sales order:', error);
    ...
    const handleConfirmOrder = async () => {
        ...
        try {
            ...
            logger.error('Error confirming sales order:', error);
    ...
    const statusOptions = [
        { value: 'Draft', label: 'Draft', color: 'bg-[var(--erp-bg-sunken)] text-main' },
            ...
            <div className="flex flex-col h-[calc(100vh-theme(spacing.16))] bg-[var(--erp-bg-sunken)] animate-fade-in">
                ...
                <div className="bg-white border-b border-default px-6 py-4 flex items-center justify-between shadow-sm shrink-0 z-10">
                    <div>
                        ...
                        <p className="text-sm text-muted mt-0.5">Manage customer orders and reservations</p>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto space-y-6 pb-10">
                        ...
                        <div className="bg-white border border-default rounded-xl shadow-sm overflow-hidden">
                            <div className="bg-[var(--erp-bg-sunken)]/50 px-6 py-3 border-b border-default flex justify-between items-center">
                                ...
                                <div className="text-xs font-bold text-muted bg-white px-2.5 py-1 rounded-md border border-default shadow-sm">
                                    DRAFT MODE
                                </div>
                            </div>
                            <div className="p-6 grid grid-cols-1 md:grid-cols-4 gap-8">
                                ...
                                <div className="space-y-5">
                                    <div>
                                        <label className="block text-xs font-bold text-muted mb-1.5 uppercase">Order Date <span className="text-red-500">*</span></label>
                                        <input
                                            type="date"
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-muted mb-1.5 uppercase">Expected Delivery <span className="text-red-500">*</span></label>
                                        <input
                                            type="date"
                                ...
                                <div className="md:col-span-2 border border-default rounded-xl bg-[var(--erp-bg-sunken)]/50 p-4 relative group hover:border-indigo-300 transition-colors">
                                    ...
                                    {formData.customer ? (
                                        ...
                                        <div className="h-full flex flex-col items-center justify-center gap-3 py-4">
                                            ...
                                            <p className="text-sm font-medium text-muted">No customer selected</p>
                                            <button
                        ...
                        <div className="bg-white border border-default rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[300px]">
                            <div className="px-6 py-3 border-b border-default bg-[var(--erp-bg-sunken)]/50 flex justify-between items-center">
                                <h2 className="text-xs font-bold text-muted uppercase tracking-wider">Line Items</h2>
                                <button
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-[var(--erp-bg-sunken)] border-b border-default text-xs font-bold text-muted uppercase tracking-wider">
                                            <th className="px-4 py-3 border-r border-default w-12 text-center">#</th>
                                            <th className="px-4 py-3 border-r border-default">Product</th>
                                            <th className="px-4 py-3 border-r border-default w-28 text-right">Qty</th>
                                            <th className="px-4 py-3 border-r border-default w-32 text-right">Rate</th>
                                            <th className="px-4 py-3 border-r border-default w-24 text-right">Tax %</th>
                                            <th className="px-4 py-3 border-r border-default w-28 text-right">Disc</th>
                                            <th className="px-4 py-3 border-r border-default w-32 text-right">Total</th>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {formData.items.length === 0 ? (
                                            <tr>
                                                <td colSpan={8} className="px-4 py-16 text-center text-muted">
                                                    <div className="flex flex-col items-center gap-2">
                                                        <svg className="w-8 h-8 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                                            ...
                                            formData.items.map((item, index) => (
                                                <tr key={index} className="hover:bg-[var(--erp-bg-sunken)]/80 group transition-colors">
                                                    <td className="px-4 py-2 text-xs text-muted text-center border-r border-default bg-[var(--erp-bg-sunken)]/30">
                                                        {index + 1}
                                                    </td>
                                                    <td className="px-4 py-2 border-r border-default max-w-xs">
                                                        <p className="text-sm font-semibold text-main truncate">{item.name}</p>
                                                        <p className="text-[10px] uppercase font-bold text-muted mt-0.5">Stock: {item.availableStock}</p>
                                                    </td>
                                                    <td className="px-4 py-2 border-r border-default text-right p-1.5">
                                                        <input
                                                            type="number"
                                                            min="1"
                                                            ...
                                                            className="w-full text-right px-2 py-1.5 text-sm border border-default rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all font-medium"
                                                        />
                                                    </td>
                                                    <td className="px-4 py-2 border-r border-default text-right p-1.5">
                                                        <input
                                                    ...
                                                    <td className="px-4 py-2 border-r border-default text-right p-1.5">
                                                        <select
                                                            ...
                                                            className="w-full text-right px-1 py-1.5 text-sm border border-default rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all bg-white"
                                                        >
                                                    ...
                                                    <td className="px-4 py-2 border-r border-default text-right p-1.5">
                                                        <input
                                                            ...
                                                            className="w-full text-right px-2 py-1.5 text-sm border border-default rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
                                                        />
                                                    </td>
                                                    <td className="px-4 py-2 border-r border-default text-right font-bold text-secondary">
                                                        ₹{calculateItemTotal(item).toFixed(2)}
                                                    </td>
                                                    <td className="px-4 py-2 text-center">
                                                        <button
                                                            onClick={() => removeItem(index)}
                                                            className="text-muted hover:text-red-600 transition-colors p-1.5 hover:bg-red-50 rounded-lg"
                                                            title="Remove Item"
                                                        >
                                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    {formData.items.length > 0 && (
                                        <tfoot className="bg-[var(--erp-bg-sunken)]/50 border-t border-default">
                                            <tr>
                                                <td colSpan={6} className="px-4 py-3 text-xs text-right text-muted uppercase font-bold border-r border-default">Subtotal</td>
                                                <td className="px-4 py-3 text-sm text-right font-bold text-secondary border-r border-default">₹{subTotalAmount.toFixed(2)}</td>
                                                <td></td>
                                            </tr>
                                            <tr>
                                                <td colSpan={6} className="px-4 py-3 text-xs text-right text-muted uppercase font-bold border-r border-default">Total Tax</td>
                                                <td className="px-4 py-3 text-sm text-right font-bold text-secondary border-r border-default">₹{totalTaxAmount.toFixed(2)}</td>
                                                <td></td>
                                            </tr>
                                        </tfoot>
                                    )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {/* Notes Panel */}
                            <div className="md:col-span-2 bg-white border border-default rounded-xl shadow-sm overflow-hidden flex flex-col">
                                <div className="bg-[var(--erp-bg-sunken)]/50 px-6 py-3 border-b border-default">
                                    <h2 className="text-xs font-bold text-muted uppercase tracking-wider">Terms & Notes</h2>
                                </div>
                                <div className="p-4 flex-1">
                                    <textarea
                                        ...
                                        rows={4}
                                        className="w-full h-full px-4 py-3 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none placeholder:text-muted"
                            ...
                            <div className="bg-white border border-indigo-100 rounded-xl shadow-sm overflow-hidden flex flex-col">
                                ...
                                <div className="p-6 space-y-4 flex-1">
                                    ...
                                    <div className="flex justify-between text-sm">
                                        <span className="text-muted font-medium">Item Discounts</span>
