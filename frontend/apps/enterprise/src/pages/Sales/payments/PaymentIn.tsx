...
const PaymentIn = () => {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      logger.error("Error saving payment for print:", error);
      logger.error("Error response:", error.response?.data);
      const errorMessage =
        error.response?.data?.message ||
        error.response?.data?.error ||
  ...
  return (
    <Layout>
      ...
      <div className="flex flex-col h-[calc(100vh-theme(spacing.16))] bg-[var(--erp-bg-sunken)] animate-fade-in">
        {/* Sticky Action Bar */}
        <div className="bg-white border-b border-default px-6 py-4 flex items-center justify-between shadow-sm shrink-0 z-10">
          <div>
            <h1 className="text-2xl font-bold text-main tracking-tight">Payment In</h1>
            <p className="text-sm text-muted mt-0.5">Record customer payments and receipts</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate("/sales/payment-in-list")}
              className="px-4 py-2 text-sm font-medium text-secondary bg-white border border-slate-300 rounded-lg hover:bg-[var(--erp-bg-sunken)] transition-colors shadow-sm"
            >
              Cancel
            </button>
            <button
            ...
            <button
              onClick={handlePrint}
              disabled={loading}
              className="px-4 py-2 text-sm font-medium text-secondary bg-white border border-slate-300 rounded-lg hover:bg-[var(--erp-bg-sunken)] transition-colors shadow-sm flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white border border-default rounded-xl shadow-sm overflow-hidden">
                <div className="bg-[var(--erp-bg-sunken)]/50 px-6 py-3 border-b border-default">
                  <h2 className="text-xs font-bold text-muted uppercase tracking-wider">Payment Information</h2>
                </div>
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-bold text-muted mb-1.5 uppercase">Receipt Number</label>
                    <input
                      type="text"
                      value={formData.receiptNo}
                      disabled
                      className="w-full px-3 py-2 bg-[var(--erp-bg-sunken)] border border-default rounded-lg text-sm font-medium text-muted"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-muted mb-1.5 uppercase">Receipt Date <span className="text-red-500">*</span></label>
                    <input
                      type="date"
              ...
              <div className="bg-white border border-default rounded-xl shadow-sm overflow-hidden">
                <div className="bg-[var(--erp-bg-sunken)]/50 px-6 py-3 border-b border-default flex justify-between items-center">
                ...
                <div className="p-6">
                  {formData.customer ? (
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="flex gap-4 mt-4">
                            <div className="bg-white px-3 py-1.5 rounded-lg border border-default shadow-sm">
                              <p className="text-[10px] uppercase font-bold text-muted">Outstanding</p>
                    ...
                    <button
                      ...
                      className="w-full py-8 border-2 border-dashed border-slate-300 rounded-xl hover:border-indigo-500 hover:bg-[var(--erp-bg-sunken)] transition-all flex flex-col items-center justify-center gap-2 group"
                    >
                      <div className="p-3 bg-white rounded-full shadow-sm group-hover:shadow-md transition-shadow">
                        <svg className="w-6 h-6 text-muted group-hover:text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                      </div>
                      <span className="font-bold text-secondary group-hover:text-indigo-600">Select Customer</span>
                      <span className="text-xs text-muted">Search by name, phone or email</span>
                    </button>
                  )}
                </div>
              </div>
              ...
              {formData.customer && customerInfo.outstandingInvoices.length > 0 && (
                <div className="bg-white border border-default rounded-xl shadow-sm overflow-hidden flex flex-col">
                  <div className="bg-[var(--erp-bg-sunken)]/50 px-6 py-3 border-b border-default">
                    <h2 className="text-xs font-bold text-muted uppercase tracking-wider">Outstanding Invoices</h2>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-[var(--erp-bg-sunken)] border-b border-default text-xs font-bold text-muted uppercase tracking-wider">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {customerInfo.outstandingInvoices.map((invoice: any) => (
                          <tr key={invoice._id} className="hover:bg-[var(--erp-bg-sunken)] transition-colors">
                            ...
                            <td className="px-6 py-3 text-sm font-medium text-right text-secondary">
                            ...
                            <td className="px-6 py-2 text-right">
                              <input
                                ...
                                className="w-full px-2 py-1.5 text-right text-sm border border-default rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all font-medium"
                                placeholder="0.00"
                                max={invoice.balance}
                              />
                            </td>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white border border-default rounded-xl shadow-sm overflow-hidden">
                <div className="bg-[var(--erp-bg-sunken)]/50 px-6 py-3 border-b border-default flex justify-between items-center">
                  <h2 className="text-xs font-bold text-muted uppercase tracking-wider">Payment Methods</h2>
                  <button
                ...
                <div className="p-4 space-y-4">
                  {formData.paymentMethods.map((pm: any, index: number) => (
                    <div key={index} className="p-4 border border-default rounded-xl bg-[var(--erp-bg-sunken)]/30 relative">
                      ...
                      <div className="space-y-3">
                        {/* Method Select */}
                        <div>
                          <label className="block text-[10px] uppercase font-bold text-muted mb-1">Method</label>
                          <select
                            value={pm.method}
                        ...
                        {/* Amount Input */}
                        <div>
                          <label className="block text-[10px] uppercase font-bold text-muted mb-1">Amount</label>
                          <div className="relative">
                            <span className="absolute left-3 top-2 text-muted font-bold">₹</span>
                            <input
                              type="number"
                              value={pm.amount}
                              onChange={(e) => updatePaymentMethod(index, "amount", parseFloat(e.target.value) || 0)}
                              className="w-full pl-8 pr-3 py-2 border border-slate-300 rounded-lg text-sm font-bold text-main focus:ring-2 focus:ring-indigo-500"
                              placeholder="0.00"
                            />
                          </div>
                        </div>
                        ...
                        {pm.method === "upi" && (
                          <div>
                            <label className="block text-[10px] uppercase font-bold text-muted mb-1">Transaction Ref</label>
                            <input
                              type="text"
                              value={pm.reference}
                        ...
                        {(pm.method === "bank_transfer" || pm.method === "cheque") && (
                          <div className="space-y-3 pt-2 border-t border-default mt-2">
                            <div>
                              <label className="block text-[10px] uppercase font-bold text-muted mb-1">Deposit To Bank</label>
                              <select
              ...
              {/* Deposit To (Global) */}
              <div className="bg-white border border-default rounded-xl shadow-sm p-5">
                <h2 className="text-xs font-bold text-muted uppercase tracking-wider mb-3">Deposit Verification</h2>
                {(() => {
              ...
              <div className="bg-white border border-indigo-100 rounded-xl shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="p-6 space-y-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted font-medium">Total Received</span>
                    <span className="font-bold text-main">₹{totalPayment.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm border-b border-default pb-3">
                    <span className="text-muted font-medium">Used from Credit</span>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-muted">(Available: ₹{customerInfo.availableCredit})</span>
                      <input
                        type="number"
                        value={creditApplied}
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-muted font-medium">Allocated to Invoices</span>
                    <span className="font-bold text-indigo-600">₹{totalAllocated.toFixed(2)}</span>
                  </div>

                  <div className="pt-2">
                    <div className="flex justify-between items-center bg-[var(--erp-bg-sunken)] p-3 rounded-lg border border-default">
              ...
              {/* Notes */}
              <div className="bg-white border border-default rounded-xl shadow-sm p-5">
                <h2 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Notes</h2>
                <textarea
