        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white dark:bg-[var(--erp-card)] p-5 rounded-2xl border border-default dark:border-default shadow-sm">
                    ...
                    <p className="text-xs font-bold text-muted dark:text-muted uppercase tracking-wider">Active Counters</p>
                    <h3 className="text-2xl font-bold text-main mt-1">{counterStats.filter(s => s.count > 0).length}</h3>
                </div>
                ...
                <div className="bg-white dark:bg-[var(--erp-card)] p-5 rounded-2xl border border-default dark:border-default shadow-sm">
                    ...
                    <p className="text-xs font-bold text-muted dark:text-muted uppercase tracking-wider">Avg Transaction</p>
                    <h3 className="text-2xl font-bold text-main mt-1">₹{(totalRevenue / (totalTransactions || 1)).toLocaleString(undefined, { maximumFractionDigits: 0 })}</h3>
                </div>
                ...
                <div className="bg-white dark:bg-[var(--erp-card)] p-5 rounded-2xl border border-default dark:border-default shadow-sm">
                    ...
                    <p className="text-xs font-bold text-muted dark:text-muted uppercase tracking-wider">Total Bills</p>
                    <h3 className="text-2xl font-bold text-main mt-1">{totalTransactions}</h3>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-[var(--erp-card)] p-6 rounded-2xl border border-default dark:border-default shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <h4 className="font-bold text-main">Revenue by Counter</h4>
                        <button className="p-2 hover:bg-[var(--erp-bg-sunken)] dark:hover:bg-slate-700 rounded-lg text-muted transition-colors">
                ...
                {/* Pie Chart / Table */}
                <div className="bg-white dark:bg-[var(--erp-card)] p-6 rounded-2xl border border-default dark:border-default shadow-sm">
                    <h4 className="font-bold text-main mb-6">Counter Contribution</h4>
                                ...
                                <div key={stat.name} className="flex flex-col gap-1">
                                    <div className="flex items-center justify-between text-xs">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-medium text-secondary dark:text-muted truncate max-w-[80px]">{stat.name}</span>
                                        </div>
                                        <span className="font-bold text-main">{((stat.revenue / totalRevenue) * 100).toFixed(1)}%</span>
                                    </div>
                                    <div className="w-full bg-[var(--erp-bg-sunken)] dark:bg-slate-700 h-1.5 rounded-full overflow-hidden">
                                        <div
            ...
            <div className="bg-white dark:bg-[var(--erp-card)] rounded-2xl border border-default dark:border-default overflow-hidden shadow-sm">
                <div className="p-4 border-b border-default dark:border-default flex justify-between items-center bg-[var(--erp-bg-sunken)]/50 dark:bg-[var(--erp-bg)]/20">
                    <h4 className="font-bold text-main text-sm">Detailed Performance Summary</h4>
                                ...
                                <tr key={stat.name} className="hover:bg-[var(--erp-bg-sunken)] dark:hover:bg-slate-700/30 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] flex items-center justify-center font-bold text-muted">
                                                {stat.name.charAt(0)}
                                            </div>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <span className={`px-2 py-1 rounded text-[10px] font-bold ${idx === 0 ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10' : 'bg-[var(--erp-bg-sunken)] text-secondary dark:bg-[var(--erp-card)]'}`}>
