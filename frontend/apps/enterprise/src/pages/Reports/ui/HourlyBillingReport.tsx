        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-[var(--erp-card)] p-5 rounded-2xl border border-default dark:border-default shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-[10px] font-bold text-muted uppercase">Insights</span>
                    </div>
                    <p className="text-xs font-bold text-muted dark:text-muted uppercase tracking-wider">Peak Traffic Hour</p>
                    <h3 className="text-2xl font-bold text-main mt-1">{peakHour?.label || 'N/A'}</h3>
                    <p className="text-xs text-muted mt-1">{peakHour?.transactions} bills generated in this hour</p>
                </div>
                ...
                <div className="bg-white dark:bg-[var(--erp-card)] p-5 rounded-2xl border border-default dark:border-default shadow-sm">
                    ...
                    <p className="text-xs font-bold text-muted dark:text-muted uppercase tracking-wider">Highest Hourly Revenue</p>
                    {(() => {
                        const topRev = [...hourlyData].sort((a, b) => b.revenue - a.revenue)[0];
                        return (
                            <>
                                <h3 className="text-2xl font-bold text-main mt-1">₹{topRev?.revenue.toLocaleString()}</h3>
                ...
                <div className="bg-white dark:bg-[var(--erp-card)] p-5 rounded-2xl border border-default dark:border-default shadow-sm">
                    ...
                    <p className="text-xs font-bold text-muted dark:text-muted uppercase tracking-wider">Load Recommendation</p>
                    ...
                    <p className="text-xs text-muted mt-1">Based on peak vs average load</p>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-[var(--erp-card)] p-6 rounded-2xl border border-default dark:border-default shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                        <h4 className="font-bold text-main">Transaction Density (24h)</h4>
                        <p className="text-xs text-muted">Identifying peak business hours across the selected period</p>
                    </div>
            ...
            <div className="bg-white dark:bg-[var(--erp-card)] p-6 rounded-2xl border border-default dark:border-default shadow-sm">
                <h4 className="font-bold text-main mb-6">Revenue Spread by Hour</h4>
                <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="bg-white dark:bg-[var(--erp-card)] rounded-2xl border border-default dark:border-default overflow-hidden shadow-sm">
                <div className="p-4 border-b border-default dark:border-default bg-[var(--erp-bg-sunken)]/50 dark:bg-[var(--erp-bg)]/20">
                    <h4 className="font-bold text-main text-sm">Hourly Transaction Ledger</h4>
                </div>
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {hourlyData.filter(d => d.transactions > 0).map((stat) => (
                                <tr key={stat.hour} className="hover:bg-[var(--erp-bg-sunken)] dark:hover:bg-slate-700/30 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-right text-muted dark:text-muted font-mono">
