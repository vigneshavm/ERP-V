...
const NodeTopology: React.FC<{ devices: DeviceRegistryEntry[] }> = ({ devices }) => {
    ...
    return (
        <div className="relative w-full h-[300px] bg-slate-950 rounded-[2.5rem] border border-default overflow-hidden group">
            {/* Grid Background */}
                    ...
                    <React.Fragment key={device.id}>
                        ...
                        >
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center border transition-all ${
                                isOnline 
                                    ? 'bg-[var(--erp-bg)] border-indigo-500/50 text-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.2)]' 
                                    : 'bg-[var(--erp-bg)]/50 border-default text-secondary'
                            }`}>
...
const Sync: React.FC = () => {
    ...
    const [activeSection, setLocalSection] = useState<SyncSection>('device');
    ...
    const getStatusBadge = (status: DeviceStatus, isOnline: boolean) => {
        if (!isOnline) return <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] text-muted flex items-center gap-1"><WifiOff className="w-3 h-3" /> Offline</span>;
    ...
    const getSyncStatusBadge = (status: SyncStatus) => {
        ...
        const Icon = s.icon;
        return (
            <span className={`px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest ${s.bg} ${s.text} flex items-center gap-2 border border-black/5 dark:border-default`}>
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-700">
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                {/* Visualizer Card */}
                <div className="bg-[var(--erp-bg)] rounded-[2.5rem] p-8 border border-default shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 space-y-8">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-main uppercase tracking-tight">Active <span className="text-indigo-400">Topology</span></h3>
                                    <p className="text-muted text-[10px] font-black uppercase tracking-widest mt-0.5">Real-time Node Status</p>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-default/50">
                            {[
                                ...
                                <div key={stat.label} className="text-center">
                                    ...
                                    <p className="text-sm font-black text-main tabular-nums">{stat.value}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="space-y-6 flex flex-col justify-center">
                    <div className="space-y-2">
                        <h1 className="text-4xl font-black text-main leading-tight">Data Sync <br/><span className="text-indigo-600">Cross-Platform Hub</span></h1>
                        <p className="text-muted font-medium text-lg leading-relaxed max-w-md">Seamlessly bridge your branch data between local terminals and the cloud core.</p>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white dark:bg-[var(--erp-bg)] p-6 rounded-3xl border border-default dark:border-default shadow-sm">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-10 h-10 bg-indigo-50 dark:bg-indigo-900/30 rounded-xl flex items-center justify-center">
                                    <Clock className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                                </div>
                                <span className="text-[10px] font-black text-muted uppercase tracking-widest leading-none">Last Sync</span>
                            </div>
                            <p className="text-2xl font-black text-main">{formatTimeAgo(syncConfig.lastSyncAt || '')}</p>
                        </div>
                        <div className="bg-white dark:bg-[var(--erp-bg)] p-6 rounded-3xl border border-default dark:border-default shadow-sm">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <span className="text-[10px] font-black text-muted uppercase tracking-widest leading-none">Devices</span>
                            </div>
                            <p className="text-2xl font-black text-main">{syncConfig.devices.filter((d: any) => d.isOnline).length} / {syncConfig.devices.length} <span className="text-[10px] font-black text-muted uppercase ml-1">Live</span></p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4">
                        <button
                            onClick={handleSync}
                            disabled={isSyncing}
                            className="w-full sm:flex-1 h-16 bg-[var(--erp-bg)] dark:bg-white text-main rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
                        >
                        ...
                        <button 
                            ...
                            className="w-full sm:w-16 h-16 bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default rounded-2xl flex items-center justify-center text-muted hover:text-indigo-600 transition-all hover:scale-[1.02]"
                        >
                    ...
                    {isSyncing && (
                        <div className="w-full bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] h-1 overflow-hidden rounded-full">
            ...
            {/* Tab Navigation */}
            <div className="flex items-center gap-2 bg-white dark:bg-[var(--erp-bg)] p-2 rounded-[1.5rem] border border-default dark:border-default w-fit shadow-sm overflow-x-auto">
                {[
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-3 px-6 py-3.5 rounded-xl text-xs font-black transition-all whitespace-nowrap uppercase tracking-widest ${activeSection === tab.id ? 'bg-indigo-600 text-white shadow-xl' : 'text-muted hover:bg-[var(--erp-bg-sunken)] dark:hover:bg-[var(--erp-card)]'}`}
                        >
            ...
            <div className="animate-in slide-in-from-bottom-4 duration-500">
                ...
                {activeSection === 'add' && (
                    <div className="max-w-2xl mx-auto text-center py-10">
                        <button onClick={() => handleSectionChange('device')} className="text-xs font-black text-muted hover:text-indigo-600 flex items-center gap-2 mx-auto mb-10 transition-all uppercase tracking-widest">
                            <ChevronLeft className="w-4 h-4" /> Back to Dashboard
                        </button>
                        <div className="bg-white dark:bg-[var(--erp-bg)] rounded-[3rem] border border-default dark:border-default p-16 shadow-2xl relative overflow-hidden">
                            ...
                            <h2 className="text-3xl font-black text-main mb-4 italic uppercase">Link New <span className="text-indigo-600">Terminal</span></h2>
                            <p className="text-muted font-medium mb-12 max-w-sm mx-auto">Authorize an additional node to your enterprise network by scanning this cryptographic token.</p>
                            
                            <div className="relative w-64 h-64 mx-auto mb-10 p-6 bg-white dark:bg-[var(--erp-card)] rounded-[2.5rem] border border-default dark:border-default shadow-inner group">
                                <div className="w-full h-full border-4 border-dashed border-default dark:border-default rounded-2xl flex items-center justify-center opacity-40 group-hover:opacity-100 transition-opacity">
                                    <QrCode className="w-40 h-40 text-muted" />
                                </div>
                            ...
                            <div className="flex flex-col items-center gap-3">
                                <span className="px-6 py-2 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] text-muted text-[10px] font-black rounded-full uppercase tracking-widest">
                                    Expires in 04:59
                                </span>
            ...
            <div className="mt-12 p-8 bg-indigo-50/30 dark:bg-indigo-900/10 border border-indigo-100/50 dark:border-indigo-800 rounded-[2.5rem] flex flex-col md:flex-row items-center gap-8">
                <div className="w-20 h-20 bg-white dark:bg-[var(--erp-bg)] rounded-[2rem] shadow-sm flex items-center justify-center shrink-0 border border-default dark:border-default">
                    <ShieldCheck className="w-10 h-10 text-indigo-500" />
                </div>
                <div className="flex-1 text-center md:text-left">
                    <p className="text-lg font-black text-main leading-none">Military-Grade Data Encryption</p>
                    <p className="text-sm text-muted mt-2 font-medium leading-relaxed">All synchronization payload is encrypted using AES-256-GCM before exiting your local terminal. Neither our servers nor unauthorized device nodes can decrypt your sensitive transaction ledgers.</p>
                </div>
                <div className="flex items-center gap-3 bg-white dark:bg-[var(--erp-card)] p-2 rounded-2xl border border-default dark:border-default shadow-sm">
                    <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                    <span className="text-[10px] font-black text-muted uppercase tracking-widest mr-2">E2E ENCRYPTION ACTIVE</span>
                </div>
            </div>
        </div>
    );
