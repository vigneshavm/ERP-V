    ...
    return (
        <div className={`relative w-40 h-40 mx-auto bg-gradient-to-b ${bgColor} rounded-full p-2`}>
            <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
                {/* Background ring */}
                <circle cx="60" cy="60" r="56" fill="none" stroke="currentColor" strokeWidth="6" className="text-slate-100 dark:text-main" />
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-muted mt-0.5">/ 100</span>
            </div>
            {/* Pulse */}
            <div
...
const SecurityMetric: React.FC<{
    ...
    return (
        <div className={`${c.bg} ${c.border} border rounded-3xl p-6 transition-all hover:shadow-md`}>
            ...
            <h4 className="text-[10px] font-black text-muted uppercase tracking-widest mb-1">{label}</h4>
            <p className="text-xl font-black text-main mb-2">{value}</p>
            <p className="text-[10px] text-muted font-medium leading-relaxed">{detail}</p>
        </div>
    );
};
    ...
    return (
        <div className="p-6 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Gauge */}
                <div className="bg-white dark:bg-[var(--erp-bg)] p-8 rounded-3xl border border-default dark:border-default shadow-sm flex flex-col items-center justify-center">
                    <SecurityGauge score={securityScore} />
                    <h3 className="text-[10px] font-black text-muted uppercase tracking-widest mt-4 mb-1">Enterprise Security Score</h3>
            ...
            {/* Account Safeguards */}
            <div className="bg-white dark:bg-[var(--erp-bg)] rounded-[2.5rem] border border-default dark:border-default overflow-hidden shadow-sm">
                <div className="p-8 border-b border-default dark:border-default bg-[var(--erp-bg-sunken)]/50 dark:bg-[var(--erp-bg)]/50 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white dark:bg-[var(--erp-card)] rounded-2xl shadow-sm border border-default dark:border-default flex items-center justify-center">
                            <Lock className="w-6 h-6 text-muted" />
                        </div>
                        <div>
                            <h3 className="text-lg font-black text-main">Account Safeguards</h3>
                            <p className="text-xs text-muted font-bold mt-1">Manage your personal security credentials</p>
                        </div>
                    </div>
                    <button
                        onClick={() => setIsPasswordModalOpen(true)}
                        className="px-8 py-4 bg-[var(--erp-bg)] dark:bg-white text-main border border-transparent dark:border-default text-xs font-black uppercase tracking-widest rounded-2xl hover:opacity-90 transition-all active:scale-95 shadow-xl shadow-slate-200 dark:shadow-none"
                    >
                        Change Access Key
                    </button>
                </div>
            ...
            <section>
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-main leading-none">Password Governance</h3>
                        <p className="text-xs text-muted font-medium mt-1">Manage encryption and data security protocols</p>
                    </div>
                </div>
                ...
                {insecureUsers.length === 0 ? (
                    <div className="p-8 bg-emerald-50/30 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800 rounded-3xl flex flex-col md:flex-row items-center gap-8">
                        <div className="w-20 h-20 rounded-2xl bg-emerald-500 text-main flex items-center justify-center shadow-xl shadow-emerald-200 dark:shadow-none shrink-0">
                            <CheckCircle className="w-10 h-10" />
                        </div>
                        <div className="text-center md:text-left">
                            <h4 className="text-xl font-black text-main leading-none mb-2">Maximum Security Active</h4>
                        ...
                        <div className="px-6 py-2 bg-white dark:bg-[var(--erp-card)] rounded-2xl border border-default dark:border-default text-[10px] font-black tracking-widest text-emerald-600 uppercase">
                            ENCRYPTED
                        </div>
                    </div>
                ) : (
                    <div className="p-8 bg-amber-50/50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-3xl flex flex-col md:flex-row items-center gap-8 shadow-sm">
                        <div className="w-20 h-20 rounded-2xl bg-amber-500 text-main flex items-center justify-center shadow-xl shadow-amber-200 dark:shadow-none shrink-0 border-4 border-white dark:border-default">
                        ...
                        <div className="flex-1 text-center md:text-left">
                            ...
                            <h4 className="text-xl font-black text-main leading-none mb-2">Legacy Credentials Detected</h4>
                            <p className="text-sm text-muted font-medium leading-relaxed max-w-xl">
                        ...
                        <form action={migrationAction}>
                            <button
                                ...
                                className="bg-[var(--erp-bg)] dark:bg-indigo-600 hover:scale-105 text-white px-8 py-4 rounded-2xl text-xs font-black tracking-widest transition-all shadow-xl active:scale-95 disabled:opacity-50 flex items-center gap-3"
                            >
            ...
            <section>
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-main leading-none">Enterprise Roles & ACL</h3>
                            <p className="text-xs text-muted font-medium mt-1">Configure access control levels for each staff role</p>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    {roles.filter((r: any) => r.code !== SystemRole.OWNER).map((role: any) => (
                        <div key={role.id} className="bg-white dark:bg-[var(--erp-bg)] border border-default dark:border-default rounded-3xl p-6 shadow-sm">
                            <div className="flex items-center gap-4 mb-8">
                                <div className="w-12 h-12 rounded-2xl bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] flex items-center justify-center border border-default dark:border-default">
                                    <Users className="w-6 h-6 text-muted" />
                                </div>
                                <div>
                                    <h4 className="text-base font-black text-main leading-none uppercase tracking-wider">{role.description || role.code}</h4>
                                    <p className="text-[10px] text-muted font-bold mt-1">ACCESS LEVEL: {role.code === 'admin' ? 'FULL' : 'GRADED'}</p>
                                </div>
                            </div>
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`p-4 rounded-2xl text-[10px] font-black transition-all border flex flex-col justify-between h-24 text-left ${isAllowed
                                                ...
                                                : 'bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)]/20 border-default dark:border-default text-muted hover:border-indigo-300'
                                                }`}
                                        >
                                            <span className={`opacity-60 text-[8px] uppercase tracking-tighter ${isAllowed ? 'text-main' : ''}`}>{view.category}</span>
                    ...
                    {roles.length === 0 && (
                        <div className="p-20 text-center space-y-4 opacity-30 grayscale">
                            <Users className="w-16 h-16 mx-auto mb-2 text-muted" />
