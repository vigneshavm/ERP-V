...
import { getPayments, updatePaymentStatus } from "@/entities/purchase/model/paymentOutSlice";
            ...
            <div className="bg-white dark:bg-[var(--erp-card)] rounded-xl border border-default dark:border-default shadow-sm overflow-hidden">
                        ...
                        {['all', 'pending', 'cleared', 'bounced'].map(status => (
                            <button
                                ...
                                className={`px-3 py-1 text-xs font-medium rounded-full capitalize ${filterStatus === status
                                    ...
                                    : 'text-muted hover:text-secondary dark:text-muted dark:hover:text-slate-200'
                                    }`}
                            >
                                {status}
                            </button>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-[var(--erp-bg-sunken)] dark:bg-slate-700/50 text-muted font-medium">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {loading ? (
                                ...
                                <tr><td colSpan={7} className="text-center py-8 text-muted">No records found</td></tr>
                            ) : (
                                filteredPayments.map((payment) => (
                                    <tr key={payment._id} className="hover:bg-[var(--erp-bg-sunken)] dark:hover:bg-slate-700/30 transition-colors">
                                        <td className="px-6 py-4 font-medium text-main dark:text-slate-200">
                                            {payment.paymentNo}
                                            {payment.referenceNo && <div className="text-xs text-muted">{payment.referenceNo}</div>}
                                        </td>
                                        <td className="px-6 py-4 text-secondary dark:text-muted">{payment.supplierId?.businessName || 'Unknown Supplier'}</td>
                                        <td className="px-6 py-4 text-muted">{new Date(payment.paymentDate).toLocaleDateString()}</td>
                                        <td className="px-6 py-4">
                                            <span className="px-2 py-1 bg-[var(--erp-bg-sunken)] dark:bg-slate-700 rounded text-xs font-medium text-secondary dark:text-muted">
                                                {payment.paymentMode}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 font-bold text-main dark:text-slate-200">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 rounded-full text-xs font-bold capitalize ${payment.status === 'cleared' ? 'bg-emerald-100 text-emerald-600' :
                                                payment.status === 'bounced' ? 'bg-red-100 text-red-600' :
                                                    payment.status === 'cancelled' ? 'bg-slate-200 text-secondary' :
