        ...
        <Layout>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-[var(--erp-bg)] p-8 rounded-[3rem] border border-default dark:border-default shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-muted dark:text-muted font-medium">Analyze sentiment, protect brand reputation, and automate CX escalations with AI.</p>
                </div>
                ...
                <div className="flex items-center gap-3 relative z-10">
                    ...
                    <button className="flex items-center gap-3 px-6 py-4 bg-[var(--erp-bg)] dark:bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:scale-105 transition-all">
            ...
            <div className="flex items-center gap-2 p-1.5 bg-white dark:bg-[var(--erp-bg)] border border-default dark:border-default rounded-[2rem] w-fit shadow-sm">
                {(['OVERVIEW', 'STREAM', 'ESCALATIONS', 'BENCHMARKS'] as const).map((tab) => (
                    <button
                        ...
                        className={`px-8 py-3 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeSection === tab ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-muted hover:text-main dark:hover:text-slate-200'}`}
                    >
                        {tab}
                    </button>
                ))}
                        ...
                        <div className="col-span-12 lg:col-span-8 bg-white dark:bg-[var(--erp-card)] rounded-[3rem] p-8 border border-default dark:border-default shadow-sm">
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-bold text-muted uppercase tracking-widest">Positive</span>
                                    <span className="w-3 h-3 bg-rose-500 rounded-full ml-2" />
                                    <span className="text-[10px] font-bold text-muted uppercase tracking-widest">Negative</span>
                                </div>
                            </div>
                            <div className="space-y-6">
                                {[
                                    ...
                                    <div key={idx} className="space-y-2">
                                        ...
                                        <div className="h-4 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-bg)] rounded-full overflow-hidden flex">
                                        ...
                                        <div className="flex justify-between">
                                            <p className="text-[9px] font-bold text-muted uppercase tracking-widest">Sentiment: {row.score > 80 ? 'Strongly Positive' : row.score > 60 ? 'Positive' : 'Critical'}</p>
                        ...
                        <div className="col-span-12 lg:col-span-4 space-y-8">
                            <div className="bg-[var(--erp-bg)] dark:bg-indigo-950 rounded-[3rem] p-8 text-white relative overflow-hidden group shadow-2xl">
                                ...
                                <div className="space-y-4">
                                    <div className="p-6 bg-[var(--erp-bg-sunken)] border border-default rounded-2xl">
                                        <h4 className="text-2xl font-black text-emerald-400 mb-1">124</h4>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-4">Promoters Ready for Review</p>
                                    ...
                                    <div className="p-6 bg-rose-500/10 border border-rose-500/20 rounded-2xl">
                                        ...
                                        <p className="text-xs text-muted font-medium italic">Route to VIP resolution immediately.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="bg-white dark:bg-[var(--erp-bg)] rounded-[3rem] border border-default dark:border-default shadow-sm overflow-hidden animate-in slide-in-from-right-4 duration-500">
                    <div className="p-8 border-b border-default dark:border-default flex items-center justify-between bg-[var(--erp-bg-sunken)]/50 dark:bg-[var(--erp-bg)]/50">
                        <div>
                            ...
                            <p className="text-xs font-bold text-muted uppercase tracking-widest mt-1">Real-time multi-channel CX feed</p>
                        </div>
                        <div className="flex items-center gap-4">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="pl-11 pr-4 py-3 bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default rounded-xl text-xs font-bold w-64 outline-none"
                                />
                            </div>
                            <button className="p-3 bg-white dark:bg-[var(--erp-card)] rounded-xl text-muted border border-default dark:border-default shadow-sm">
                    ...
                    <div className="divide-y divide-slate-50 dark:divide-slate-800">
                        {[
                            ...
                            <div key={idx} className="p-8 hover:bg-[var(--erp-bg-sunken)]/50 transition-colors group cursor-pointer relative overflow-hidden">
                                {row.sentiment === 'NEGATIVE' && <div className="absolute left-0 top-0 bottom-0 w-1 bg-rose-500" />}
                                <div className="flex items-start justify-between relative z-10">
                                    <div className="flex items-start gap-6">
                                        <div className="flex flex-col items-center gap-1">
                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg ${row.sentiment === 'POSITIVE' ? 'bg-emerald-500/10 text-emerald-600' : row.sentiment === 'NEGATIVE' ? 'bg-rose-500/10 text-rose-600' : 'bg-slate-500/10 text-secondary'}`}>
                                                {row.rating}
                                            </div>
                                            <span className="text-[8px] font-black uppercase text-muted tracking-widest">NPS</span>
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-3 mb-1">
                                                <h4 className="font-black text-main uppercase tracking-tight">{row.name}</h4>
                                            ...
                                            <p className="text-sm font-medium italic text-secondary dark:text-muted max-w-2xl mb-4 leading-relaxed">"{row.comment}"</p>
                                            <div className="flex items-center gap-2">
                                                {row.tags.map(tag => (
                                                    <span key={tag} className="px-3 py-1 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-card)] rounded-full text-[9px] font-black uppercase tracking-widest text-muted">
                                                        #{tag}
                                                    </span>
                                                ))}
                                    ...
                                    <div className="text-right flex flex-col items-end gap-3">
                                        <p className="text-[10px] font-bold text-muted uppercase tracking-widest">{row.time}</p>
                                        <div className="flex gap-2">
                                            <button className="p-3 bg-white dark:bg-[var(--erp-card)] border border-default dark:border-default rounded-xl text-muted hover:text-emerald-600 transition-all shadow-sm">
                    ...
                    <div className="p-8 bg-[var(--erp-bg-sunken)]/50 dark:bg-[var(--erp-bg)]/50 text-center">
