...
export const StockHistoryDrawer: React.FC<StockHistoryDrawerProps> = ({ isOpen, onClose, itemName, history, isLoading }) => {
    ...
    return (
        <div className="fixed inset-0 z-[150] flex justify-end animate-in fade-in duration-300">
            ...
            <div className="relative w-full max-w-lg bg-white dark:bg-[var(--erp-card)] h-full shadow-2xl border-l border-default dark:border-default flex flex-col animate-in slide-in-from-right duration-500">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6">
                    {isLoading ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center">
                            <div className="w-16 h-16 bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-bg)] rounded-full flex items-center justify-center mb-4 border border-default dark:border-default">
                        ...
                        <div className="relative space-y-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-px before:bg-[var(--erp-bg-sunken)] dark:before:bg-neutral-700">
                            {history.map((log) => (
                                <div key={log._id} className="relative pl-10 group">
                                    <div className="absolute left-0 top-1 w-6 h-6 rounded-lg bg-white dark:bg-[var(--erp-bg)] border border-default dark:border-default flex items-center justify-center z-10 shadow-sm group-hover:border-primary transition-all">
                                        {getLogIcon(log.type)}
                                    </div>
                                    <div className="flex flex-col">
                                        <div className="flex justify-between items-start">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-neutral-800 dark:text-main">{getLogLabel(log.type)}</span>
                                            <span className="text-[9px] font-bold text-neutral-400 bg-[var(--erp-bg-sunken)] dark:bg-neutral-700 px-2 py-0.5 rounded uppercase">
                                                {new Date(log.createdAt).toLocaleString('en-IN', { month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false })}
                                            </span>
                                        </div>
                                        <div className="mt-2 p-3 rounded-xl bg-[var(--erp-bg-sunken)] dark:bg-[var(--erp-bg)]/50 border border-default dark:border-default">
                ...
                <div className="p-8 border-t border-default dark:border-default bg-[var(--erp-bg-sunken)]/50 dark:bg-neutral-700/50">
                    <button onClick={onClose} className="w-full py-4 bg-[var(--erp-bg)] dark:bg-[var(--erp-bg-sunken)] text-main dark:text-neutral-900 rounded-xl font-black uppercase tracking-widest text-[10px] hover:scale-[1.02] active:scale-95 transition-all shadow-xl">
                        Dismiss Audit Panel
                    </button>
                </div>
            </div>
