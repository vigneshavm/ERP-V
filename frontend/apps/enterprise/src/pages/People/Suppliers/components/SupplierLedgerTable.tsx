...
const SupplierLedgerTable: React.FC<SupplierLedgerTableProps> = ({ data, loading, onRowClick }) => {
    ...
    const SortIcon = ({ column }: { column: SortKey }) => {
        if (sortConfig?.key !== column) return <ArrowUpDown className="w-3 h-3 text-muted dark:text-neutral-600 opacity-50 group-hover:opacity-100" />;
    ...
    const renderHeader = (label: string, key: SortKey, align: 'left' | 'right' = 'left') => (
        <th
            className={`py-3 px-5 font-bold text-muted dark:text-neutral-500 uppercase text-[11px] tracking-wider text-${align} whitespace-nowrap cursor-pointer group hover:bg-[var(--erp-bg-sunken)] dark:hover:bg-[var(--erp-card)] transition-colors select-none`}
    ...
    return (
        <div className="bg-white dark:bg-[var(--erp-card)] rounded-2xl border border-default dark:border-default shadow-sm overflow-hidden relative">
            {/* Table Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-default dark:border-default print:hidden">
                <div>
                    <h3 className="text-sm font-bold text-main dark:text-neutral-100 flex items-center gap-2">
                        <Book className="w-4 h-4 text-indigo-500" /> Transaction History
                    </h3>
                    <p className="text-[10px] text-muted dark:text-neutral-500 font-bold uppercase tracking-widest mt-0.5">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    <thead className="bg-[var(--erp-bg-sunken)]/80 dark:bg-[var(--erp-bg)]/50">
                        <tr>
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-neutral-700/50">
                        {/* Opening Balance Row */}
                        <tr className="bg-[var(--erp-bg-sunken)]/30 dark:bg-[var(--erp-card)]/50">
                            <td className="py-3.5 px-5 text-muted font-medium text-sm whitespace-nowrap">{new Date(data.period.start).toLocaleDateString('en-IN')}</td>
                            <td className="py-3.5 px-5 whitespace-nowrap">
                                <span className="px-2 py-0.5 rounded-md bg-[var(--erp-bg-sunken)] dark:bg-neutral-700 text-[10px] font-black text-muted dark:text-neutral-400 uppercase tracking-wider">Opening</span>
                            </td>
                            <td className="py-3.5 px-5 italic text-muted dark:text-neutral-500 text-xs whitespace-nowrap">Opening Balance Forwarded</td>
                            <td className="py-3.5 px-5 text-right text-slate-200 dark:text-neutral-700 whitespace-nowrap">—</td>
                            <td className="py-3.5 px-5 text-right text-slate-200 dark:text-neutral-700 whitespace-nowrap">—</td>
                            <td className="py-3.5 px-5 text-right font-black text-secondary dark:text-neutral-300 whitespace-nowrap text-sm">{formatCurrency(data.openingBalance)}</td>
                        </tr>
                        ...
                        {sortedTransactions.map((t, i) => (
                            ...
                            >
                                <td className="py-3.5 px-5 text-secondary dark:text-neutral-400 font-medium text-sm whitespace-nowrap">{new Date(t.date).toLocaleDateString('en-IN')}</td>
                                <td className="py-3.5 px-5 whitespace-nowrap">
                                ...
                                <td className="py-3.5 px-5 whitespace-nowrap">
                                    <div className="font-bold text-main dark:text-neutral-200 text-sm group-hover:text-indigo-500 transition-colors">#{t.refNo}</div>
                                    <div className="text-xs text-muted dark:text-neutral-500 mt-0.5 truncate max-w-[200px]">{t.description}</div>
                                </td>
                                ...
                                <td className="py-3.5 px-5 text-right font-black text-main text-sm whitespace-nowrap">
                                    {formatCurrency(t.balance)}
                                    <span className="text-[9px] ml-1 text-muted dark:text-neutral-500 font-bold">
                                        {t.balance > 0 ? 'Cr' : 'Dr'}
                                    </span>
                                </td>
                            </tr>
                    ...
                    <tfoot>
                        <tr className="bg-[var(--erp-bg-sunken)]/80 dark:bg-[var(--erp-bg)]/50">
                            <td colSpan={3} className="py-4 px-5 text-right text-[11px] font-bold text-muted dark:text-neutral-400 uppercase tracking-wider">Closing Balance</td>
