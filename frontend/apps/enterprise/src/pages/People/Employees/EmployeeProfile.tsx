...
const EmployeeProfile: React.FC = () => {
    ...
    const StatCard = ({ icon: Icon, label, value, color }: any) => (
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none">{label}</p>
            <p className="text-xl font-black text-slate-800 dark:text-white mt-1.5">{value}</p>
        </div>
    );
        ...
        <Layout>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <PageHeader 
                    ...
                    actions={
                        <button className="bg-white dark:bg-slate-800 text-slate-700 dark:text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-sm border border-slate-200 dark:border-slate-700 hover:border-blue-500 transition-all">
                            <Edit3 size={18} />
                            Modify Profile
                        </button>
                    }
                ...
                <div className="grid lg:grid-cols-4 gap-8">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 text-center relative overflow-hidden">
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        {/* Tabs */}
                        <div className="flex p-1.5 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-x-auto">
                            {[
                                        ...
                                        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                            ...
                                            <div className="space-y-4">
                                                <div className="flex justify-between py-2 border-b border-slate-50 dark:border-slate-700/50">
                                                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Base Salary</span>
                                                    <span className="text-sm font-black text-slate-700 dark:text-slate-200">{formatCurrency(employee.baseSalary || 0)} / {employee.wageType?.toLowerCase()}</span>
                                                </div>
                                                <div className="flex justify-between py-2 border-b border-slate-50 dark:border-slate-700/50">
                                                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Branch Unit</span>
                                                    <span className="text-sm font-black text-slate-700 dark:text-slate-200">{employee.branchId || 'Headquarters'}</span>
                                                </div>
                                                <div className="flex justify-between py-2 border-b border-slate-50 dark:border-slate-700/50">
                                                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Employment Type</span>
                                                    <span className="text-sm font-black text-slate-700 dark:text-slate-200">Full-Time</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                            ...
                                            <div className="space-y-4">
                                                {[
                                                    ...
                                                    <div key={i} className="flex gap-4">
                                                        ...
                                                        <div>
                                                            <p className="text-sm font-bold text-slate-700 dark:text-slate-200 leading-none">{item.label}</p>
                                                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1.5">{item.date} // {item.type}</p>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                            ...
                            {activeTab === 'DOCUMENTS' && (
                                <div className="grid md:grid-cols-2 gap-4">
                                    {[
                                        ...
                                        <div key={i} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between group hover:border-blue-500/50 transition-all cursor-pointer">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-blue-600">
                                                    <FileText size={20} />
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-slate-800 dark:text-white truncate max-w-[150px]">{doc.name}</p>
                                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{doc.size} // {doc.date}</p>
                                                </div>
                                            </div>
