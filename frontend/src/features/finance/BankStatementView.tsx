...
const SummaryCard: React.FC<SummaryCardProps> = ({ label, value, sub, icon, colorClass, bgClass }) => (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-[2rem] p-6 shadow-sm flex items-start gap-4 group hover:border-primary/20 transition-all duration-500">
            ...
            <div className="pt-8 space-y-10 pb-32">
                    ...
                    actions={
                        <div className="flex gap-3">
                            <button
                                ...
                                disabled={fetching}
                                className="px-5 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-black flex items-center gap-2 hover:bg-slate-50 shadow-sm transition active:scale-95 uppercase tracking-widest disabled:opacity-50"
                            >
                ...
                <div
                    ...
                    className={`relative border-2 border-dashed rounded-xl p-12 cursor-pointer transition-all duration-700 text-center flex flex-col items-center gap-6 group overflow-hidden ${
                        dragging
                            ...
                            : 'border-slate-200 dark:border-slate-800 hover:border-primary/40 hover:bg-slate-50/50 dark:hover:bg-slate-900/30'
                ...
                >
                    ...
                    <div>
                        ...
                        {!loading && (
                            <p className="text-xs font-bold text-slate-400 mt-2 uppercase tracking-widest italic">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-6 duration-700">
                    <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                        ...
                        <div className="flex items-center gap-2 p-1.5 bg-slate-50 dark:bg-slate-900 rounded-sm border border-slate-100 dark:border-slate-800">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800">
                                <tr>
                                    <th
                                        className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary transition-colors"
                                        onClick={() => toggleSort('date')}
                                    >
                                        Fiscal Date <SortIcon field="date" />
                                    </th>
                                    <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Narration</th>
                                ...
                                {fetching ? (
                                    Array.from({ length: 5 }).map((_, i) => (
                                        <tr key={i} className="animate-pulse">
                                            <td className="px-8 py-6"><div className="h-2 w-20 bg-slate-100 dark:bg-slate-900 rounded-full" /></td>
                                            <td className="px-8 py-6"><div className="h-2 w-48 bg-slate-100 dark:bg-slate-900 rounded-full" /></td>
                                            <td className="px-8 py-6"><div className="h-4 w-16 bg-slate-100 dark:bg-slate-900 rounded-full" /></td>
                                            <td className="px-8 py-6"><div className="h-2 w-24 bg-slate-100 dark:bg-slate-900 rounded-full ml-auto" /></td>
                                            <td className="px-8 py-6"><div className="h-2 w-24 bg-slate-100 dark:bg-slate-900 rounded-full ml-auto" /></td>
                                            <td className="px-8 py-6"><div className="h-4 w-20 bg-slate-100 dark:bg-slate-900 rounded-full mx-auto" /></td>
                                        </tr>
                                    ))
                                ) : displayed.length === 0 ? (
                                    <tr>
                                    ...
                                    displayed.map((tx, __idx) => (
                                        <tr key={tx._id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition-all cursor-default">
                                            <td className="px-8 py-6 whitespace-nowrap">
                                                <span className="text-xs font-black text-slate-500 font-mono tracking-tighter uppercase">{fmtDate(tx.date)}</span>
                                            </td>
                                            <td className="px-8 py-6 max-w-xs">
                                                <p className="text-xs font-bold text-slate-900 dark:text-white truncate uppercase tracking-tighter" title={tx.description}>{tx.description}</p>
                                                {tx.reference && (
                                                    <p className="text-[10px] font-black text-slate-400 mt-1 font-mono uppercase tracking-widest">REF: {tx.reference}</p>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-8 py-6 text-right whitespace-nowrap text-xs font-black text-slate-400 font-mono hidden md:table-cell tabular-nums">
                    ...
                    {displayed.length > 0 && (
                        <div className="px-8 py-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-wrap items-center justify-between gap-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <div className="flex items-center gap-4">
                                <span className="text-slate-500">{displayed.length} NODES AUDITED</span>
