                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                            <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 mb-4">Count Physical Cash</h3>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                {DENOMINATIONS.map((d) => (
                                    <div key={d}>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">₹{d} Notes</label>
                                        <div className="flex items-center gap-2 mt-1">
                                            <input
                                                ...
                                                placeholder="0"
                                                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-primary/30"
                                            />
                                ...
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Coins / Loose Change (₹)</label>
                                    <input
                                        type="number"
                                        min="0"
                                        value={coinsAmount}
                                        ...
                                        placeholder="0"
                                        className="w-full px-3 py-2 mt-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold outline-none focus:ring-2 focus:ring-primary/30"
                                    />
                                </div>
                            </div>
                            <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                                <span className="text-xs font-black uppercase tracking-widest text-slate-500">Total Counted</span>
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                            ...
                            <div className="mt-4 space-y-2">
                                ...
                                <textarea
                                    ...
                                    rows={2}
                                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium outline-none focus:ring-2 focus:ring-primary/30 resize-none"
                    ...
                    <div className="space-y-6">
                        <div className="bg-slate-900 text-white p-6 rounded-[2rem] shadow-xl">
                            ...
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between"><span className="text-slate-400">Opening Cash</span><span className="font-bold">₹{formatCurrency(summary.openingCash)}</span></div>
                                <div className="flex justify-between"><span className="text-slate-400">+ Cash Sales</span><span className="font-bold text-success">₹{formatCurrency(summary.cashSales)}</span></div>
                                <div className="flex justify-between"><span className="text-slate-400">- Cash Expenses</span><span className="font-bold text-error">₹{formatCurrency(summary.cashExpenses)}</span></div>
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between"><span className="text-slate-500">Card Sales</span><span className="font-bold">₹{formatCurrency(summary.cardAmount)}</span></div>
                                <div className="flex justify-between"><span className="text-slate-500">Credit / Due Sales</span><span className="font-bold">₹{formatCurrency(summary.creditAmount)}</span></div>
                            </div>
                        </div>
                        ...
                        <div className={`p-6 rounded-[2rem] border ${variance === 0 ? 'bg-success/10 border-success/20' : variance > 0 ? 'bg-success/10 border-success/20' : 'bg-error/10 border-error/20'}`}>
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Variance</p>
                                ...
                                <div className="space-y-3">
                                    {history.map((h) => (
                                        <div key={h._id} className="flex items-center justify-between text-xs">
                                            <span className="font-bold text-slate-500">{new Date(h.date).toLocaleDateString('en-IN')}</span>
