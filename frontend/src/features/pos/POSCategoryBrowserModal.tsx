...
export const POSCategoryBrowserModal: React.FC<POSCategoryBrowserModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-4xl max-h-[85vh] rounded-sm shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                {/* Header */}
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 hover:bg-white dark:hover:bg-slate-700 rounded-xl transition-colors text-slate-400 hover:text-error"
                    >
                        <X className="w-6 h-6" />
                    </button>
                </div>
                    ...
                    <div className="px-6 py-4 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-primary rounded-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 transition-all outline-none"
                                value={searchQuery}
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            {categories.filter(c => c !== 'All').map(cat => (
                                <button
                                    ...
                                    className="group p-6 bg-slate-50 dark:bg-slate-800/50 hover:bg-primary rounded-sm transition-all duration-300 flex flex-col items-center gap-4 border-2 border-transparent hover:border-primary/40 hover:shadow-xl hover:shadow-primary/20"
                                >
                                    <div className="w-16 h-16 bg-white dark:bg-slate-700 rounded-sm flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            {filteredSubcategories.map(sub => (
                                <button
                                    ...
                                    className="p-6 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 hover:border-primary rounded-sm transition-all flex flex-col items-center gap-3 hover:shadow-lg"
                                >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                            {filteredProducts.map(product => (
                                <button
                                    ...
                                    className={`text-left p-4 bg-slate-50 dark:bg-slate-800/50 hover:bg-white dark:hover:bg-slate-800 border-2 border-transparent hover:border-primary rounded-sm transition-all flex flex-col gap-2 ${product.stockQty <= 0 ? 'opacity-50 grayscale' : ''}`}
                                >
