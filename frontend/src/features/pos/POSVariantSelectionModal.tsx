...
export const POSVariantSelectionModal: React.FC<POSVariantSelectionModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-800 rounded-sm shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                            ...
                            {uniqueColors.map(color => (
                                <tr key={color}>
                                    ...
                                    {uniqueSizes.map(size => {
                                        ...
                                        return (
                                            <td key={size} className={`p-2 border dark:border-slate-700 ${qty > 0 ? 'bg-primary/10 dark:bg-primary/20' : ''}`}>
                                                <div className="flex flex-col gap-1 items-center">
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        <span className={`text-[10px] font-mono ${isOutOfStock ? 'text-error' : 'text-slate-400'}`}>
                                                            {isOutOfStock ? 'Out' : `${product.stockQty} left`}
                                                        </span>
                                                        {product.sellingPrice !== baseProduct.sellingPrice && (
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            ...
                            disabled={totalQty === 0}
                            className="px-6 py-2 bg-primary hover:bg-primary/90 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-lg font-bold shadow-lg shadow-primary/20 flex items-center gap-2 transition-all"
                        >
