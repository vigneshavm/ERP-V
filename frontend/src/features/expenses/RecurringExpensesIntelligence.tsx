        ...
        return (
            <div className="flex items-center justify-center h-96">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm font-black text-slate-400 uppercase tracking-widest">Running Fixed-Cost Analysis...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="space-y-6 animate-fade-in text-slate-900 dark:text-slate-100 pb-20">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                        <button className="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 hover:bg-slate-50 shadow-sm transition-all active:scale-95">
                            <FileText className="w-4 h-4" /> Policy Audit
                        </button>
                        <button className="px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 hover:bg-slate-800 shadow-xl transition-all active:scale-95">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm group">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Target Monthly Burn</p>
                        <h3 className="text-2xl font-black tabular-nums">₹{recurringExpenses.reduce((s, e) => s + e.amount, 0).toLocaleString()}</h3>
                        <div className="flex items-center gap-1.5 mt-2 text-slate-500 font-bold text-[10px] uppercase">
                            <Clock className="w-3.5 h-3.5" /> Next cycle: 1st Feb
                        </div>
                    </div>

                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Avg Fixed Cost Ratio</p>
                        ...
                        <div className="w-full h-1 bg-slate-100 dark:bg-slate-900 rounded-full mt-3 overflow-hidden">
                    ...
                    <div className="bg-slate-900 text-white p-6 rounded-[2rem] shadow-xl border border-slate-800 relative group overflow-hidden">
                        ...
                        <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-1 italic">Suggestions</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                            ...
                            <div className="divide-y divide-slate-50 dark:divide-slate-800 text-sm">
                                {intelligence.summaryByBranch.map((branch, idx) => (
                                    <div key={idx} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2 flex items-center gap-1.5">
                                                    Sales: ₹{branch.monthly_sales.toLocaleString()} <span className="text-slate-200">|</span> GP: {branch.cost_ratio}
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className="text-right mr-2">
                                                ...
                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Monthly total</p>
                                            </div>
                        ...
                        <div className="bg-slate-900 text-white rounded-[2.5rem] p-8 border border-slate-800 shadow-2xl relative overflow-hidden group">
                                        ...
                                        <div key={idx} className="bg-white/5 border border-white/10 p-5 rounded-sm flex items-center justify-between hover:bg-white/10 transition-all">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">{due.vendor}</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className={`font-black text-sm ${due.status === 'OVERDUE' ? 'text-error' : 'text-slate-300'}`}>₹{due.amount.toLocaleString()}</p>
                    ...
                    <div className="space-y-4">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Anomaly Guard</h4>
                        ...
                        <div className="p-5 rounded-sm border border-slate-100 bg-slate-50/50 flex items-start gap-4">
                            <div className="p-2 bg-white rounded-xl text-slate-400 shrink-0 shadow-sm">
                                <Info className="w-5 h-5" />
                            </div>
                            <p className="text-[10px] text-slate-500 font-bold leading-relaxed uppercase tracking-tight">
                                Recurring Intelligence ensures <span className="text-slate-900 italic">Zero-Surprise Month Ends</span> through continuous cashflow anticipation.
                            </p>
                        </div>
                    </div>
                </div>
