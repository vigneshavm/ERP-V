            ...
            <div className="space-y-6 animate-fade-in text-slate-900 dark:text-slate-100 pb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Policy Coverage</p>
                        <h3 className="text-3xl font-black">{categories.length} <span className="text-xs text-slate-400 font-black uppercase">Active Categories</span></h3>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Budget Cap</p>
                        <h3 className="text-3xl font-black tabular-nums">₹{categories.reduce((s, c) => s + c.monthly_budget, 0).toLocaleString()}</h3>
                        <p className="text-[10px] text-slate-500 mt-2 font-bold uppercase tracking-tight">Across all organizational units</p>
                    </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                    <div className="p-6 border-b border-slate-50 dark:border-slate-800 flex flex-col md:flex-row gap-4 items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <button className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl text-slate-400 hover:text-primary transition-colors">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {filteredCategories.map(cat => (
                                    <tr key={cat.id} className="group hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="p-6 text-sm text-slate-500 font-bold">{groupName(cat.groupId)}</td>
                                        <td className="p-6">
                                            <p className="font-bold text-sm tracking-tight italic">₹{cat.monthly_budget.toLocaleString()}</p>
                                            <p className="text-[9px] text-slate-400 font-black uppercase tracking-tighter">Per Month Cap</p>
                                        </td>
                                        <td className="p-6">
                                            {cat.is_cash_allowed ? (
                                        ...
                                        <td className="p-6">
                                            {cat.approval_required ? (
                                                ...
                                                <span className="px-2.5 py-1 bg-slate-100 text-slate-500 text-[9px] font-black rounded-lg uppercase tracking-widest">Auto-Approve</span>
                                            )}
                                        </td>
                ...
                {isModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 duration-200">
                            <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                                <div>
                                    <h3 className="text-xl font-black tracking-tight">{editingCategory?.id ? 'Edit Policy' : 'New Spend Policy'}</h3>
                                    <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Master Control Layer</p>
                                </div>
                            ...
                            <form onSubmit={handleSave} className="p-8 space-y-6">
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input
                                            required
                                            className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-sm text-sm font-bold focus:ring-2 focus:ring-primary/20"
                                    ...
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Expense Group</label>
                                        <select
                                            className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-sm text-sm font-bold focus:ring-2 focus:ring-primary/20"
                                        ...
                                        {expenseGroups.length === 0 && (
                                            <p className="text-[10px] text-slate-400 mt-1.5 ml-1">No expense groups yet — add one in Settings → Master Data.</p>
                                        )}
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Monthly Budget Cap (₹)</label>
                                        <input
                                            type="number"
                                            required
                                            className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-sm text-sm font-bold focus:ring-2 focus:ring-primary/20 tabular-nums"
                                            placeholder="100000"
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-sm flex items-center justify-between">
                                            <div>
                                                <p className="text-[10px] font-black uppercase tracking-widest">Cash Allowed</p>
                                                <p className="text-[9px] text-slate-400 font-bold">Bypass Bank requirement</p>
                                            </div>
                                            <button
                                                type="button"
                                                onClick={() => setEditingCategory(prev => ({ ...prev!, is_cash_allowed: !prev!.is_cash_allowed }))}
                                                className={`w-12 h-6 rounded-full transition-all relative ${editingCategory?.is_cash_allowed ? 'bg-primary' : 'bg-slate-200'}`}
                                            >
                                        ...
                                        <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-sm flex items-center justify-between">
                                            <div>
                                                ...
                                                <p className="text-[9px] text-slate-400 font-bold">Mandate owner sign-off</p>
                                            </div>
                                            <button
                                                type="button"
                                                onClick={() => setEditingCategory(prev => ({ ...prev!, approval_required: !prev!.approval_required }))}
                                                className={`w-12 h-6 rounded-full transition-all relative ${editingCategory?.approval_required ? 'bg-primary' : 'bg-slate-200'}`}
                                            >
