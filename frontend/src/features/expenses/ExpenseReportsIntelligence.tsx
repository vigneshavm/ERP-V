            ...
            <div className="space-y-6 animate-fade-in text-slate-900 dark:text-slate-100 pb-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Gross Burn</p>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Fixed Cost Ratio</p>
                        ...
                        <p className="text-[10px] text-slate-500 mt-2 font-bold uppercase tracking-tight">Focusing on scalability</p>
                    </div>

                    <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm relative border-l-4 border-l-error">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Audit Flags</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm p-8">
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <span className="text-[10px] font-medium text-slate-400 italic">Top 80% contributors highlighted</span>
                            </div>
                                    ...
                                    <div key={idx} className="group">
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className={`text-[8px] px-1.5 py-0.5 rounded uppercase font-black tracking-widest ${cat.type === 'FIXED' ? 'bg-slate-100 text-slate-500' : 'bg-primary/10 text-primary'}`}>
                                                    {cat.type}
                                                </span>
                                            </div>
                                            <div className="text-right">
                                                <span className="text-xs font-black italic mr-3">₹{cat.amount.toLocaleString()}</span>
                                                <span className="text-[10px] font-black text-slate-400 tabular-nums">{cat.percentage}</span>
                                            </div>
                                        </div>
                                        <div className="w-full h-2 bg-slate-50 dark:bg-slate-900 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full rounded-full transition-all duration-1000 ${cat.type === 'FIXED' ? 'bg-slate-300' : 'bg-primary'}`}
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                ...
                                <div className="divide-y divide-slate-50 dark:divide-slate-800">
                                    {report.by_branch.length === 0 && (
                                        <p className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Expenses aren't recorded by branch, so there is no branch breakdown.</p>
                                    )}
                                    {report.by_branch.map((branch, idx) => (
                                        <div key={idx} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                                            <div>
                                                <p className="text-xs font-black tracking-tight">{branch.branch}</p>
                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">₹{branch.amount.toLocaleString()} Allocated</p>
                                            </div>
                                    ...
                                    {report?.by_payment_mode?.map((mode, idx) => (
                                        <div key={idx} className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-black uppercase tracking-widest text-slate-500">{mode.mode}</span>
                                            </div>
                                            <span className="text-xs font-black italic">₹{mode.amount.toLocaleString()}</span>
                                        </div>
                                    ))}
                                    <div className="pt-4 border-t border-slate-100 flex items-center gap-2 text-[10px] font-bold text-slate-400 italic">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="p-6 rounded-sm bg-slate-900 text-white relative overflow-hidden">
                            ...
                            <div className="flex items-end gap-2">
                                ...
                                <span className="text-[10px] font-bold text-slate-500 mb-1">Wings Standard</span>
                            </div>
                        </div>
                    </div>
                </div>
