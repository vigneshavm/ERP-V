...
const MarketingTools: React.FC = () => {
    ...
    const themes: Theme[] = [
        { id: 'indigo', name: 'Royal Indigo', color: 'bg-primary', gradient: 'from-indigo-600 to-violet-700' },
        { id: 'rose', name: 'Rose Gold', color: 'bg-danger', gradient: 'from-rose-500 to-pink-600' },
        { id: 'emerald', name: 'Deep Emerald', color: 'bg-success', gradient: 'from-emerald-600 to-teal-700' },
        { id: 'amber', name: 'Sunset Amber', color: 'bg-warning', gradient: 'from-amber-500 to-orange-600' },
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div className="lg:col-span-8 space-y-8">
                            ...
                            <motion.div variants={itemVariants} className="bg-white rounded-sm shadow-sm border border-slate-200 p-8 overflow-hidden relative">
                                <div className="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-primary-soft rounded-full blur-3xl opacity-50" />
                                <div className="relative z-10">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {templates.map((template) => (
                                            ...
                                            >
                                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 shadow-sm ${selectedTemplate?.id === template.id ? 'bg-primary text-white' : 'bg-white text-slate-600'
                                                    }`}>
                                                    {template.icon}
                                                </div>
                                                <h3 className="font-bold text-slate-900 mb-1 flex items-center">
                                                    {template.name}
                                                    {template.id === 1 && <span className="ml-2 w-2 h-2 bg-primary rounded-full animate-pulse" />}
                                                </h3>
                                                <p className="text-xs text-slate-500 mb-4 line-clamp-2 leading-relaxed">{template.description}</p>
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[10px] uppercase tracking-wider font-bold text-primary bg-primary-soft px-2 py-0.5 rounded">
                                                        {template.category}
                                                    </span>
                                ...
                                <div className="p-8 space-y-8">
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            rows={4}
                                            className="w-full px-5 py-4 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-primary transition-all outline-none text-slate-700 bg-slate-50/30 placeholder:text-slate-400"
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-4">
                                            ...
                                            <input
                                                ...
                                                className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-primary transition-all outline-none"
                                                placeholder="Public Image URL (Required for API)"
                                            />

                                            <label className="group relative border-2 border-dashed border-slate-200 rounded-sm p-6 flex flex-col items-center justify-center hover:bg-slate-50 hover:border-primary/30 transition-all cursor-pointer overflow-hidden">
                                                ...
                                                {uploadedImage ? (
                                                    ...
                                                    <div className="w-12 h-12 bg-slate-50 text-slate-400 rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 group-hover:bg-primary-soft group-hover:text-primary transition-all">
                            ...
                            <motion.div variants={itemVariants} className="sticky top-8 space-y-8">
                                                    ...
                                                    >
                                                        ...
                                                        <div className="p-5">
                                                            <div className="flex items-center space-x-2 mb-3">
                                                                <span className="text-[9px] font-bold text-primary uppercase tracking-widest bg-primary-soft px-2 py-0.5 rounded">
                                ...
                                <div className="space-y-3">
                                    <motion.button
                                        ...
                                        className="btn-primary w-full py-4 bg-primary hover:bg-primary-hover text-white rounded-sm font-bold shadow-xl shadow-indigo-200 flex items-center justify-center space-x-2 relative overflow-hidden"
                                    >
                                        {isGenerating ? (
                                    ...
                                    {instagramAccounts.length > 0 ? (
                                        <div className="mb-2">
                                            ...
                                            <select
                                                ...
                                                className="w-full px-4 py-2 rounded-xl border border-primary/30 text-xs font-bold text-slate-700 bg-primary/50 outline-none focus:ring-2 focus:ring-indigo-500 mb-2"
                                            >
                                        ...
                                        <div className="mb-2 p-3 bg-warning-soft rounded-xl border border-warning-line text-warning text-xs font-medium text-center">
                                            No Instagram accounts connected.
                                            <button onClick={handleConnect} className="text-primary font-bold underline ml-1 hover:text-primary">Connect Now</button>
                                        </div>
                                    )}
                                ...
                                <div className="bg-gradient-to-br from-indigo-900 to-indigo-950 rounded-[2rem] p-8 text-white relative overflow-hidden shadow-2xl">
                                    <div className="absolute top-0 right-0 -mt-10 -mr-10 w-32 h-32 bg-primary rounded-full blur-[80px] opacity-40" />
                                    <div className="relative z-10">
                                        ...
                                        <ul className="space-y-4">
                                            {suggestions.map((tip, idx) => (
                                                ...
                                                >
                                                    <div className="mt-1 p-0.5 bg-primary/20 rounded-full border border-primary/30 group-hover:bg-primary transition-colors">
                                                        <CheckCircle2 className="w-3.5 h-3.5" />
                                                    </div>
                                                    <span className="text-sm text-primary/80 leading-snug font-medium group-hover:text-white transition-colors">{tip}</span>
                                                </motion.li>
                                            ))}
                                        </ul>
