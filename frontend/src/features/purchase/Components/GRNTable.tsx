    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                        {records.length === 0 ? (
                            ...
                            records.map(grn => (
                                <tr key={grn.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-medium text-slate-900 dark:text-white">{grn.vendorName}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-secondary">
                                        {formatDate(grn.receivedDate)}
                                    </td>
                                    <td className="p-4 text-slate-500">{getBranchName(grn.branchId)}</td>
                                    <td className="p-4 text-center">
                                        <span className={`font-bold ${grn.receivedItems < grn.expectedItems ? 'text-warning' : 'text-success'}`}>
                                            {grn.receivedItems}
                                        </span>
                                        <span className="text-slate-400"> / {grn.expectedItems}</span>
                                    </td>
                                    <td className="p-4 text-center">
            ...
            <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700">
                {records.length === 0 ? (
                    <div className="p-8 text-center text-slate-500">No GRN records</div>
                ) : (
                    records.map(grn => (
                        <div key={grn.id} className="p-4">
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    ...
                                    <p className="font-bold text-slate-900 dark:text-white">{grn.vendorName}</p>
                            ...
                            <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-700/50 p-3 rounded-lg mt-2">
                                <div className="text-sm">
                                    ...
                                    <span className="text-slate-400"> / {grn.expectedItems} items</span>
                                </div>
                                <p className="text-xs text-slate-500">{formatDate(grn.receivedDate)}</p>
                            </div>
                        </div>
                    ))
                )}
