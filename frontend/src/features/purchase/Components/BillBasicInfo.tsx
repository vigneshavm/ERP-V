...
const BillBasicInfo: React.FC<BillBasicInfoProps> = ({ bill, vendors, onVendorChange, onBillChange }) => {
    return (
        <div className="bg-white dark:bg-slate-900 p-6 rounded-sm border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <FileText className="w-4 h-4" /> Basic Information
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block text-xs font-bold text-slate-500 mb-2">Vendor / Supplier</label>
                    <select
                        value={bill.vendor_id || ''}
                        onChange={(e) => onVendorChange(e.target.value)}
                        className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:ring-2 focus:ring-brand-500/20"
                    >
                ...
                <div>
                    <label className="block text-xs font-bold text-slate-500 mb-2">Bill Number</label>
                    <input
                        type="text"
                ...
                <div>
                    <label className="block text-xs font-bold text-slate-500 mb-2">Bill Date</label>
                    <input
                        type="date"
