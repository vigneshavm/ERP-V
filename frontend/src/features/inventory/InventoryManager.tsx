...
const MetricCard = ({ title, value, subtext, icon: Icon, color, trend }: any) => (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group hover:border-primary/50 transition-all">
        ...
        <div className="flex justify-between items-start relative z-10">
            <div>
                ...
                <h3 className="text-2xl font-black text-slate-900 dark:text-white mt-1 italic tracking-tight">{value}</h3>
                {subtext && (
                    <p className={`text-[10px] font-bold mt-2 flex items-center gap-1 ${trend === 'up' ? 'text-success' : trend === 'down' ? 'text-danger' : 'text-slate-400'}`}>
                        {subtext}
                    </p>
                )}
            </div>
            <div className={`p-3 rounded-xl bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400 group-hover:text-primary transition-colors`}>
    ...
    return (
        <Layout>
            <div className="space-y-6 pt-8 text-main pb-16">
                ...
                {/* View Toggle Tabs */}
                <div className="flex border-b border-slate-200 dark:border-slate-700 gap-6 pt-2">
                    <button
                        onClick={() => setActiveView('TABLE')}
                        className={`pb-3 text-xs font-black uppercase tracking-wider transition-all border-b-2 ${activeView === 'TABLE' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}
                    >
                        All Inventory SKUs
                    </button>
                    <button
                        onClick={() => setActiveView('VARIANT_SEARCH')}
                        className={`pb-3 text-xs font-black uppercase tracking-wider transition-all border-b-2 ${activeView === 'VARIANT_SEARCH' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}
                    >
                ...
                {/* Controls Bar */}
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 bg-white dark:bg-slate-800 p-6 rounded-sm border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto items-stretch md:items-center">
                        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl shrink-0">
                            <button
                                onClick={() => { setStockFilter('ALL'); setPage(1); }}
                                className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${stockFilter === 'ALL' ? 'bg-white dark:bg-slate-800 text-primary shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                All Items
                            </button>
                            <button
                                onClick={() => { setStockFilter('LOW_STOCK'); setPage(1); }}
                                className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1.5 whitespace-nowrap ${stockFilter === 'LOW_STOCK' ? 'bg-white dark:bg-slate-800 text-danger shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                        ...
                        <div className="relative flex-1 xl:w-[450px]">
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-bold text-main placeholder:text-slate-400"
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                            />
                        </div>
                        <select
                            className="px-6 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] outline-none hover:border-primary transition-all text-main cursor-pointer"
                    ...
                    <div className="flex gap-3 w-full md:w-auto justify-end">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="px-6 py-3 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-main rounded-xl text-xs font-black uppercase tracking-[0.15em] transition-all shadow-sm flex items-center gap-2"
                            >
                                More Actions <MoreVertical className="w-4 h-4" />
                            </button>

                            {showMoreActions && (
                                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm shadow-2xl z-[50] py-2 animate-in fade-in slide-in-from-top-2 duration-200">
                                    <div className="px-4 py-2 border-b border-slate-100 dark:border-slate-700 mb-1">
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Bulk Operations</p>
                                    </div>
                                    <button
                                        disabled={selectedItems.size === 0}
                                        onClick={() => { setShowMoreActions(false); /* handlePrintLabels(); */ }}
                                        className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 hover:bg-primary/5 hover:text-primary transition-all flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 hover:bg-success/5 hover:text-success transition-all flex items-center gap-3"
                                    >
                ...
                <div className="bg-white dark:bg-slate-800 rounded-sm border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm tabular-nums border-collapse">
                            <thead className="bg-slate-50/50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700 text-slate-500 font-black uppercase tracking-[0.25em] text-[10px]">
                                <tr>
                                    <th className="px-6 py-5 w-10">
                                        <button onClick={toggleSelectAll} className="text-slate-400 hover:text-primary transition-all">
                                ...
                                {isLoading ? (
                                    <tr>
                                        <td colSpan={7} className="p-20 text-center">
                                            ...
                                            <p className="mt-4 text-xs font-black uppercase tracking-widest text-slate-400">Indexing Stock...</p>
                                        </td>
                                    </tr>
                                ) : items.length === 0 ? (
                                    <tr>
                                    ...
                                    items.map((item: any) => {
                                        ...
                                        return (
                                            <tr key={item._id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all group ${isSelected ? 'bg-primary/5 dark:bg-primary/10' : ''}`}>
                                                <td className="px-6 py-5">
                                                    <button onClick={() => toggleSelect(item._id)} className="text-slate-400 hover:text-primary transition-all">
                                                        {isSelected ?
                                                            <CheckSquare className="w-5 h-5 text-primary" /> :
                                                            <Square className="w-5 h-5" />
                                                        }
                                                ...
                                                <td className="px-6 py-5">
                                                    <div className="flex items-start gap-4">
                                                        <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 flex items-center justify-center shrink-0 overflow-hidden">
                                                            {item.image ? <img src={item.image} alt="" className="w-full h-full object-cover" /> : <ImageIcon className="w-5 h-5 text-slate-400" />}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <div
                                                                className="font-bold text-slate-900 dark:text-slate-100 uppercase tracking-tight truncate max-w-[200px] group-hover:text-primary transition-colors cursor-pointer"
                                                                onClick={() => handleEditProduct(item)}
                                                            >
                                                                {item.name}
                                                            </div>
                                                            <div className="flex items-center gap-2 mt-1">
                                                                <span className="text-[10px] text-slate-500 font-bold">SKU: {item.sku || 'N/A'}</span>
                                                                {item.barcode && <div className="flex items-center gap-1"><Barcode className="w-3 h-3 text-slate-400" /><span className="text-[10px] text-slate-500 font-bold">{item.barcode}</span></div>}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <div className="flex flex-col gap-1">
                                                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 rounded text-[9px] font-black uppercase tracking-widest w-fit border border-slate-200 dark:border-slate-600">{item.category}</span>
                                                        <span className="text-[10px] text-primary font-bold italic">{item.brand || 'No Brand'}</span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <div className="flex flex-col">
                                                        <div className={`text-sm font-black italic ${isLowStock ? 'text-danger' : 'text-slate-900 dark:text-slate-100'}`}>
                                                            {item.stockQty} {item.unit}
                                                        </div>
                                                        <div className="flex items-center gap-1.5 mt-1">
                                                            <Warehouse className="w-3 h-3 text-slate-400" />
                                                            <span className="text-[10px] text-slate-400 font-bold uppercase">{item.location || 'General Floor'}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-5 text-right">
                                                    <div className="font-bold text-slate-400 text-[10px] uppercase tracking-wider line-through decoration-rose-500/30">₹{item.costPrice?.toLocaleString()}</div>
                                                ...
                                                <td className="px-6 py-5 text-right relative">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <button
                                                            ...
                                                            className={`p-2.5 rounded-xl transition-all shadow-sm ${rowActionDropdown === item._id ? 'bg-primary text-white' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-primary'}`}
                                                            title="More Actions"
                                                        >
                                                        ...
                                                        {rowActionDropdown === item._id && (
                                                            <div className="absolute right-6 top-16 w-56 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm shadow-2xl z-[50] py-2 animate-in fade-in slide-in-from-top-2 duration-200 text-left">
                                                                <button
                                                                    onClick={() => { handleEditProduct(item); setRowActionDropdown(null); }}
                                                                    className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 hover:bg-primary/5 hover:text-primary transition-all flex items-center gap-3"
                                                                >
                        ...
                        {/* Pagination */}
                        <div className="px-8 py-6 border-t border-slate-200 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-800/30 flex flex-col md:flex-row justify-between items-center gap-6">
                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Showing {items.length} of {pagination?.total || items.length} Registered Products</div>
                            <div className="flex items-center gap-4">
                                <button
                                    onClick={() => setPage(p => Math.max(1, p - 1))}
                                    disabled={page === 1}
                                    className="p-2.5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-400 hover:text-primary hover:border-primary transition-all shadow-sm rounded-xl disabled:opacity-50"
                                >
                                    <ChevronLeft className="w-5 h-5" />
                                </button>
                                <span className="text-[11px] font-black uppercase tracking-[0.2em] px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-inner text-main">
            ...
            {showAgingModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white dark:bg-slate-800 w-full max-w-5xl rounded-sm border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="px-10 py-8 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-700/50">
                            <div>
                                <h3 className="text-2xl font-black italic flex items-center gap-3 text-slate-900 dark:text-slate-100">
                                    <AlertOctagon className="w-8 h-8 text-danger" /> Dead Stock Analysis
                                </h3>
                                <p className="text-[10px] font-black text-slate-400 mt-1 uppercase tracking-[0.2em]">Identified items with stock age {'>'} 180 days</p>
                            </div>
                            <button onClick={() => setShowAgingModal(false)} className="p-3 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-danger rounded-full transition-all"><X className="w-6 h-6" /></button>
                        </div>
                        <div className="flex-1 overflow-y-auto">
                            {isLoading ? (
                                <div className="p-32 text-center">
                                    <div className="animate-spin rounded-full h-12 w-12 border-4 border-primary border-t-transparent mx-auto"></div>
                                    <p className="mt-4 font-black text-slate-400 uppercase tracking-[0.25em] text-xs">Running aging algorithm...</p>
                                </div>
                                ...
                                <div className="p-32 text-center flex flex-col items-center">
                                    ...
                                    <h4 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-2 italic tracking-tight">Inventory Healthy</h4>
                                    <p className="font-bold text-slate-500 text-sm italic">No dead stock detected. Outstanding maintenance!</p>
                                </div>
                            ) : (
                                <table className="w-full text-left text-sm border-collapse">
                                    <thead className="bg-slate-50/50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700 text-slate-500 font-black uppercase tracking-[0.25em] text-[10px] sticky top-0 z-10">
                                        <tr>
                                    ...
                                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                        {agingReport.map((item: any) => (
                                            <tr key={item._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-all group">
                                                <td className="px-8 py-6">
                                                    <div className="font-bold text-slate-900 dark:text-slate-100 text-base uppercase tracking-tight group-hover:text-primary transition-colors">{item.name}</div>
                                                    <div className="text-[10px] text-slate-400 font-black uppercase mt-1 tracking-widest">SKU: {item.sku}</div>
                                                </td>
                                                <td className="px-8 py-6">
                                                ...
                                                <td className="px-8 py-6 text-right">
                                                    <div className="font-black text-slate-900 dark:text-slate-100 text-lg italic">₹{item.valuation?.toLocaleString()}</div>
                                                    <div className="text-[10px] text-slate-400 font-black uppercase mt-1 tracking-widest">Qty: {item.stockQty}</div>
                                                </td>
                                                <td className="px-8 py-6 text-right">
                                                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                                        <button
                                                            onClick={() => handleAgingAction(item._id, 'CLEARANCE', item.sellingPrice)}
                                                            className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 hover:text-slate-900 dark:hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-slate-200 dark:border-slate-600 flex items-center gap-2"
                                                        >
                        ...
                        <div className="px-10 py-6 border-t border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-700/50 flex justify-end">
                            <button onClick={() => setShowAgingModal(false)} className="px-8 py-4 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl font-black uppercase tracking-widest text-[10px] hover:scale-105 active:scale-95 transition-all shadow-xl">
                                Close Panel
                            </button>
                        </div>
                    </div>
