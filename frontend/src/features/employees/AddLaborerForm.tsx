...
const AddLaborerForm: React.FC<AddLaborerFormProps> = ({
    ...
    return (
        <Card className="p-3 bg-primary-soft dark:bg-slate-800 border-primary/30 dark:border-slate-700 shadow-md relative z-10">
            <h4 className="font-bold text-primary dark:text-primary mb-2 text-xs">{title}</h4>
            <form onSubmit={onSubmit} className="space-y-2">
                <input
                    ...
                    className="w-full p-2 text-xs rounded border border-primary/30 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-300"
                    autoFocus
                />
                <input
                    type="text"
                ...
                <div className="flex bg-white dark:bg-slate-900 rounded border border-primary/30 dark:border-slate-600 overflow-hidden">
                    <button
                        type="button"
                        onClick={() => setWageType('DAILY')}
                        className={`flex-1 py-1.5 text-[10px] font-bold transition-colors ${wageType === 'DAILY' ? 'bg-primary text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                    >
                        Daily Rate
                    </button>
                    <div className="w-px bg-primary-soft dark:bg-slate-600"></div>
                    <button
                        type="button"
                        onClick={() => setWageType('MONTHLY')}
                        className={`flex-1 py-1.5 text-[10px] font-bold transition-colors ${wageType === 'MONTHLY' ? 'bg-primary text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                    >
                        Monthly Salary
                    </button>
                </div>
                ...
                {wageType === 'DAILY' ? (
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-6 p-2 text-xs rounded border border-primary/30 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-300"
                        />
                    </div>
                ) : (
                    <div className="space-y-1">
                        ...
                        {newEmp.dailyRate && (
                            <div className="flex items-center gap-1.5 text-[10px] text-primary dark:text-primary bg-primary/50 dark:bg-primary-soft p-1.5 rounded">
                                <Calculator size={10} />
                ...
                <div className="flex gap-2 mt-1">
                    <button type="submit" className="flex-1 bg-primary text-white py-1 rounded text-xs font-medium hover:bg-primary-hover">{submitLabel}</button>
