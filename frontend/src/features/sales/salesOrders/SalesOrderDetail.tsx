...
const SalesOrderDetail = () => {
    ...
    const getStatusConfig = (status: string | undefined): StatusConfig => {
        const configs: Record<string, StatusConfig> = {
            'Draft': { color: 'text-slate-500', glow: 'bg-slate-500/10', icon: ClipboardList, text: 'Draft Mode' },
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="relative">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Protocol Active // Log Date: {formatDate(order.orderDate)}</p>
                            </div>
                        </div>
                    </div>

                    <div className="flex items-center gap-3">
                        <button className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:text-warning transition-colors shadow-sm group">
                            <Printer className="w-4 h-4" onClick={() => window.print()} />
                        </button>
                        <button className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:text-warning transition-colors shadow-sm">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="p-8 grid grid-cols-2 gap-6">
                                    {[
                                        ...
                                        <div key={i} className="space-y-1">
                                            ...
                                            <p className={`text-xs font-black uppercase tracking-tight ${item.urgent ? 'text-danger' : 'text-slate-900 dark:text-white'}`}>{item.value}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[40px] overflow-hidden shadow-sm">
                                ...
                                <div className="p-8 flex items-center gap-6">
                                    ...
                                    <div>
                                        <p className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">{order.customer?.name}</p>
                                        <div className="space-y-1 mt-2">
                                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2"><Phone className="w-3 h-3" /> {order.customer?.phone}</p>
                                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2"><Mail className="w-3 h-3" /> {order.customer?.email}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[40px] overflow-hidden shadow-sm">
                            <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 flex items-center justify-between">
                                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Manifest Registry</h3>
                                <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest">{order.items.length} Nodes</span>
                            </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50/50 dark:bg-slate-950/50">
                                            ...
                                            <th className="px-8 py-4 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-200 dark:border-slate-800 text-right">Net Valuation</th>
                                        </tr>
                                    </thead>
                                                ...
                                                <tr key={index} className="group hover:bg-warning/90/[0.01] transition-colors">
                                                    ...
                                                    <td className="px-8 py-6">
                                                        <div className="flex flex-col">
                                                            <span className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">{item.item?.name || item.name}</span>
                                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Rate: ₹{item.rate} // Tax: {item.tax}%</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-8 py-6 text-right">
                                                        <span className="text-sm font-mono font-black text-slate-900 dark:text-white">{item.quantity}</span>
                                                    </td>
                                                    <td className="px-8 py-6 text-right w-40">
                                                        <div className="flex flex-col items-end gap-1.5">
                                                            ...
                                                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{item.deliveredQty || 0} / {item.quantity} DISPATCHED</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-8 py-6 text-right">
                    ...
                    <div className="space-y-8 sticky top-8">
                        ...
                        <div className="bg-slate-900 dark:bg-white rounded-[40px] p-8 text-white dark:text-slate-900 shadow-2xl relative overflow-hidden">
                            ...
                            <div className="space-y-4 relative z-10">
                                {[
                                    { label: 'Sub-Valuation', value: order.subtotal, color: 'text-white dark:text-slate-900' },
                        ...
                        {((order.deliveryChallans && order.deliveryChallans.length > 0) || (order.invoices && order.invoices.length > 0)) && (
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[40px] overflow-hidden shadow-sm">
                                <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50">
                                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Linked Manifests</h3>
                                </div>
                                <div className="p-8 space-y-6">
                                    {order.deliveryChallans && order.deliveryChallans.length > 0 && (
                                        <div className="space-y-3">
                                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Dispatch Nodes</p>
                                            {order.deliveryChallans.map((dc: any, index: number) => (
                                                <div key={index} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-950 rounded-sm border border-slate-100 dark:border-slate-800 group cursor-pointer hover:border-warning/30 transition-all">
                                                    <div className="flex items-center gap-3">
                                    ...
                                    {order.invoices && order.invoices.length > 0 && (
                                        <div className="space-y-3">
                                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Fiscal Nodes</p>
                                            {order.invoices.map((inv: any, index: number) => (
                                                <div key={index} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-950 rounded-sm border border-slate-100 dark:border-slate-800 group cursor-pointer hover:border-success/30 transition-all">
                                                    <div className="flex items-center gap-3">
