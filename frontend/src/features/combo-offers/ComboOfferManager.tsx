            ...
            <div className="space-y-6 animate-fade-in text-slate-900 dark:text-slate-100 pb-12">
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {isLoading && combos.length === 0 && (
                                    <tr><td colSpan={6} className="p-8 text-center text-sm text-slate-400">Loading combo offers...</td></tr>
                                )}
                                {!isLoading && filteredCombos.length === 0 && (
                                    <tr><td colSpan={6} className="p-8 text-center text-sm text-slate-400">No combo offers yet.</td></tr>
                                )}
                                {filteredCombos.map((combo: ComboOffer) => (
                                    <tr key={combo._id} className="group hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                                        <td className="p-6">
                                        ...
                                        <td className="p-6 text-right"><p className="text-sm font-bold text-slate-400 line-through">₹{combo.regularPrice.toLocaleString()}</p></td>
                                        ...
                                        <td className="p-6 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => startEdit(combo)} className="p-2 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-all">
                                                    <Edit3 className="w-4 h-4" />
                                                </button>
                                                <button onClick={() => handleDelete(combo._id)} className="p-2 text-slate-400 hover:text-error hover:bg-error/5 rounded-lg transition-all">
                ...
                {isModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                            ...
                            <form onSubmit={handleSave} className="p-8 space-y-5 overflow-y-auto">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Combo Name</label>
                                        <input
                                            required
                                    ...
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Combo Code (optional)</label>
                                        <input
                                            className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-sm text-sm font-bold focus:ring-2 focus:ring-primary/20"
                                ...
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Bundle Items</label>
                                    <div className="space-y-2">
                                        {draft.lines.map((line, index) => (
                                            <div key={index} className="flex gap-2">
                                                <select
                                                    required
                                                    className="flex-1 px-4 py-3 bg-slate-50 dark:bg-slate-900 border-none rounded-sm text-sm font-bold focus:ring-2 focus:ring-primary/20"
                                                    value={line.itemId}
                                                ...
                                                <input
                                                    ...
                                                    min={1}
                                                    required
                                                    className="w-20 px-3 py-3 bg-slate-50 dark:bg-slate-900 border-none rounded-sm text-sm font-bold text-center focus:ring-2 focus:ring-primary/20 tabular-nums"
                                                    value={line.quantity}
                                                ...
                                                <button
                                                    ...
                                                    className="px-3 text-slate-400 hover:text-error disabled:opacity-30 transition-colors"
                                                >
                                                    <X className="w-4 h-4" />
                                                </button>
                                            </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 items-end">
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Offer (Bundle) Price ₹</label>
                                        <input
                                            type="number"
                                            required
                                            min={0}
                                    ...
                                    <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-sm">
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Regular Price</p>
