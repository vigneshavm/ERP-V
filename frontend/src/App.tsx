...
// Moved to module scope - these were defined inside App's render body, which recreated (and
// reset the DOM/animation state of) each one on every render. They previously read App's state
// directly from closure; they now take what they need as props instead.
...
const LandingPage = ({ setViewMode, setCurrentTenant, setIsLoggedIn }: {
...
const AdminView = ({ isAdminAuthenticated, setIsAdminAuthenticated, setViewMode, setCurrentTenant, setIsLoggedIn }: {
...
const App: React.FC = () => {
  ...
  // NOTE: this used to `return <POSCustomerDisplay />` right here, before the hooks below were
  // declared. That violates the Rules of Hooks (hooks must run in the same order on every
  // render) and could crash with "Rendered more hooks than during the previous render" when
  // navigating between customer-display mode and the normal app. The condition is now only
  // acted on right before the final render, after every hook has run unconditionally.
  const urlParams = new URLSearchParams(window.location.search);
  const mode = urlParams.get('mode');
  const isCustomerDisplayMode = mode === 'customer_display';
  ...
  // getProfile is a plain Redux thunk (not react-query), so it has no
  // built-in request dedup: under React.StrictMode (enabled in main.tsx)
  // this effect's body runs twice on mount, and without a guard that fired
  // GET /api/auth/profile twice for the same token every time. Track the
  // token this effect already fetched for, so the StrictMode re-run for the
  // *same* token is a no-op, while a real token change (login/reload) still
  // triggers a fresh fetch as before.
  const fetchedProfileForToken = useRef<string | undefined>(undefined);
  React.useEffect(() => {
    if (user && user.token && fetchedProfileForToken.current !== user.token) {
      fetchedProfileForToken.current = user.token;
  ...
  const [currentTenant, setCurrentTenant] = useState<Tenant | null>(() => {
    const _storedTenantId = localStorage.getItem('erp_current_tenant');
