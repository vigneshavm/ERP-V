...
import { Filter, Search, Plus, Download, ChevronRight, Users, ShieldCheck, IndianRupee } from 'lucide-react';
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-4">
                        <button className="h-12 px-6 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 text-neutral-900 dark:text-white font-bold text-xs tracking-widest rounded-sm hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-all shadow-sm flex items-center gap-3 uppercase">
                            <Download className="w-4 h-4 text-primary" /> Export Schema
                        </button>
                        <button className="h-12 px-8 bg-primary text-white font-black uppercase tracking-widest text-xs rounded-sm transition-all shadow-lg shadow-primary/20 flex items-center gap-3 hover:opacity-90">
                            <Plus className="w-4 h-4" /> Define Structure
                        </button>
                    </div>
                </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-neutral-50 dark:bg-neutral-950/80 sticky top-0 z-20 backdrop-blur-md border-b border-neutral-200 dark:border-neutral-800">
                                <tr className="text-neutral-500 dark:text-neutral-400 text-[10px] font-black uppercase tracking-[0.2em]">
                                    ...
                                    <th className="px-8 py-5">Functional Role</th>
                                    <th className="px-8 py-5">Department Node</th>
                                    <th className="px-8 py-5 text-center">Payroll Status</th>
                                    <th className="px-8 py-5 text-right">Base Valuation</th>
                                    ...
                                    <tr key={i} className="hover:bg-primary/[0.02] transition-all group cursor-pointer" onClick={() => navigate(`/people/employees/labor/view/${rec.id}`)}>
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <div className="text-sm font-black text-neutral-900 dark:text-white">{rec.full_name}</div>
                                                    <div className="text-[9px] font-black text-neutral-400 uppercase tracking-widest mt-1">ID: {rec.id.split('-').pop()}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="text-xs font-black uppercase tracking-tight text-neutral-700 dark:text-neutral-300">{rec.role}</div>
                                        </td>
                                        <td className="px-8 py-6">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex justify-center">
                                                <span className="px-4 py-1.5 rounded-sm bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 text-[9px] font-black uppercase tracking-widest flex items-center gap-2">
                                                    <ShieldCheck className="w-3 h-3" /> VERIFIED
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <div className="flex items-center justify-end gap-2 text-sm font-black text-neutral-900 dark:text-white tabular-nums tracking-tighter">
                                                <IndianRupee className="w-3.5 h-3.5 text-primary" />
                                                {rec.salary.toLocaleString()}
                                            </div>
                                            <div className="text-[9px] text-neutral-400 font-black uppercase tracking-widest mt-1">CTC Per Annum</div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="flex justify-center">
