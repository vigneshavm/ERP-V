...
const GoodsReceived: React.FC = () => {
    ...
    const filteredRecords = useMemo(() => {
        return grnRecords.filter((grn: any) => {
            if (searchTerm) {
    ...
    // Summary stats
    const completeCount = filteredRecords.filter((g: any) => g.status === 'COMPLETE').length;
    const partialCount = filteredRecords.filter((g: any) => g.status === 'PARTIAL').length;
    const pendingCount = filteredRecords.filter((g: any) => g.status === 'PENDING').length;
    const totalExpected = filteredRecords.reduce((acc: number, g: any) => acc + g.expectedItems, 0);
    const totalReceived = filteredRecords.reduce((acc: number, g: any) => acc + g.receivedItems, 0);
                                        ...
                                        <tr key={grn.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-700/50">
                                            ...
                                            <td className="p-4 text-center">
                                                <div className="flex justify-center gap-1">
                                                    <button
                                                        onClick={() => navigate(`/purchase/grn/view/${grn.id}`)}
                                                    ...
                                                    <button
                                                        onClick={() => navigate(`/purchase/bills/new/${grn.id}`)}
                                                        className="p-2 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded-lg"
