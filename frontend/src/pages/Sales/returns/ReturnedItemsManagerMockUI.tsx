...
import { Filter, Search, Plus, Download, ChevronRight, BarChart2, RotateCcw, TrendingUp, TrendingDown, Clock, ShieldCheck, ClipboardCheck, AlertCircle, RefreshCw } from 'lucide-react';
import returnedItemsManagerData from '../../../mockData/returnedItemsManagerData.json';
    ...
    return (
        <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-white font-sans selection:bg-amber-500/30 overflow-hidden flex flex-col transition-colors animate-fade-in relative">
            ...
            <main className="relative z-10 flex-1 flex flex-col max-w-[1600px] w-full mx-auto px-8 py-8 space-y-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="relative">
                        ...
                        <h1 className="text-3xl font-display font-black tracking-tighter text-neutral-900 dark:text-white flex items-center gap-3">
                            Returns <span className="text-amber-500">Intelligence</span>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <p className="text-[10px] uppercase tracking-[0.2em] font-black text-neutral-500 dark:text-neutral-400">QC Pipeline Active // Protocol R-7</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 text-neutral-900 dark:text-white rounded-xl hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-all text-xs font-bold shadow-sm">
                            <Download className="w-4 h-4 text-amber-500" /> Export RMA
                        </button>
                ...
                {/* Neural Metrics Grid - Updated to Requested Specifications */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {returnedItemsManagerData.metrics.map((stat, i) => (
                        <div key={i} className="bg-white dark:bg-neutral-900 rounded-2xl p-6 border border-neutral-200 dark:border-neutral-800 relative overflow-hidden group hover:border-amber-500/50 transition-all cursor-pointer shadow-sm">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className={`p-2.5 rounded-xl ${
                                    ...
                                    {stat.label.includes('Amount') ? <IndianRupee className="w-4 h-4" /> : 
                                     stat.label.includes('Full') ? <ShieldCheck className="w-4 h-4" /> : 
                                     stat.label.includes('Partial') ? <RefreshCw className="w-4 h-4" /> :
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center justify-between mt-2">
                                    ...
                                    {stat.label.includes('Returns') && i > 1 && (
                            ...
                            <div className="mt-5 h-1.5 w-full bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
                                <div 
                                    ...
                                    style={{ width: stat.trend.includes('%') ? stat.trend : '70%' }}
                                ></div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="flex-1 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-[32px] overflow-hidden flex flex-col shadow-sm">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-neutral-50 dark:bg-neutral-950/50 border-b border-neutral-200 dark:border-neutral-800 sticky top-0 z-20">
                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400">Return Node</th>
                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400">Inflow Date</th>
                                    ...
                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400">Reason Node</th>
                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400">QC Status</th>
                                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400 text-right">Credit Value</th>
                                    <th className="px-8 py-5 w-16"></th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                                {returnedItemsManagerData.records.map((rec, i) => (
                                    <tr key={i} className="hover:bg-amber-500/[0.02] transition-all cursor-pointer group">
                                        ...
                                        <td className="px-8 py-6">
                                            ...
                                            <div className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mt-1">Verified Logistics</div>
                                        </td>
                                        <td className="px-8 py-6">
                                            ...
                                            <div className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mt-1">Tier-1 Returnee</div>
                                        </td>
                                        <td className="px-8 py-6">
                                        ...
                                        <td className="px-8 py-6">
                                            <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border flex items-center gap-2 w-fit ${
                                                rec.status === 'QC Passed' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' :
                                                rec.status === 'QC Pending' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                                                'bg-rose-500/10 text-rose-500 border-rose-500/20'
                                            }`}>
                                                <div className={`w-1.5 h-1.5 rounded-full ${
                                                    rec.status === 'QC Passed' ? 'bg-emerald-500 shadow-[0_0_5px_currentColor]' :
                                                    rec.status === 'QC Pending' ? 'bg-amber-500 animate-pulse' :
                                                    'bg-rose-500'
                                                }`} />
                                                {rec.status}
                                            </span>
                                            <div className="text-[9px] text-neutral-500 font-black uppercase tracking-widest mt-1 ml-1">{rec.action}</div>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <div className="text-lg font-mono font-black text-neutral-900 dark:text-white tracking-tighter">{rec.amount}</div>
                                            <div className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mt-0.5">Automated Adjust</div>
                                        </td>
                    ...
                    <div className="p-8 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50 flex flex-col sm:flex-row justify-between items-center gap-6 mt-auto">
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-neutral-500">
                            Processing <span className="text-amber-500 font-black">06</span> of <span className="text-neutral-900 dark:text-white">24</span> active Return Nodes
                        </p>
