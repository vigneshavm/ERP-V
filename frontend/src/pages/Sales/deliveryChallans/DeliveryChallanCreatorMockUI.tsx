...
import { Save, X, Plus, FileText, ArrowLeft, Trash2, Zap, Truck } from 'lucide-react';
import { inventory, customers, MockProduct } from '../../../data';
import Layout from '../../../components/shared/Layout';

const DeliveryChallanCreatorMockUI: React.FC = () => {
    const navigate = useNavigate();
    
    // Use sample items from real inventory
    const lineItems = useMemo(() => {
        return (inventory as MockProduct[]).slice(1, 4).map(item => ({
            id: item.id,
            name: item.name,
            qty: 15,
            rate: item.selling_price,
            total: 15 * item.selling_price
        }));
    }, []);
                ...
                <div className="flex justify-between items-center bg-white dark:bg-neutral-900 p-6 rounded-sm border border-neutral-200 dark:border-neutral-800 shadow-sm">
                    ...
                    <div className="flex gap-4">
                        ...
                        <button className="flex items-center gap-2 px-8 py-3 bg-amber-500 text-white border border-amber-600 rounded-sm hover:bg-amber-600 shadow-lg shadow-amber-500/20 transition-all text-xs font-black uppercase tracking-widest">
                            <Truck className="w-4 h-4" /> Dispatch Record
                        </button>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <div className="bg-white dark:bg-neutral-900 p-8 rounded-sm space-y-6 border border-neutral-200 dark:border-neutral-800 shadow-sm">
                            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400 flex items-center gap-3">
                                <FileText className="w-4 h-4 text-warning" /> Consignment Header
                            </h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[9px] font-black text-neutral-500 uppercase tracking-widest">Recipient Customer</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[9px] font-black text-neutral-500 uppercase tracking-widest">Dispatch Timestamp</label>
                                    <input type="datetime-local" className="w-full bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-sm p-4 text-xs font-bold focus:border-amber-500/50 outline-none transition-all text-neutral-900 dark:text-white shadow-inner" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white dark:bg-neutral-900 p-8 rounded-sm space-y-6 border border-neutral-200 dark:border-neutral-800 shadow-sm">
                            <div className="flex justify-between items-center">
                                <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400">Consignment Line Items</h2>
                                ...
                                <table className="w-full text-left">
                                    <thead className="bg-neutral-100 dark:bg-neutral-900 text-neutral-500 dark:text-neutral-400 border-b border-neutral-200 dark:border-neutral-800">
                                        <tr className="text-[9px] font-black uppercase tracking-widest">
                                            <th className="p-4">SKU / Item Entity</th>
                                            <th className="p-4 text-right w-24">Qty</th>
                                            <th className="p-4 text-right w-32">Valuation</th>
                                        ...
                                        {lineItems.map((item, idx) => (
                                            <tr key={idx} className="hover:bg-amber-500/[0.02] transition-colors group">
                                                <td className="p-4">
                                                    ...
                                                    <div className="text-[9px] text-neutral-500 font-bold uppercase mt-0.5">UUID: {item.id}</div>
                                                </td>
                    ...
                    <div className="space-y-8">
                        <div className="bg-white dark:bg-neutral-900 p-8 rounded-sm border border-neutral-200 dark:border-neutral-800 shadow-sm relative overflow-hidden">
                            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400 mb-8 border-b border-neutral-100 dark:border-neutral-800 pb-4 flex items-center gap-2">
                                <Zap className="w-4 h-4 text-warning" /> Consignment Valuation
                            </h2>
                            <div className="space-y-4 text-sm relative z-10">
                                <div className="flex justify-between text-neutral-500 dark:text-neutral-400 font-bold">
                                    <span className="text-[10px] uppercase tracking-widest">Base Item Value</span>
                                    <span className="font-mono">₹{summary.subtotal.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between text-neutral-500 dark:text-neutral-400 font-bold">
                                    <span className="text-[10px] uppercase tracking-widest text-success/70">Calculated GST</span>
                                ...
                                <div className="pt-6 border-t border-neutral-200 dark:border-neutral-800 flex justify-between items-end mt-8">
                                    <div>
                                        <p className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.2em] mb-1">Total Challan Worth</p>
...
export default DeliveryChallanCreatorMockUI;
