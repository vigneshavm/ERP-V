...
import { Filter, Search, Plus, Download, ChevronRight, BarChart2, IndianRupee, TrendingUp, TrendingDown, Clock, ShieldCheck, CreditCard, Banknote, Landmark } from 'lucide-react';
import { payments, customers, salesInvoices, MockPayment, MockSalesInvoice, MockCustomer } from '../../../data';
import Layout from '../../../components/shared/Layout';

const PaymentInListMockUI: React.FC = () => {
    const navigate = useNavigate();

    // Derive data from real payments
    const paymentsList = useMemo(() => {
        return (payments as MockPayment[]).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    }, []);

    const metrics = useMemo(() => {
        const totalCollected = paymentsList.reduce((sum, p) => sum + p.amount, 0);
        const todayCollected = paymentsList
            .filter(p => new Date(p.date).toDateString() === new Date().toDateString())
            .reduce((sum, p) => sum + p.amount, 0);

        return [
            { label: "Total Collections", value: `₹${totalCollected.toLocaleString()}`, trend: "+8.4%", status: "success" },
            { label: "Avg Receipt Value", value: `₹${Math.round(totalCollected / (paymentsList.length || 1)).toLocaleString()}`, trend: "+2.1%", status: "info" },
            { label: "Today's Volume", value: `₹${todayCollected.toLocaleString()}`, trend: "+12%", status: "warning" },
            { label: "Active Channels", value: "4 Protocols", trend: "0%", status: "neutral" }
        ];
    }, [paymentsList]);

    const getCustomerData = (payment: MockPayment) => {
        const invoice = (salesInvoices as MockSalesInvoice[]).find(inv => inv.id === payment.ref_id || inv.invoice_no === payment.ref_id);
        if (invoice) {
            return { name: invoice.customer_name, id: invoice.customer_id };
        }
        return { name: "Retail Customer", id: "C000" };
    };
                ...
                <main className="relative z-10 flex-1 flex flex-col max-w-[1600px] w-full mx-auto px-8 py-8 space-y-8">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div className="relative">
                            ...
                            <h1 className="text-3xl font-display font-black tracking-tighter text-neutral-900 dark:text-white flex items-center gap-3">
                                Payment <span className="text-warning">Register</span>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <p className="text-[10px] uppercase tracking-[0.2em] font-black text-neutral-500 dark:text-neutral-400">Live Collection Stream // Protocol X-1</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 text-neutral-900 dark:text-white rounded-xl hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-all text-xs font-bold shadow-sm">
                                <Download className="w-4 h-4 text-warning" /> Export Sheet
                            </button>
                            <button 
                            ...
                            <div key={i} className="bg-white dark:bg-neutral-900 rounded-sm p-6 border border-neutral-200 dark:border-neutral-800 relative overflow-hidden group hover:border-amber-500/50 transition-all cursor-pointer shadow-sm">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className={`p-2.5 rounded-xl ${
                                        ...
                                        {stat.status === 'success' ? <ShieldCheck className="w-4 h-4" /> : 
                                         stat.status === 'warning' ? <Clock className="w-4 h-4" /> : 
                                ...
                                <div className="flex items-end justify-between">
                                    ...
                                    <div className={`flex items-center gap-1 text-[10px] font-bold ${
                                        ...
                                        stat.trend.startsWith('-') ? 'text-danger' : 'text-neutral-400'
                                    }`}>
                                ...
                                <div className="mt-5 h-1.5 w-full bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
                                    <div 
                                        className={`h-full rounded-full ${
                                            stat.status === 'success' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]' : 
                                            stat.status === 'warning' ? 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.4)]' : 
                                            'bg-neutral-500 shadow-[0_0_8px_rgba(115,115,115,0.4)]'
                                        }`}
                                        style={{ width: '75%' }}
                                    ></div>
                                </div>
                    ...
                    <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-sm p-4 flex flex-col md:flex-row gap-4 items-center shadow-sm">
                        <div className="relative flex-1 w-full">
                            ...
                            <input 
                                type="text" 
                                placeholder="SEARCH BY RECEIPT ID / CUSTOMER / REF..." 
                    ...
                    <div className="flex-1 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-[32px] overflow-hidden flex flex-col shadow-sm">
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-neutral-50 dark:bg-neutral-950/50 border-b border-neutral-200 dark:border-neutral-800 sticky top-0 z-20">
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400">Receipt Node</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400">Timestamp</th>
                                        ...
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400 text-center">Protocol</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 dark:text-neutral-400">Verification</th>
                                ...
                                <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                                    {paymentsList.map((p, i) => {
                                        const customer = getCustomerData(p);
                                        return (
                                            <tr 
                                                key={i} 
                                            ...
                                            >
                                                <td className="px-8 py-6">
                                                    <span className="font-mono text-sm font-bold text-warning group-hover:underline cursor-pointer tracking-tighter">#RCT-{p.id.split('-').pop()?.toUpperCase()}</span>
                                                </td>
                                                <td className="px-8 py-6">
                                                    <div className="text-sm font-bold text-neutral-900 dark:text-neutral-200">{p.date}</div>
                                                    <div className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mt-1">Confirmed Sync</div>
                                                </td>
                                                <td className="px-8 py-6">
                                                    <div className="text-sm font-black text-neutral-900 dark:text-white group-hover:translate-x-1 transition-transform inline-block">{customer.name}</div>
                                                    <div className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mt-1">Institutional Client</div>
                                                </td>
                                                <td className="px-8 py-6 text-center">
                                                    <div className="flex flex-col items-center">
                                                        <div className="p-1.5 bg-neutral-100 dark:bg-neutral-800 rounded-lg text-neutral-500 mb-1">
                                                            {p.mode === 'Bank Transfer' ? <Landmark className="w-3.5 h-3.5" /> : 
                                                             p.mode === 'UPI' ? <CreditCard className="w-3.5 h-3.5" /> : 
                                                             p.mode === 'CASH' ? <Banknote className="w-3.5 h-3.5" /> : 
                                                             <IndianRupee className="w-3.5 h-3.5" />}
                                                        </div>
                                                        <span className="text-[9px] font-black uppercase tracking-tighter text-neutral-400">{p.mode}</span>
                                                    </div>
                                                </td>
                                                <td className="px-8 py-6">
                                                    <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border flex items-center gap-2 w-fit bg-success/10 text-success border-success/20`}>
                                                        <div className={`w-1.5 h-1.5 rounded-full bg-emerald-500`} />
                                                        Verified
                                                    </span>
                                                </td>
                                                <td className="px-8 py-6 text-right">
                                                    <div className="text-lg font-mono font-black text-neutral-900 dark:text-white tracking-tighter">₹{p.amount.toLocaleString()}</div>
                                                    <div className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mt-0.5">Ref: {p.ref_id}</div>
                                                </td>
                                            <td className="px-8 py-6 text-center">
                        ...
                        <div className="p-8 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50 flex flex-col sm:flex-row justify-between items-center gap-6 mt-auto">
                            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-neutral-500">
                                Monitoring <span className="text-warning font-black">{paymentsList.length.toString().padStart(2, '0')}</span> of <span className="text-neutral-900 dark:text-white">{paymentsList.length}</span> Historical Assets
                            </p>
                            <div className="flex gap-3">
                                <button className="px-6 py-3 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-sm text-[10px] font-black uppercase tracking-widest text-neutral-400 disabled:opacity-30 transition-all hover:bg-neutral-50" disabled>Previous Page</button>
                                <button className="px-6 py-3 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-sm text-[10px] font-black uppercase tracking-widest text-warning hover:border-amber-500/50 transition-all shadow-sm">Next Page</button>
                            </div>
                        </div>
                    </div>
                </main>
...
export default PaymentInListMockUI;
