...
import paymentInListData from '../../../mockData/paymentInListData.json';
        ...
        <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-white font-sans selection:bg-amber-500/30 overflow-hidden flex flex-col transition-colors animate-fade-in relative">
            ...
            <main className="relative z-10 flex-1 flex flex-col max-w-[1600px] w-full mx-auto px-8 py-8 space-y-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="relative">
                        ...
                        <h1 className="text-3xl font-display font-black tracking-tighter text-neutral-900 dark:text-white flex items-center gap-3">
                            Payment <span className="text-amber-500">Register</span>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 text-neutral-900 dark:text-white rounded-xl hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-all text-xs font-bold shadow-sm">
                            <Download className="w-4 h-4 text-amber-500" /> Export Sheet
                        </button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {paymentInListData.metrics.map((stat, i) => (
                        <div key={i} className="bg-white dark:bg-neutral-900 rounded-2xl p-6 border border-neutral-200 dark:border-neutral-800 relative overflow-hidden group hover:border-amber-500/50 transition-all cursor-pointer shadow-sm">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className={`p-2.5 rounded-xl ${
                                    stat.status === 'success' ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 
                            ...
                            <div className="flex items-end justify-between">
                                ...
                                <div className={`flex items-center gap-1 text-[10px] font-bold ${
                                    ...
                                    stat.trend.startsWith('-') ? 'text-rose-500' : 'text-neutral-400'
                                }`}>
                ...
                <div className="flex-1 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-[32px] overflow-hidden flex flex-col shadow-sm">
                    <div className="overflow-x-auto custom-scrollbar">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                                {paymentInListData.records.map((rec, i) => (
                                    <tr key={i} className="hover:bg-amber-500/[0.02] transition-all cursor-pointer group">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="text-sm font-bold text-neutral-900 dark:text-neutral-200">{rec.date}</div>
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col items-center">
                                                <div className="p-1.5 bg-neutral-100 dark:bg-neutral-800 rounded-lg text-neutral-500 mb-1">
                                                    {rec.mode === 'Bank Transfer' ? <Landmark className="w-3.5 h-3.5" /> : 
                                                     rec.mode === 'UPI / G-Pay' ? <CreditCard className="w-3.5 h-3.5" /> : 
                                                     rec.mode === 'Cash' ? <Banknote className="w-3.5 h-3.5" /> : 
                                                     <IndianRupee className="w-3.5 h-3.5" />}
                                                </div>
                                                <span className="text-[9px] font-black uppercase tracking-tighter text-neutral-400">{rec.mode}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border flex items-center gap-2 w-fit ${
                                                rec.status === 'Verified' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' :
                                                rec.status === 'Pending' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                                                'bg-rose-500/10 text-rose-500 border-rose-500/20'
                                            }`}>
                                                <div className={`w-1.5 h-1.5 rounded-full ${
                                                    rec.status === 'Verified' ? 'bg-emerald-500' :
                                                    rec.status === 'Pending' ? 'bg-amber-500 animate-pulse' :
                                                    'bg-rose-500'
                                                }`} />
                                                {rec.status}
                                            </span>
                                        ...
                                        <td className="px-8 py-6 text-right">
                                            ...
                                            <div className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mt-0.5">Ref: {rec.reference}</div>
                                        </td>
                    ...
                    <div className="p-8 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50 flex flex-col sm:flex-row justify-between items-center gap-6 mt-auto">
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-neutral-500">
                            Monitoring <span className="text-amber-500 font-black">06</span> of <span className="text-neutral-900 dark:text-white">12,452</span> Historical Assets
                        </p>
                        <div className="flex gap-3">
                            <button className="px-6 py-3 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl text-[10px] font-black uppercase tracking-widest text-neutral-400 disabled:opacity-30 transition-all hover:bg-neutral-50" disabled>Previous Page</button>
                            <button className="px-6 py-3 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl text-[10px] font-black uppercase tracking-widest text-amber-500 hover:border-amber-500/50 transition-all shadow-sm">Next Page</button>
                        </div>
                    </div>
                </div>
            </main>
