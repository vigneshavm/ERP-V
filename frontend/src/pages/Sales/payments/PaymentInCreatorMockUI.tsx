...
import paymentInCreatorData from '../../../mockData/paymentInCreatorData.json';
                    ...
                    <div className="glass-panel p-6 rounded-xl space-y-4 border border-white/5 bg-white/[0.02]">
                        ...
                        <div className="bg-black/20 rounded-lg border border-white/5 overflow-hidden">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {paymentInCreatorData.lineItems.map((item, idx) => (
                    ...
                    <div className="glass-panel p-6 rounded-xl border border-white/5 bg-white/[0.02]">
                        ...
                        <div className="space-y-3 text-sm">
                            <div className="flex justify-between text-main/70">
                                <span>Subtotal</span>
                                <span>{paymentInCreatorData.summary.subtotal}</span>
                            </div>
                            <div className="flex justify-between text-main/70">
                                <span>{paymentInCreatorData.summary.taxLabel}</span>
                                <span>{paymentInCreatorData.summary.tax}</span>
                            </div>
                            <div className="pt-3 border-t border-white/10 flex justify-between font-bold text-lg mt-4">
                                <span>Total</span>
                                <span className="text-emerald-400">{paymentInCreatorData.summary.total}</span>
                            </div>
                        </div>
                    </div>
                </div>
