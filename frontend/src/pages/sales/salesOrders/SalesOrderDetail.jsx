...
const SalesOrderDetail = () => {
    ...
    const getLifecycleStages = () => {
        ...
        const currentIndex = stages.indexOf(order?.status?.replace('Partially ', '')) || 0;
    ...
    const hasItemsToDeliver = order?.items?.some(item => (item.reservedQty - item.deliveredQty) > 0);
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {order.items.map((item, index) => {
                                            const deliveryProgress = item.quantity > 0 ? (item.deliveredQty / item.quantity) * 100 : 0;
                                            return (
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <div className="flex flex-col items-center gap-1">
                                                            ...
                                                            <span className="text-[10px] font-bold text-slate-500">
                                                                {item.deliveredQty}/{item.quantity} delivered
                                                            </span>
                                                        </div>
                                                    </td>
