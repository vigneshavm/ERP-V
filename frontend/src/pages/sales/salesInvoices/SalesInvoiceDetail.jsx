                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                            <div>
                                ...
                                <p className="text-white/80 font-medium">
                                    {invoice.customer?.name || 'Walk-in Customer'} • {new Date(invoice.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                                </p>
                                {balanceDue > 0 && (
                                        ...
                                        <div className="flex flex-wrap gap-4 mt-2 text-sm text-slate-600">
                                            {invoice.customer.phone && (
                                                <span className="flex items-center gap-1.5">
                                                    <Phone className="w-4 h-4 text-slate-400" /> {invoice.customer.phone}
                                                </span>
                                            )}
                                            {invoice.customer.email && (
                                                <span className="flex items-center gap-1.5">
                                                    <Mail className="w-4 h-4 text-slate-400" /> {invoice.customer.email}
                                                </span>
                                            )}
                                        </div>
                                        {invoice.customer.address && (
                                            <p className="mt-2 text-sm text-slate-500 flex items-start gap-1.5">
                                                <MapPin className="w-4 h-4 text-slate-400 mt-0.5" /> {invoice.customer.address}
                                            </p>
                                        )}
                                    </div>
                                </div>
