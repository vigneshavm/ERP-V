...
const POSMockUI: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-[#000000] text-slate-200 font-sans selection:bg-cyan-500/30 flex overflow-hidden">
            {/* Ambient Background */}
            ...
            <div className="flex-1 flex flex-col relative z-10 border-r border-slate-800/60 bg-black/40 backdrop-blur-md">
                {/* Header */}
                <header className="h-20 px-8 flex items-center justify-between border-b border-slate-800/60">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-96 bg-slate-900/50 border border-slate-700/50 rounded-xl py-2.5 pl-12 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all text-slate-200 placeholder:text-slate-600"
                            />
                        </div>
                    </div>
                    <button className="h-10 px-4 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-xl flex items-center gap-2 transition-colors">
                ...
                {/* Categories */}
                <div className="px-8 py-4 flex gap-3 overflow-x-auto no-scrollbar border-b border-slate-800/60">
                    {['All Items', 'Hardware', 'Components', 'Enclosures', 'Networking', 'Peripherals'].map((cat, idx) => (
                        <button key={cat} className={`whitespace-nowrap px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${idx === 0 ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'bg-slate-900/50 text-slate-400 border border-slate-800 hover:bg-slate-800'}`}>
                            {cat}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex-1 p-8 overflow-y-auto">
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {PRODUCTS.map(product => (
                            <div key={product.id} className="group bg-slate-900/30 border border-slate-800/60 rounded-2xl overflow-hidden hover:border-cyan-500/50 hover:bg-slate-900/50 transition-all cursor-pointer">
                                <div className="h-40 overflow-hidden relative">
                                    ...
                                    <div className="absolute top-2 right-2 px-2 py-1 bg-black/60 backdrop-blur-md rounded border border-white/10 text-[10px] font-bold text-white">
                                        {product.stock} left
                                    </div>
                                </div>
                                <div className="p-4">
                                    <p className="text-xs text-slate-500 uppercase tracking-widest font-bold mb-1">{product.category}</p>
                                    <h3 className="font-bold text-slate-200 mb-2 truncate">{product.name}</h3>
            ...
            {/* Right Side: Cart */}
            <div className="w-[450px] bg-slate-900/80 backdrop-blur-xl border-l border-slate-800/60 flex flex-col relative z-10 shadow-[-20px_0_40px_rgba(0,0,0,0.5)]">
                {/* Customer Select */}
                <div className="p-6 border-b border-slate-800/60">
                    <button className="w-full h-12 bg-black/40 border border-slate-700 hover:border-cyan-500/50 rounded-xl flex items-center justify-between px-4 transition-all group">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center">
                                <User className="w-4 h-4 text-slate-400" />
                            </div>
                            <span className="text-sm font-bold text-slate-300 group-hover:text-cyan-400 transition-colors">Select Customer</span>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                    {cart.map((item, idx) => (
                        <div key={idx} className="bg-black/30 border border-slate-800/60 rounded-xl p-4 flex gap-4">
                            <img src={item.product.image} className="w-16 h-16 rounded-lg object-cover border border-slate-700" alt="" />
                            <div className="flex-1 flex flex-col justify-between">
                                <div className="flex justify-between items-start">
                                    <h4 className="text-sm font-bold text-slate-200 line-clamp-2">{item.product.name}</h4>
                                ...
                                <div className="flex justify-between items-center mt-2">
                                    ...
                                    <div className="flex items-center gap-3 bg-slate-900 border border-slate-700 rounded-lg p-1">
                                        <button className="w-6 h-6 flex items-center justify-center hover:bg-slate-800 rounded text-slate-400">
                ...
                {/* Cart Summary */}
                <div className="p-6 bg-black/60 border-t border-slate-800/60 mt-auto">
                    <div className="space-y-3 mb-6">
                        ...
                        <div className="flex justify-between items-end">
                            ...
                            <span className="text-3xl font-black text-white font-mono tracking-tighter">₹{total.toLocaleString()}</span>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                        <button className="h-14 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors border border-slate-700">
