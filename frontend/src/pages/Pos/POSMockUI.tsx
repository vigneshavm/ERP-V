...
import posData from '../../mockData/posData.json';
...
const PRODUCTS: Product[] = posData.products;
                ...
                <div className="px-8 py-4 flex gap-3 overflow-x-auto no-scrollbar border-b border-default">
                    {posData.categories.map((cat) => (
                        <button 
                            ...
                            className={`whitespace-nowrap px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${activeCategory === cat ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'glass-panel text-muted border border-default hover:bg-card'}`}
                        >
                            {cat}
                        </button>
                    ))}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {filteredProducts.length > 0 ? (
                            ...
                            <div className="col-span-full flex flex-col items-center justify-center py-20 text-center">
                                <Search className="w-12 h-12 text-secondary opacity-20 mb-4" />
                                <h3 className="text-lg font-bold text-main/60">No products found</h3>
                                <p className="text-sm text-secondary">Try adjusting your search or category filter</p>
                            </div>
                        )}
                    </div>
                </div>
