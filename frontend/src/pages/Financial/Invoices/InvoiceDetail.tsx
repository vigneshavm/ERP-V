...
import { getInvoiceById, resetPosState, clearInvoice } from '../../../redux/slices/posSlice';
import Layout from '../../../components/shared/Layout/Layout';
import { RootState } from '../../../redux/store';
import { AppDispatch } from '../../../redux/store';
import { PopulatedInvoice } from '../../../types/sales';

const InvoiceDetail = () => {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const dispatch = useDispatch<AppDispatch>();
    const { invoice, isLoading, isError, message } = useSelector((state: RootState) => state.pos);
    ...
    const fmt = (value: number | undefined | null) => Number(value || 0).toFixed(2);
    const items = Array.isArray(invoice.items) ? invoice.items : [];

    // Cast invoice to PopulatedInvoice to access deep properties safely if we know it's populated
    // Ideally, useSelector would return 'Invoice | null'. 
    // If invoice.customer is an object, we can cast or check type.
    const customer = typeof invoice.customer === 'object' && invoice.customer !== null ? invoice.customer : null;
                        ...
                        <div className="flex space-x-2">
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    // dispatch(reset()); // Reset doesn't seem to be imported or exist in slice logic shown. resetPosState is used in unmount.
                                    navigate('/pos');
                                }}
                ...
                <div className="bg-white dark:bg-[rgb(var(--color-card))] rounded-xl shadow-sm dark:shadow-lg border dark:border-[rgb(var(--color-border))] p-8 print:shadow-none">
                    ...
                    <div className="grid grid-cols-2 gap-8 mb-8">
                        <div>
                            ...
                            {customer ? (
                                <div>
                                    <div className="font-bold text-gray-900 dark:text-[rgb(var(--color-text))] text-lg">{customer.name}</div>
                                    <div className="text-gray-600 dark:text-[rgb(var(--color-text-secondary))] mt-1">{customer.phone}</div>
                                    {customer.email && (
                                        <div className="text-gray-600 dark:text-[rgb(var(--color-text-secondary))]">{customer.email}</div>
                                    )}
                                    {customer.address && (
                                        <div className="text-gray-600 dark:text-[rgb(var(--color-text-secondary))] mt-1">{customer.address}</div>
                                    )}
                                </div>
                            ) : (
                    ...
                    <div className="flex justify-end">
                        <div className="w-64">
                            ...
                            {customer && customer.dues !== undefined && (
                                <div className={`flex justify-between py-2 mt-2 pt-2 border-t ${customer.dues < 0 ? 'bg-green-50' : customer.dues > 0 ? 'bg-red-50' : 'bg-gray-50'}`}>
                                    <span className="text-sm font-medium text-gray-700">
                                        {customer.dues < 0 ? 'Available Credit Balance:' : customer.dues > 0 ? 'Customer Outstanding Due:' : 'Account Balance:'}
                                    </span>
                                    <span className={`text-sm font-bold ${customer.dues < 0 ? 'text-green-700' : customer.dues > 0 ? 'text-red-700' : 'text-gray-700 dark:text-[rgb(var(--color-text))]'}`}>
                                        ₹{fmt(Math.abs(customer.dues))}
                                    </span>
                                </div>
                            )}
                        </div>
