...
const OutstandingPayables: React.FC = () => {
    ...
    const agedData = useMemo(() => {
        ...
        orders
            .filter(o => o.status === 'Approved' || o.status === 'Converted')
            .forEach(order => {
                const vendor = suppliers.find(v => v._id === order.vendor_id);
                const invoiceDate = new Date(order.po_date);
                const creditPeriod = vendor?.creditPeriod || 30;

                const dueDate = order.expected_delivery ? new Date(order.expected_delivery) : new Date(invoiceDate);
                ...
                const payable: OutstandingPayable = {
                    ...
                    supplierName: vendor?.businessName || order.vendor_name || 'Unknown Supplier',
                    orderNo: order.po_number || 'N/A',
