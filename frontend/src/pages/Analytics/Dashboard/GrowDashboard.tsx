...
import { Rocket, TrendingUp, Users, ShoppingCart, ArrowRight, LayoutGrid, Sparkles, MessageSquare, Bell } from 'lucide-react';
...
const GrowDashboard: React.FC = () => {
    ...
    const stats = [
        { label: 'Total Online Sales', value: '₹42,350', change: '+12.5%', target: 'Target: ₹50k', icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
        { label: 'Website Visitors', value: '1,240', change: '+18.2%', target: 'Target: 1.5k', icon: Users, color: 'text-blue-500', bg: 'bg-blue-500/10' },
        { label: 'Conversion Rate', value: '3.24%', change: '+0.8%', target: 'Avg: 2.8%', icon: ShoppingCart, color: 'text-purple-500', bg: 'bg-purple-500/10' },
    ];

    const alerts = [
        { title: 'Google Business Profile', message: 'You have 4 new reviews to respond to.', type: 'action', icon: MessageSquare },
        { title: 'Inventory Warning', message: '3 online-exclusive items are low on stock.', type: 'warning', icon: Bell },
        { title: 'Campaign Success', message: 'WhatsApp campaign "Weekend Sale" reached 1.2k users.', type: 'insight', icon: Sparkles },
    ];
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
                    ...
                    <div className="grid grid-cols-1 gap-4 w-full lg:w-72">
                        <div className="bg-white/5 backdrop-blur-xl rounded-[2rem] p-6 border border-white/10">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-black">All Systems Operational</p>
                                    <p className="text-[10px] text-slate-500 font-bold">Last Sync: 2m ago</p>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white/5 backdrop-blur-xl rounded-[2rem] p-6 border border-white/10">
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Next Milestone</h4>
                            <p className="text-sm font-black mb-1">₹50,000 Sales Target</p>
                            <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                                <div className="bg-indigo-500 h-full w-[84%] rounded-full shadow-[0_0_10px_rgba(99,102,241,0.5)]" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {stats.map((stat, idx) => (
                    <div key={idx} className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm group hover:border-indigo-500 transition-all cursor-pointer">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-10 border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {alerts.map((alert, idx) => (
                        <div key={idx} className="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-100 dark:border-slate-700 flex items-start gap-4">
                            <div className={`mt-1 p-2 rounded-lg ${alert.type === 'action' ? 'bg-indigo-500/10 text-indigo-500' : alert.type === 'warning' ? 'bg-amber-500/10 text-amber-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
