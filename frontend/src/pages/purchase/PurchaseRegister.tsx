...
const PurchaseRegister: React.FC = () => {
    ...
    const sectorOrders = useMemo(() =>
        orders.filter(o => o.sector === currentSector),
        [orders, currentSector]
    );
    ...
    const filteredOrders = useMemo(() => {
        return sectorOrders.filter(order => {
            ...
            if (searchTerm) {
                ...
                if (!order.vendor.toLowerCase().includes(search) &&
            ...
            if (dateTo) {
                ...
                if (new Date(order.date) >= nextDay) return false;
            }
    ...
    // Summary stats
    const totalPurchases = filteredOrders.reduce((acc, o) => acc + o.total, 0);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-neutral-500 text-sm mt-1">
                        Complete record of all purchase transactions • {getBranchName(currentBranch)}
                    </p>
                </div>
                                    ...
                                    <tr key={order.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-700/50">
                                        ...
                                        <td className="p-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-medium text-neutral-900 dark:text-white">{order.vendor}</span>
                                            </div>
                                        </td>
                                        <td className="p-4 text-neutral-500">{getBranchName(order.branchId)}</td>
                            ...
                            <div key={order.id} className="p-4">
                                <div className="flex justify-between items-start mb-2">
                                    <div>
                                        ...
                                        <p className="font-bold text-neutral-900 dark:text-white">{order.vendor}</p>
                                    </div>
                                    {getStatusBadge(order.status)}
                                </div>
