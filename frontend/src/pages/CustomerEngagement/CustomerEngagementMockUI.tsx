...
const CustomerEngagementMockUI: React.FC = () => {
    return (
        <div className="min-h-screen bg-[#050408] text-slate-200 font-sans selection:bg-rose-500/30 overflow-hidden flex flex-col">
            ...
            <main className="relative z-10 flex-1 flex flex-col max-w-[1600px] w-full mx-auto px-8 py-8">
                <header className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1 font-medium">Drive loyalty, gather feedback, and nurture customer relationships at scale.</p>
                    </div>
                    <div className="flex gap-4">
                        ...
                        <button className="h-11 px-6 bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-500 hover:to-pink-500 text-white font-black uppercase tracking-widest text-xs rounded-xl transition-all shadow-[0_0_20px_rgba(244,63,94,0.3)] flex items-center gap-2">
                ...
                <div className="grid grid-cols-4 gap-6 mb-8">
                    {[
                        ...
                        { label: 'Negative Feedback', val: '11%', sub: '42 issues raised', icon: ThumbsDown, color: 'text-slate-400', bg: 'bg-slate-500/10', border: 'border-slate-700' },
                    ].map((card, i) => (
                ...
                <div className="grid grid-cols-3 gap-6 mb-6">
                    {[
                        ...
                        <div key={i} className={`bg-slate-900/40 backdrop-blur-xl border border-slate-800/60 rounded-3xl p-6 hover:bg-slate-800/40 transition-all group cursor-pointer`}>
                            ...
                            <div className="grid grid-cols-3 gap-3 mb-5">
                                {tool.stats.map((s, si) => (
                                    <div key={si} className="bg-slate-800/60 rounded-xl p-3 text-center">
                            ...
                            <div className={`flex items-center justify-between px-4 py-3 bg-${tool.color}-500/5 border border-${tool.color}-500/20 rounded-xl`}>
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{tool.rateLabel}</span>
                    ...
                    <div className="p-5 border-b border-slate-800/60 flex justify-between items-center bg-black/20">
                        ...
                        <div className="flex gap-3">
                            <div className="relative">
                                ...
                                <input type="text" placeholder="Search feedback..." className="w-60 bg-black/50 border border-slate-700/50 rounded-xl py-2 pl-10 pr-4 text-sm focus:outline-none focus:border-rose-500 transition-colors text-slate-200 placeholder:text-slate-600" />
                            </div>
                            <button className="h-9 px-4 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl text-xs font-bold text-slate-300 flex items-center gap-2">
                            ...
                            <thead className="bg-slate-900/80 sticky top-0 z-20 backdrop-blur-md">
                                <tr>
                                    {['Customer', 'Channel', 'Rating', 'Feedback', 'Date', 'Sentiment', 'Actions'].map(h => (
                                        <th key={h} className={`px-8 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500 border-b border-slate-800/60 ${h === 'Rating' || h === 'Sentiment' || h === 'Actions' ? 'text-center' : ''}`}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                                    ...
                                    <tr key={idx} className="hover:bg-slate-800/30 transition-colors group">
                                        <td className="px-8 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-sm font-bold text-slate-200">{fb.customer}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-4"><span className="text-xs font-bold text-slate-400">{fb.channel}</span></td>
                                        ...
                                        <td className="px-8 py-4"><p className="text-sm text-slate-400 max-w-xs truncate">{fb.feedback}</p></td>
                                        <td className="px-8 py-4"><span className="text-sm font-bold text-slate-400">{fb.date}</span></td>
                                        ...
                                        <td className="px-8 py-4"><div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button className="p-2 text-slate-400 hover:text-rose-400 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors"><MoreHorizontal className="w-4 h-4" /></button>
                                        </div></td>
                                    </tr>
                                ))}
                            </tbody>
