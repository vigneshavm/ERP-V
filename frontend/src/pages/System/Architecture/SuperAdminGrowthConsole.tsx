...
const SuperAdminGrowthConsole: React.FC = () => {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-primary text-xs font-bold uppercase tracking-wider rounded">Tenant Management</span>
                        </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-sm border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden animate-in slide-in-from-bottom-4 duration-500">
                                ...
                                <tr key={tenant.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg flex items-center justify-center text-primary font-bold text-xs">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${tenant.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-success' : tenant.status === 'SUSPENDED' ? 'bg-rose-50 text-rose-700 dark:bg-rose-900/20 dark:text-danger' : 'bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-warning'}`}>
                                            {tenant.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button
                                            ...
                                            className="p-2 text-slate-400 hover:text-primary hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-all"
