    ...
    const statementData: StatementData | null = useMemo(() => {
        ...
        const customerSales = salesHistory
            .filter(s =>
                ...
                new Date(s.date) >= new Date(periodFrom) &&
                new Date(s.date) <= new Date(periodTo)
            )
        ...
        customerSales.forEach(sale => {
            ...
            transactions.push({
                date: sale.date,
                reference: `INV-${sale.id.substring(0, 8)}`,
            ...
            // Payment
            const paid = (sale as any).amountPaid ?? sale.total;
            if (paid > 0) {
                runningBalance -= paid;
                transactions.push({
                    date: sale.date,
                    reference: `REC-${sale.id.substring(0, 6)}`,
