...
const CustomerDetail = () => {
        ...
        return (
            <Layout>
                <div className="p-20 text-center flex flex-col items-center gap-4">
                    ...
                    <button onClick={() => navigate('/customers')} className="mt-4 px-8 py-3 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all">Return to Portfolio</button>
                </div>
            </Layout>
        );
    }
    ...
    const DetailCard = ({ title, icon: Icon, children }) => (
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm space-y-8">
            <div className="flex items-center gap-4">
                ...
                <h3 className="text-lg font-black text-slate-800 dark:text-white leading-none uppercase tracking-tight">{title}</h3>
            </div>
    ...
    const DataPoint = ({ label, value, subLabel }) => (
        <div className="group">
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-1.5 transition-colors group-hover:text-indigo-500">{label}</p>
            <p className="text-sm font-black text-slate-700 dark:text-slate-200 ml-1">{value || '--'}</p>
            {subLabel && <p className="text-[10px] font-bold text-slate-400 italic ml-1 mt-0.5">{subLabel}</p>}
        </div>
    );
    ...
    return (
        <Layout>
            <PageHeader
                title={customer.name}
                description={`Monitoring strategic relations for ${customer.name} (Global ID: CID-${customer._id.slice(-6).toUpperCase()})`}
                breadcrumbs={[{ label: 'Portfolio', link: '/customers' }, { label: 'Partner Intel' }]}
                actions={
                    <div className="flex gap-2">
                        <button
                            onClick={() => navigate(`/customers/edit/${customer._id}`)}
                            className="flex items-center gap-2 px-6 py-2 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 rounded-xl text-sm font-bold hover:bg-slate-50 transition-all"
                        >
                            Modify Profile
                        </button>
                        <button
                            onClick={() => navigate(`/customers/adjust-due/${customer._id}`)}
                            className="flex items-center gap-2 px-6 py-2 bg-emerald-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-200 dark:shadow-none hover:bg-emerald-700 transition-all"
                        >
                            Adjust Ledger
                        </button>
                    </div>
                ...
                <div className="col-span-12 lg:col-span-4 space-y-6">
                    ...
                    <DetailCard title="Partner Intel" icon={UserCheck}>
                        <div className="space-y-6">
                            ...
                            <DataPoint label="Market Origin" value={customer.referredBy?.name} subLabel="Source of introduction" />
                        </div>
                    </DetailCard>
                </div>
                ...
                <div className="col-span-12 lg:col-span-8 space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-800">
                                    <div className="flex items-center gap-3 text-slate-500">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest">Joined {new Date(customer.createdAt).toLocaleDateString(undefined, { month: 'short', year: 'numeric' })}</span>
                                    </div>
                                </div>
                            </div>
                        </DetailCard>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                        <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/20">
                            ...
                            <button className="p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-white dark:hover:bg-slate-800 rounded-xl transition-all shadow-sm border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                        {transactions.map((tx, index) => (
                                            <tr key={index} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-all">
