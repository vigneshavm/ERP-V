...
export const ReturnInvoiceSelector: React.FC<ReturnInvoiceSelectorProps> = ({ onSelectInvoice }) => {
    ...
    const filteredSales = salesHistory.filter(sale => {
        ...
        const customerPhone = getCustomerPhone(sale.customerId);
                        ...
                        >
                            <div>
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="font-mono font-bold text-slate-700 dark:text-slate-300">#{sale.id.substring(0, 8)}</span>
                                    <span className="text-xs text-slate-400">• {new Date(sale.date).toLocaleDateString()}</span>
                                </div>
                                <div className="text-sm font-medium text-slate-900 dark:text-white">
                                    {getCustomerName(sale.customerId)} <span className="text-slate-400 font-normal">({getCustomerPhone(sale.customerId)})</span>
                                </div>
                                <div className="text-xs text-slate-500 mt-1">
                                    {sale.items.length} Items • <span className={sale.status === 'COMPLETED' ? 'text-emerald-600' : 'text-amber-600'}>{sale.status}</span>
                                </div>
                            </div>
                            <div className="text-right">
                                <div className="text-lg font-bold text-slate-900 dark:text-white">₹{sale.total.toFixed(2)}</div>
