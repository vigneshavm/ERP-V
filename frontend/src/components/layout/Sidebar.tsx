...
import { RootState, setSidebarOpen, setDesktopCollapsed, setBranch, setSyncing } from '../../store';
...
const Sidebar: React.FC<SidebarProps> = ({ onLogout }) => {
            ...
            return (
                <div key={item.id}>
                    <NavGroup
                        ...
                        defaultOpen={isGroupActive(item) || item.isGrow}
                    >
                        {visibleChildren.map(child => renderRecursive(child, true))}
                    </NavGroup>
                </div>
            );
        }
    ...
    const erpItems = useMemo(() => MENU_ITEMS.filter(item => !item.isGrow), []);
    const growItems = useMemo(() => MENU_ITEMS.filter(item => item.isGrow), []);

    // Filtered items (only visible ones)
    const visibleErpItems = useMemo(() => erpItems.filter(item => {
        const hasAccess = checkAccess(item.id) && checkModuleAccess(item.module);
        if (hasAccess) return true;
        // Also show if it has visible children
        return item.children?.some(child => checkAccess(child.id) && checkModuleAccess(child.module));
    }), [erpItems, checkAccess, checkModuleAccess]);

    const visibleGrowItems = useMemo(() => growItems.filter(item => {
        const hasAccess = checkAccess(item.id) && checkModuleAccess(item.module);
        if (hasAccess) return true;
        return item.children?.some(child => checkAccess(child.id) && checkModuleAccess(child.module));
    ...
    return (
        ...
        `}>
            ...
            <nav className="flex-1 space-y-1 overflow-y-auto custom-scrollbar pr-2">
                ...
                {visibleErpItems.map((item) => renderRecursive(item, false))}
