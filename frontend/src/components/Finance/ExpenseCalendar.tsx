        ...
        <div className="bg-slate-900/40 border border-white/5 rounded-[2rem] p-6 backdrop-blur-xl">
            ...
            <div className="grid grid-cols-7 gap-2">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                    <div key={day} className="text-center text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">
                        {day}
                    </div>
                ))}
                ...
                {days.map((d, i) => (
                    ...
                    >
                        <span className="text-xs font-bold text-slate-400 group-hover:text-white">{d.day}</span>
                        {d.expense > 0 && (
