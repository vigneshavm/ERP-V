...
import { Sale, Customer, Session, SaleStatus, CartItem, Invoice } from "../types";
...
export const createInvoice = createAsyncThunk(
    ...
    async (invoiceData: any, thunkAPI) => {
        try {
            ...
            const response = await api.post<any>(`${API_URL}/invoice`, invoiceData, getConfig(token));
            return response;
        } catch (error: any) {
            const message =
...
export const getAllInvoices = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        try {
            ...
            const response = await api.get<any>(`/api/sales-invoice/invoices`, getConfig(token));
            return response;
        } catch (error: any) {
            const message =
                (error.response && error.response.data && error.response.data.message) ||
...
export const getInvoiceById = createAsyncThunk(
    ...
    async (id: string, thunkAPI) => {
        try {
            ...
            const token = state.auth.user?.token;
            if (!token) return thunkAPI.rejectWithValue("Not authenticated");
            const response = await api.get<any>(`${API_URL}/invoice/${id}`, getConfig(token));
            return response;
        } catch (error: any) {
            const message =
                (error.response && error.response.data && error.response.data.message) ||
...
>(
    ...
    async ({ id, status }, thunkAPI) => {
        try {
            ...
            const token = state.auth.user?.token;
            if (!token) return thunkAPI.rejectWithValue("Not authenticated");
            const response = await api.put<any>(`${API_URL}/invoice/${id}/status`, { status }, getConfig(token));
            return { id, status: response.status || status };
        } catch (error: any) {
            const message =
                (error.response && error.response.data && error.response.data.message) ||
                error.message ||
...
export const posSlice = createSlice({
    ...
    reducers: {
        ...
        removeFromCart: (state, action: PayloadAction<string>) => {
            const session = state.sessions[state.activeSessionIndex];
            if (!session) return;
            session.cart = session.cart.filter((i: CartItem) => i.id !== action.payload);
        },
        updateCartQty: (state, action: PayloadAction<{ id: string; qty: number }>) => {
            const session = state.sessions[state.activeSessionIndex];
            if (!session) return;
            const item = session.cart.find((i: CartItem) => i.id === action.payload.id);
            if (item) {
        ...
        setTaxMode: (state, action: PayloadAction<typeof TaxMode[keyof typeof TaxMode]>) => {
            const session = state.sessions[state.activeSessionIndex];
