    ...
    <div className="space-y-6 relative">
        ...
        {advanceModalEmp && (
            <div className="absolute inset-0 z-20 bg-black/60 backdrop-blur-sm flex items-center justify-center rounded-xl">
                <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-2xl w-80">
                    <h3 className="text-lg font-bold text-white mb-4">Give Advance</h3>
                    <div className="mb-4">
                        <label className="text-xs text-slate-400 font-bold uppercase mb-1 block">Amount ($)</label>
                        <input 
                            type="number" 
                            autoFocus
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => setAdvanceModalEmp(null)} className="flex-1 py-2 bg-slate-700 text-slate-300 rounded hover:bg-slate-600">Cancel</button>
        ...
        {isAdding && (
            <form onSubmit={handleAdd} className="bg-slate-800 p-6 rounded-xl border border-slate-700 grid grid-cols-1 md:grid-cols-4 gap-4 animate-fade-in">
                ...
                <div className="flex flex-col gap-1">
                     ...
                     <input required placeholder="Full Name" className="bg-slate-700 p-2 rounded text-white" value={newEmp.name} onChange={e => setNewEmp({...newEmp, name: e.target.value})} />
                </div>
                <div className="flex flex-col gap-1">
                     <label className="text-xs text-slate-400 font-bold uppercase">Role</label>
                     <input required placeholder="Role (e.g. Cashier)" className="bg-slate-700 p-2 rounded text-white" value={newEmp.role} onChange={e => setNewEmp({...newEmp, role: e.target.value})} />
                </div>
                <div className="flex flex-col gap-1">
                     <label className="text-xs text-slate-400 font-bold uppercase">Daily Rate</label>
                     <input required type="number" placeholder="Daily Rate ($)" className="bg-slate-700 p-2 rounded text-white" value={newEmp.dailyRate} onChange={e => setNewEmp({...newEmp, dailyRate: e.target.value})} />
                </div>
                <div className="md:col-span-4 flex justify-end gap-3 mt-2 border-t border-slate-700 pt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {sectorEmps.map(emp => {
                ...
                return (
                    <div key={emp.id} className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg relative overflow-hidden group hover:border-indigo-500/50 transition-all">
                        <div className="absolute top-0 right-0 p-2">
                             <span className="text-[10px] bg-slate-900 border border-slate-700 px-2 py-1 rounded text-slate-400 font-bold">{emp.branch}</span>
                        </div>
                        <div className="flex items-start justify-between mb-4 mt-2">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 uppercase tracking-wide">{emp.role}</p>
                                </div>
                            </div>
                            <div className="text-right mt-1">
                                <span className="block text-xl font-bold text-indigo-400">${emp.dailyRate}</span>
                        ...
                        <div className="space-y-4">
                            <div className="bg-slate-900/50 p-3 rounded-lg flex justify-between items-center">
                                ...
                                <div className="flex gap-2">
                                    <button 
                                        ...
                                        className={`px-3 py-1 rounded text-xs font-bold transition-all ${status === 'PRESENT' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-900/40' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'}`}
                                    >
                                        P
                                    </button>
                                    <button 
                                        onClick={() => dispatch(markAttendance({ id: Math.random().toString(), employeeId: emp.id, date: today, status: 'ABSENT', advanceTaken: 0 }))}
                                        className={`px-3 py-1 rounded text-xs font-bold transition-all ${status === 'ABSENT' ? 'bg-red-500 text-white shadow-lg shadow-red-900/40' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'}`}
                                    >
                                        A
                                    </button>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-slate-900/50 p-3 rounded-lg">
                                    ...
                                    <p className="text-white font-bold">${accruedSalary.toLocaleString()}</p>
                                </div>
                                <div className="bg-slate-900/50 p-3 rounded-lg relative group/adv cursor-pointer hover:bg-slate-700 transition-colors" onClick={() => setAdvanceModalEmp(emp.id)}>
                                    <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">Advances</p>
                                    <p className="text-red-400 font-bold">${totalAdvances.toLocaleString()}</p>
