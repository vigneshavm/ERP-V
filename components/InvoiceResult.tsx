...
const InvoiceResult: React.FC<InvoiceResultProps> = ({ initialData, file, sector, branch, onSave, onCancel }) => {
  ...
  const handleSave = () => {
    ...
    const finalItems = items.map(item => {
        // Generate a 12-digit barcode if one doesn't exist
          ...
          {items.map(item => (
            <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
              ...
              <td className="p-2">
                 <div className="relative">
                    <input 
                        ...
                        className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-2 py-1 w-32 text-slate-900 dark:text-slate-200 focus:border-indigo-500 focus:outline-none"
                        placeholder="Select..."
                    />
              ...
              <td className="p-2 text-slate-400">
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded px-2 py-1 cursor-not-allowed opacity-80">
              ...
              <td className="p-2">
                <div className="relative flex items-center">
                    <input 
                        ...
                        className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-2 py-1 w-20 pr-6 text-right text-emerald-600 dark:text-emerald-400 font-medium focus:border-emerald-500 focus:outline-none"
                    />
              ...
              <td className="p-2">
                <div className="relative flex items-center">
                    ...
                    <input 
                        ...
                        className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded pl-6 pr-2 py-1 w-24 text-right text-indigo-600 dark:text-indigo-400 font-bold focus:border-indigo-500 focus:outline-none"
                    />
                </div>
              </td>
              <td className="p-2">
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-2xl animate-fade-in relative transition-colors">
      {/* Loading Overlay Post-Save */}
      {isSaved && (
          <div className="absolute inset-0 z-50 bg-white/90 dark:bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center animate-fade-in">
              <CheckCircle2 className="w-16 h-16 text-emerald-500 mb-4 animate-bounce" />
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Data Committed</h3>
              <p className="text-slate-500 dark:text-slate-400">Inventory updated successfully.</p>
          </div>
      )}

      {/* Header */}
      <div className="h-16 px-6 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800 transition-colors">
         <div>
            ...
            <div className="text-xs text-slate-400 flex gap-2">
                <span className="bg-slate-200 dark:bg-slate-700 px-1.5 rounded">{sector}</span>
                <span className="bg-slate-200 dark:bg-slate-700 px-1.5 rounded">{branch}</span>
            </div>
         </div>

         {/* Layout Switcher */}
         <div className="flex bg-slate-200 dark:bg-slate-900 p-1 rounded-lg border border-slate-300 dark:border-slate-700">
            <button 
                onClick={() => setViewMode('SPLIT')}
                className={`p-2 rounded ${viewMode === 'SPLIT' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
                title="Split View"
            >
            ...
            <button 
                ...
                className={`p-2 rounded ${viewMode === 'DETAILS' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
                title="Grid Only"
            >
            ...
            <button 
                ...
                className={`p-2 rounded ${viewMode === 'PREVIEW' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
                title="Document Only"
            >
         ...
         <div className="flex gap-3">
             <button 
                onClick={onCancel}
                className="px-4 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors flex items-center gap-2 text-sm font-medium"
             >
                 <RotateCcw className="w-4 h-4" /> Discard
             </button>
             <button 
                onClick={handleSave}
                className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-colors shadow-lg shadow-emerald-500/20 flex items-center gap-2 font-bold"
             >
      ...
      <div className="flex-1 overflow-hidden">
          {viewMode === 'SPLIT' && (
              <div className="grid grid-cols-2 h-full divide-x divide-slate-200 dark:divide-slate-700">
                  <div className="h-full overflow-hidden bg-slate-50 dark:bg-slate-950/50">
                      <DataGrid />
                  </div>
                  <div className="h-full p-4 bg-slate-100 dark:bg-slate-900">
                      <Previewer />
                  </div>
              </div>
          )}
          {viewMode === 'DETAILS' && (
              <div className="h-full bg-slate-50 dark:bg-slate-950/50">
                   <DataGrid />
              </div>
          )}
          {viewMode === 'PREVIEW' && (
