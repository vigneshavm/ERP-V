  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        <div className="bg-slate-800 p-8 rounded-xl border border-slate-700 border-dashed text-center">
          <input 
            type="file" 
            id="invoiceUpload" 
            className="hidden" 
          ...
          <label htmlFor="invoiceUpload" className="cursor-pointer flex flex-col items-center gap-4">
            <div className="w-16 h-16 bg-indigo-600/20 text-indigo-500 rounded-full flex items-center justify-center">
               {isProcessing ? <Loader2 className="w-8 h-8 animate-spin" /> : <Upload className="w-8 h-8" />}
            </div>
            <div>
              <h3 className="text-xl font-bold text-slate-200">Upload Supplier Invoice</h3>
              <p className="text-slate-400 mt-2 text-sm">Supports IMG, PDF. Powered by Gemini AI.</p>
            </div>
            <span className="px-4 py-2 bg-slate-700 text-slate-300 rounded-lg text-sm hover:bg-slate-600 transition-colors">Select File</span>
          </label>
        </div>
                ...
                {['Alpha', 'Beta', 'Gamma'].map(b => (
                    <label key={b} className="flex items-center gap-2 cursor-pointer">
                        <input 
                            ...
                            className="text-indigo-600 focus:ring-indigo-500 bg-slate-700 border-slate-600"
                        />
        ...
        <div className="p-4 bg-indigo-900/20 border border-indigo-500/30 rounded-lg text-indigo-200 text-sm">
      ...
      {/* Order History */}
      <div className="bg-slate-800 rounded-xl border border-slate-700 flex flex-col h-full max-h-[calc(100vh-8rem)]">
        <div className="p-6 border-b border-slate-700">
            <h3 className="text-xl font-bold text-slate-100">Purchase Orders</h3>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {sectorOrders.map(order => (
                <div key={order.id} className="p-4 bg-slate-700/30 rounded-lg border border-slate-700 hover:bg-slate-700/50 transition-colors">
                    <div className="flex justify-between items-start mb-3">
                        <div>
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] font-bold px-1.5 py-0.5 bg-slate-600 text-slate-300 rounded">{order.branch}</span>
                                <h4 className="font-bold text-slate-200">{order.vendor}</h4>
                            </div>
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-bold ${order.status === 'APPROVED' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                            {order.status}
                        </span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-slate-300">{order.items.length} items</span>
