    ...
    <div className="space-y-6 animate-fade-in">
        <div className="flex justify-between items-center">
            ...
            <div className="text-right">
                ...
                <p className="text-xl font-bold text-emerald-400">${totalFilteredRevenue.toFixed(2)}</p>
            </div>
        </div>
        ...
        <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[200px]">
                <label className="text-xs text-slate-400 font-bold uppercase mb-1 block">Search Invoice ID</label>
                <div className="relative">
                    <input 
                        type="text" 
                        placeholder="e.g. 5x8s9..." 
                        className="w-full pl-9 pr-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white text-sm"
                        value={searchId}
            ...
            <div>
                <label className="text-xs text-slate-400 font-bold uppercase mb-1 block">From Date</label>
                <div className="relative">
                     <input 
                        type="date" 
                        className="pl-9 pr-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white text-sm"
                        value={dateFrom}
            ...
            <div>
                <label className="text-xs text-slate-400 font-bold uppercase mb-1 block">To Date</label>
                <div className="relative">
                     <input 
                        type="date" 
                        className="pl-9 pr-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white text-sm"
            ...
            <button 
                ...
                className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-lg text-sm font-bold"
            >
                Clear
            </button>
        </div>
                    ...
                    <tbody className="divide-y divide-slate-700">
                        ...
                        {filteredSales.map(sale => (
                            <tr key={sale.id} className="hover:bg-slate-700/50 transition-colors">
                                ...
                                <td className="p-4 text-white font-medium">{getCustomerName(sale.customerId)}</td>
                                <td className="p-4 text-slate-400">{sale.items.length} items</td>
                                <td className="p-4">
                                    <span className="px-2 py-1 bg-slate-700 border border-slate-600 rounded text-xs font-bold text-slate-300">
                                        {sale.paymentMethod}
                                    </span>
                                </td>
                                <td className="p-4 text-right font-bold text-emerald-400">${sale.total.toFixed(2)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
