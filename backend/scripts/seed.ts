...
import Customer from "../src/modules/crm/models/Customer.js";
import Item from "../src/modules/inventory/models/Item.js";
import Supplier from "../src/modules/crm/models/Supplier.js";
import BankAccount from "../src/modules/finance/models/BankAccount.js";
import Invoice from "../src/modules/sales/models/Invoice.js";
import Counter from "../src/modules/core/models/Counter.js";
...
const clearData = async (userId: mongoose.Types.ObjectId) => {
    ...
    await Tenant.deleteMany({ ownerId: userId });
    console.log("   ✓ Cleared tenants");
};

// Create demo user
const createDemoUser = async (planId: mongoose.Types.ObjectId) => {
    console.log("\n👤 Creating demo user...");
    ...
    // Create a temporary tenant ID for the user
    ...
    console.log(`   ✓ Created demo user and tenant: ${user.email}`);
...
const createDefaultPlan = async () => {
    ...
    if (!plan) {
        ...
        console.log("   ℹ FREE plan already exists");
    }
    return plan._id as mongoose.Types.ObjectId;
};

// Create customers
const createCustomers = async (ownerId: mongoose.Types.ObjectId) => {
...
const createItems = async (userId: mongoose.Types.ObjectId) => {
...
// Create suppliers
const createSuppliers = async (ownerId: mongoose.Types.ObjectId) => {
...
// Create bank account
const createBankAccount = async (userId: mongoose.Types.ObjectId) => {
...
const createInvoices = async (userId: mongoose.Types.ObjectId, customers: any[], items: any[]) => {
...
const seedDatabase = async () => {
    ...
    try {
        ...
        // Create default plan
        const planId = await createDefaultPlan();

        // Create or get demo user
        const user = await createDemoUser(planId);

        // Clear existing data for this user
        await clearData(user._id as mongoose.Types.ObjectId);

        // Create all sample data
        const customers = await createCustomers(user._id as mongoose.Types.ObjectId);
        const items = await createItems(user._id as mongoose.Types.ObjectId);
        await createSuppliers(user._id as mongoose.Types.ObjectId);
        await createBankAccount(user._id as mongoose.Types.ObjectId);
        await createInvoices(user._id as mongoose.Types.ObjectId, customers, items);
