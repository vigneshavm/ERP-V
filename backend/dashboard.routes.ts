...
router.get('/', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const profileR = await db.query(
      'SELECT total_wealth, currency FROM users WHERE id = $1', [uid]
    );

    // 12-month rolling summaries
    const summariesR = await db.query(
      `SELECT
    ...
    const smsR = await db.query(
      `SELECT COUNT(*) AS pending_count, MAX(created_at) AS last_detected
       FROM sms_transactions WHERE user_id = $1 AND status = 'pending'`,
      [uid]
    );
    ...
    const result = await db.query(
      `SELECT
       ...
       WHERE t.user_id = $1
         ...
         AND EXTRACT(MONTH FROM t.date::date) = $3`,
      [req.user!.sub, year, month]
    );
