...
const sign = (userId, email) => ({
    accessToken: jwt.sign({ sub: userId, email, jti: crypto.randomUUID() }, process.env.JWT_SECRET, { expiresIn: (process.env.JWT_EXPIRES_IN || '30m') }),
    refreshToken: jwt.sign({ sub: userId, jti: crypto.randomUUID() }, process.env.JWT_REFRESH_SECRET, { expiresIn: (process.env.JWT_REFRESH_EXPIRES_IN || '30d') }),
});
// ─── Routes ───────────────────────────────────────────────────────────────────
...
router.post('/login', limiter, validate([body('email').isEmail().normalizeEmail(), body('password').notEmpty()]), async (req, res, next) => {
    try {
...
router.post('/refresh', validate([body('refreshToken').notEmpty()]), async (req, res, next) => {
    try {
        let payload;
        try {
...
router.post('/forgot-password', limiter, validate([body('email').isEmail().normalizeEmail()]), async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            await PasswordResetToken.findOneAndUpdate({ userId: user._id }, { tokenHandle: handle, tokenHash, expiresAt: new Date(Date.now() + 3_600_000) }, { upsert: true });
...
router.post('/reset-password', validate([
    ...
    try {
        ...
        const handle = tokenHandle(req.body.token);
...
router.post('/pin/set', authenticate, validate([body('pin').isLength({ min: 4, max: 6 }).isNumeric()]), async (req, res, next) => {
    try {
...
router.post('/pin/verify', authenticate, validate([body('pin').isLength({ min: 4, max: 6 }).isNumeric()]), async (req, res, next) => {
    try {
        const user = await User.findById(req.user.sub).select('pinHash');
        if (!user?.pinHash)
            throw AppError.badRequest('No PIN set');
        if (!await bcrypt.compare(req.body.pin, user.pinHash))
