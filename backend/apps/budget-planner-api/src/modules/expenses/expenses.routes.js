...
import { Category, Transaction, MonthlySummary, Budget, Notification, User, } from '../../models/index.js';
...
router.patch('/categories/:id', validate([
    param('id').isMongoId(),
    ...
    try {
        ...
        if (req.body.limit !== undefined)
            update.limitAmount = req.body.limit;
        if (!Object.keys(update).length)
            throw AppError.badRequest('No fields to update');
        const cat = await Category.findOneAndUpdate({ _id: req.params.id, userId: req.user.sub }, update, { new: true });
        if (!cat)
...
router.delete('/categories/:id', async (req, res, next) => {
    try {
        await Transaction.updateMany({ categoryId: req.params.id, userId: req.user.sub }, { $unset: { categoryId: 1 } });
...
// ─── TRANSACTIONS ─────────────────────────────────────────────────────────────
router.get('/', paginate(30), async (req, res, next) => {
    try {
    ...
    try {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                await Promise.all([
                    MonthlySummary.findOneAndUpdate({ userId: uid, monthDate: month }, { $inc: { [summaryField]: amount } }, { upsert: true, session }),
                    User.findByIdAndUpdate(uid, { $inc: { totalWealth: wealthDelta } }, { session }),
                ]);
                if (type === 'expense') {
                    await Budget.findOneAndUpdate({ userId: uid, month, 'items.categoryId': categoryId }, { $inc: { 'items.$.spent': amount } }, { session });
                    // Recalculate overspent flag after increment
                    await Budget.updateOne({ userId: uid, month, 'items.categoryId': categoryId }, [{
                            $set: {
...
router.get('/history/summary', async (req, res, next) => {
    try {
        const monthStr = req.query.month || toMonthKey(new Date());
        ...
        const total = byCategory.reduce((s, r) => s + r.value, 0);
...
router.get('/anomaly/check', async (req, res, next) => {
    try {
        ...
        if (!categoryId || !amount)
            throw AppError.badRequest('categoryId and amount are required');
...
router.patch('/:id', validate([
    ...
    try {
        ...
        if (!Object.keys(update).length)
            throw AppError.badRequest('No fields to update');
        const txn = await Transaction.findOneAndUpdate({ _id: req.params.id, userId: req.user.sub }, update, { new: true });
        if (!txn)
            throw AppError.notFound('Transaction not found');
        res.json({ success: true, data: txn });
    }
        ...
        try {
            await session.withTransaction(async () => {
                ...
                await Promise.all([
                    ...
                    MonthlySummary.findOneAndUpdate({ userId: req.user.sub, monthDate: month }, { $inc: { [summaryField]: -amount } }, { session }),
                ]);
            });
        }
        finally {
    ...
    try {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                await Promise.all([...monthImpact.entries()].map(([month, total]) => MonthlySummary.findOneAndUpdate({ userId: req.user.sub, monthDate: month }, { $inc: { totalExpense: -total } }, { session })));
            });
        }
        finally {
            session.endSession();
...
router.post('/bulk/recategorize', validate([
    body('ids').isArray({ min: 1, max: 100 }).withMessage('Max 100 ids per request'),
    body('targetCategoryId').isMongoId(),
]), async (req, res, next) => {
    try {
        const { ids, targetCategoryId } = req.body;
        await Transaction.updateMany({ _id: { $in: ids }, userId: req.user.sub }, { categoryId: targetCategoryId });
    ...
    try {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                if (type === 'expense') {
                    await Promise.all([
                        User.findByIdAndUpdate(req.user.sub, { $inc: { totalWealth: amount } }, { session }),
                        MonthlySummary.findOneAndUpdate({ userId: req.user.sub, monthDate: month }, { $inc: { totalExpense: -amount } }, { session }),
                    ]);
                }
            });
        }
