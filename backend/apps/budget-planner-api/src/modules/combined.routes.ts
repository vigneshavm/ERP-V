...
import {
  Goal, Loan, Account, CreditCard, DebitCard,
  Notification, Transaction, Budget, MonthlySummary,
  Category, RecurringBill, Contact, User,
...
goalsRouter.get('/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!r) throw AppError.notFound('Goal not found');
    res.json({ success: true, data: r });
  } catch (err) { next(err); }
});
...
goalsRouter.patch('/:id/progress',
  validate([param('id').isMongoId(), body('current').isFloat({ min: 0 })]),
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
...
goalsRouter.patch('/:id', validate([param('id').isMongoId()]),
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const update: Record<string, unknown> = {};
      ['name', 'target', 'current', 'icon', 'color', 'deadline'].forEach(f => { if (req.body[f] !== undefined) update[f] = req.body[f]; });
...
loansRouter.get('/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!r) throw AppError.notFound('Loan not found');
    res.json({ success: true, data: r });
  } catch (err) { next(err); }
});
...
loansRouter.patch('/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
loansRouter.post('/:id/payment',
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const wealthDelta   = (loan as any).type === 'Borrowed' ? -paymentAmount : paymentAmount;
      ...
      try {
        await session.withTransaction(async () => {
          await Loan.findByIdAndUpdate(
            loan._id,
            { $min: { current: (loan as any).total }, $inc: {} }, // clamp via $min after inc
            { session },
          );
          // Use findByIdAndUpdate to avoid stale-doc race on concurrent payments
          await Loan.findByIdAndUpdate(
            loan._id,
            [{ $set: { current: { $min: ['$total', { $add: ['$current', paymentAmount] }] } } }],
            { session },
          );
          await User.findByIdAndUpdate(
            req.user!.sub,
            { $inc: { totalWealth: wealthDelta } },
            { session },
          );
        });
      } finally {
...
accountsRouter.post('/',
  validate([body('name').trim().notEmpty(), body('type').isIn(['Savings', 'Checking', 'Current', 'Wallet']), body('balance').isFloat({ min: 0 }), body('color').matches(/^#[0-9A-Fa-f]{6}$/)]),
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
...
accountsRouter.patch('/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const update: Record<string, unknown> = {};
    ['name', 'color', 'selected'].forEach(f => { if (req.body[f] !== undefined) update[f] = req.body[f]; });
    if (!Object.keys(update).length) throw AppError.badRequest('No fields to update');
    const r = await Account.findOneAndUpdate({ _id: req.params.id, userId: req.user!.sub }, update, { new: true });
    if (!r) throw AppError.notFound('Account not found');
    res.json({ success: true, data: r });
  } catch (err) { next(err); }
});
...
cardsRouter.patch('/credit/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!r) throw AppError.notFound('Credit card not found');
    res.json({ success: true, data: r });
  } catch (err) { next(err); }
});
...
cardsRouter.patch('/debit/:id', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!r) throw AppError.notFound('Debit card not found');
    res.json({ success: true, data: r });
  } catch (err) { next(err); }
});
...
reportsRouter.get('/stats', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const remainingBudget = budget
      ? budget.totalAmount - budget.items.reduce((s: number, i: IBudgetItem) => s + i.spent, 0)
      : 0;
...
reportsRouter.get('/detailed', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const totals = txns.reduce((acc: Record<string, number>, t) => {
      acc[t.type] = (acc[t.type] || 0) + t.amount; return acc;
    }, {});
