...
export const evaluateSupplierPerformance = async (req, res) => {
    try {
...
export const bulkUpdateOpeningBalance = async (req, res) => {
    try {
        const tenantId = req.user?.tenantId?.toString();
        if (!tenantId)
            return res.status(401).json({ message: 'Unauthorized' });
...
export const getVendorInflowOutflow = async (req, res) => {
    try {
        const tenantId = req.user?.tenantId?.toString();
        if (!tenantId) {
            return res.status(401).json({ success: false, message: 'Unauthorized access' });
