 ...
 */
export const extractSupplierFromInvoice = async (req, res) => {
    try {
        if (!req.file) {
 ...
 */
export const extractSupplierFromText = async (req, res) => {
    try {
 ...
 */
export const extractPurchaseInvoice = async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'No invoice file uploaded' });
        }
