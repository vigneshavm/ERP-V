 ...
 */
export const extractSupplierFromInvoice = asyncHandler(async (req, res) => {
    if (!req.file) {
 ...
 */
export const extractSupplierFromText = asyncHandler(async (req, res) => {
 ...
 */
export const extractPurchaseInvoice = asyncHandler(async (req, res) => {
    if (!req.file) {
        return res.status(400).json({ success: false, message: 'No invoice file uploaded' });
    }
    const prompt = `
