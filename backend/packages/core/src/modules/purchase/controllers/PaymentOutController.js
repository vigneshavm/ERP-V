...
export const createPayment = asyncHandler(async (req, res) => {
    ...
    const { supplierId, paymentDate, amount, paymentMode, referenceNo, bankAccountId, chequeDate, allocations, // [{ billId, amount, discount }]
    notes } = req.body;
    // 1. Validations
    if (amount <= 0)
...
export const updatePaymentStatus = asyncHandler(async (req, res) => {
    ...
    if (bounceReason)
        payment.notes = (payment.notes || '') + ` [Bounce: ${bounceReason}]`;
    await payment.save({ session });
    // Application Logic
