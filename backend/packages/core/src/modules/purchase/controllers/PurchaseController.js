...
import Tenant from '@smarterp/core/modules/core/models/Tenant.js';
import Supplier from '../models/Supplier.js';
import { error } from '@smarterp/shared/config/logger.js';
import { asyncHandler } from '@smarterp/shared/utils/asyncHandler.js';
const generatePurchaseNumber = async (_tenantId) => {
...
export const createPurchase = asyncHandler(async (req, res) => {
    ...
    if (status === 'COMPLETED' && supplier) {
        ...
        if (!paymentPromiseDate && !overrideCreditLimit && !isExempt) {
            ...
            if (overdueBills > 0) {
                const error = new Error(`Supplier Lockout: ${overdueBills} overdue invoices. Manager action required.`);
            ...
            if (supplier.creditLimit && supplier.creditLimit > 0) {
                ...
                if (currentOutstanding + newAmount > supplier.creditLimit) {
                    const error = new Error(`Credit Limit Exceeded. Outstanding: ${currentOutstanding}, Limit: ${supplier.creditLimit}`);
                    error.statusCode = 403;
    ...
    for (let i = 0; i < purchase.items.length; i++) {
        ...
        if (product) {
            ...
            if (status === 'COMPLETED' || status === 'RECEIVED') {
                ...
                await inventoryService.addStock(item.productId.toString(), item.quantity, item.rate, {
                    batchNumber: item.lotNumber || `${purchaseNumber}-Batch`,
                    expiryDate: undefined, // Add inputs for this later
                    supplierId: purchase.vendorId.toString()
                }, req.user.tenantId || 'default', req.user);
            }
        }
    }
...
// ... existing exports ...
export const getSupplierTotals = asyncHandler(async (req, res) => {
...
export const getPurchaseHistory = asyncHandler(async (req, res) => {
    ...
    const history = purchases.map(p => {
        ...
        return {
            _id: p._id,
            date: p.date,
            vendorName: p.vendorId?.businessName || 'Unknown',
