...
export const updateUser = async (req, res) => {
    try {
        ...
        if (businessCategory !== undefined || shopName !== undefined || phone !== undefined || shopAddress !== undefined || email !== undefined || businessType !== undefined) {
            ...
            await BusinessProfile.findOneAndUpdate({ userId: user._id }, { $set: businessUpdate }, { new: true, upsert: true, setDefaultsOnInsert: true });
...
export const updateFinanceSettings = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.user?._id, { $set: { 'personalFinanceSettings.monthStartDay': monthStartDay } }, { new: true }).select('-password');
        if (!user) {
            res.status(404).json({ message: 'User not found' });
            return;
        }
