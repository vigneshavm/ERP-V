...
export const updateUser = asyncHandler(async (req, res) => {
    ...
    if (shopName !== undefined)
        userUpdate.shopName = shopName;
    if (gstNumber !== undefined)
        userUpdate.gstNumber = gstNumber;
    if (shopAddress !== undefined)
        userUpdate.shopAddress = shopAddress;
    const needsBusinessSync = businessCategory !== undefined || shopName !== undefined ||
    ...
    await session.withTransaction(async () => {
        ...
        if (needsBusinessSync) {
            ...
            if (shopName !== undefined)
                businessUpdate.businessName = shopName;
            if (businessCategory !== undefined)
                businessUpdate.category = businessCategory;
            if (businessType !== undefined)
                businessUpdate.businessType = businessType;
            if (phone !== undefined)
                businessUpdate.phone = phone;
            if (shopAddress !== undefined)
                businessUpdate.address = shopAddress;
            if (email !== undefined)
                businessUpdate.email = email;
            businessProfile = await BusinessProfile.findOneAndUpdate({ userId: user._id }, { $set: businessUpdate }, { new: true, upsert: true, setDefaultsOnInsert: true, session });
        }
    });
});
/**
 ...
 */
export const updateFinanceSettings = asyncHandler(async (req, res) => {
    ...
    const user = await User.findByIdAndUpdate(req.user._id, { $set: { 'personalFinanceSettings.monthStartDay': day } }, { new: true }).select('-password');
    if (!user) {
        res.status(404).json({ message: 'User not found' });
        return;
    }
