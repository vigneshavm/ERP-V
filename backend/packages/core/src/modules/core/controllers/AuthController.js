...
let AuthController = class AuthController {
    ...
    forgotPassword = async (req, res) => {
        ...
        const mailSent = await sendHtmlEmail(email, 'Reset your BizzAI password', html, text);
        if (!mailSent) {
    ...
    forceLogout = async (req, res) => {
        ...
        await RefreshToken.updateMany({ user: user._id, isRevoked: false }, { isRevoked: true, revokedAt: new Date() });
    ...
    logout = async (req, res) => {
        ...
        await Promise.all([
            refreshToken
                ? RefreshToken.findOneAndUpdate({ token: refreshToken, user: req.user?._id }, { isRevoked: true, revokedAt: new Date() })
                : Promise.resolve(),
            User.findByIdAndUpdate(req.user?._id, { $set: { activeDeviceId: null, activeSessionCreatedAt: null } }),
        ]);
