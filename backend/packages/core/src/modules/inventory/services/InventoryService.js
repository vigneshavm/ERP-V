...
let InventoryService = class InventoryService {
    ...
    async invalidateInventoryCache(user) {
    ...
    async addItem(itemData, tenantId, user) {
    ...
    async getAllItemsWithPagination(tenantId, queryParams) {
    ...
    async getSingleItem(itemId, tenantId) {
    ...
    async updateItem(itemId, tenantId, updateData, user) {
        const item = await this.inventoryRepository.findById(itemId, tenantId);
        if (!item) {
            throw new AppError("Item not found or unauthorized", 404);
        }
    ...
    async deleteItem(itemId, tenantId, user) {
    ...
    async getVariations(parentId, tenantId) {
    ...
    async checkStockAlerts(tenantId) {
    ...
    async bulkAdjustStock(ids, adjustment, type, tenantId, user) {
    ...
    async importItems(items, tenantId, user) {
        ...
        existingItems.forEach((item) => {
            if (item.sku)
                existingMap.set(`sku:${item.sku.toLowerCase()}`, item);
        ...
        items.forEach((item, index) => {
            ...
            if (item.costPrice === undefined || item.costPrice === '' || isNaN(parseFloat(item.costPrice)) || parseFloat(item.costPrice) < 0)
                rowErrors.push('Valid cost price required');
            if (item.sellingPrice === undefined || item.sellingPrice === '' || isNaN(parseFloat(item.sellingPrice)) || parseFloat(item.sellingPrice) < 0)
                rowErrors.push('Valid selling price required');
            if (item.unit && !validUnits.includes(item.unit.toLowerCase().trim()))
            ...
            if (skuKey && existingMap.has(skuKey))
                match = existingMap.get(skuKey);
            else if (nameKey && existingMap.has(nameKey))
                match = existingMap.get(nameKey);
    ...
    async addStock(itemId, quantity, rate, batchInfo, tenantId, user) {
        const item = await this.inventoryRepository.findById(itemId, tenantId);
        if (!item)
    ...
    async reduceStock(itemId, quantity, tenantId, user, reason = 'SALES') {
        const item = await this.inventoryRepository.findById(itemId, tenantId);
        if (!item)
            throw new AppError("Item not found", 404);
        ...
        const updatedBatches = [...(item.batches || [])].sort((a, b) => new Date(a.receivedDate).getTime() - new Date(b.receivedDate).getTime());
    ...
    async updateBatchCost(itemId, batchNumber, newCost, tenantId, user) {
        const item = await this.inventoryRepository.findById(itemId, tenantId);
        if (!item)
            throw new AppError("Item not found", 404);
        const batchIndex = item.batches?.findIndex((b) => b.batchNumber === batchNumber);
        if (batchIndex === -1)
    ...
    async bulkUpdateCategory(ids, category, tenantId, user) {
        const result = await Item.updateMany({ _id: { $in: ids }, tenantId }, { $set: { category } });
        await this.invalidateInventoryCache(user);
        return result.modifiedCount;
    }
    async toggleItemStatus(itemId, tenantId, user) {
    ...
    async deleteItemsBatch(ids, tenantId, user) {
        const result = await Item.deleteMany({ _id: { $in: ids }, tenantId });
        await this.invalidateInventoryCache(user);
        return result.deletedCount;
    }
    async recordMovement(itemId, movementData, tenantId, user) {
