...
import { getJournalEntries, createJournalEntry, getJournalEntryById, postJournalEntry, voidJournalEntry } from '../controllers/JournalEntryController.js';
...
router.patch("/:id/status", protect, (req, res, next) => {
    ...
    if (status === "posted")
        return postJournalEntry(req, res, next);
    if (status === "voided")
        return voidJournalEntry(req, res, next);
