...
export const getAllCategories = async (req, res) => {
    try {
        ...
        error(`Get all expense categories failed: ${err.message}`);
...
export const createCategory = async (req, res) => {
    try {
        ...
        error(`Create expense category failed: ${err.message}`);
        res.status(500).json({ message: 'Server Error', error: err.message });
    }
};
/**
...
export const updateCategory = async (req, res) => {
    try {
        ...
        const updatedCategory = await ExpenseCategory.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true, runValidators: true });
        if (!updatedCategory) {
        ...
        error(`Update expense category failed: ${err.message}`);
        res.status(500).json({ message: 'Server Error', error: err.message });
    }
};
/**
...
export const deleteCategory = async (req, res) => {
    try {
        ...
        error(`Delete expense category failed: ${err.message}`);
        res.status(500).json({ message: 'Server Error', error: err.message });
    }
};
export default {
