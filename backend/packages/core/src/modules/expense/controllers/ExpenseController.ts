...
export const createExpense = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (dup) throw new AppError('Expense number already exists', 400);
    ...
    if (paymentMethod === 'bank_transfer' && bankAccount) {
        ...
        await BankAccount.updateOne(
            ...
            { $inc: { currentBalance: -amount }, $push: { transactions: txn._id } }
        );
...
export const getExpenseSummary = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (year) {
        ...
        match.date = {
            ...
            $lte: new Date(y, month ? parseInt(month as string) : 12, 0, 23, 59, 59),
        };
    }
...
export const approveExpense = asyncHandler(async (req: Request, res: Response) => {
    ...
    const expense = await Expense.findOneAndUpdate(
        { _id: req.params.id, tenantId }, { status: 'approved' }, { new: true }
    );
