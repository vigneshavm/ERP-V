...
export const createExpense = async (req, res) => {
    try {
        ...
        if (paymentMethod === 'bank_transfer' && bankAccount) {
            ...
            await BankAccount.updateOne({ _id: bankAccount, userId: req.user?._id }, {
...
export const getExpenseSummary = async (req, res) => {
    try {
        ...
        if (year) {
            ...
            const end = new Date(y, month ? parseInt(month) : 12, 0, 23, 59, 59);
...
export const approveExpense = async (req, res) => {
    try {
        const expense = await Expense.findOneAndUpdate({ _id: req.params.id, tenantId: req.tenantId }, { status: 'approved' }, { new: true });
        if (!expense) {
        ...
        error(`Approve expense failed: ${err.message}`);
        res.status(500).json({ message: 'Server Error', error: err.message });
    }
};
export default {
