...
import JournalEntry from '@smarterp/core/modules/finance/models/JournalEntry.js';
...
let PayrollService = class PayrollService {
    ...
    async generatePayroll(tenantId, month, year, _userId, branchId) {
        ...
        try {
            ...
            const empQuery = { tenantId, isActive: true };
            if (branchId)
                empQuery.branchId = branchId;
            const employees = await Employee.find(empQuery);
            if (employees.length === 0)
                throw new AppError("No active employees found to process", 400);
            const payslips = [];
            let totalAmount = 0;
            for (const emp of employees) {
                ...
                if (structure && structure.components) {
                    for (const sc of structure.components) {
                        const component = sc.componentId;
                        if (!component)
                            continue;
                        let amount = 0;
                ...
                if (overtimeHours > 0) {
                    ...
                    if (emp.wageType === 'HOURLY')
                        ...
                        hourlyRate = (emp.dailyRate || 0) / 8;
                    else
                        hourlyRate = (basic / (workedDays || 1)) / 8;
                    overtimePay = hourlyRate * overtimeHours;
                    earnings.push({ name: 'Overtime Pay', amount: Number(overtimePay.toFixed(2)) });
                }
                // --- Variable Deductions ---
                if (advanceRecovery > 0)
                    deductions.push({ name: 'Salary Advance Recovery', amount: advanceRecovery });
                // Calculate Net
                ...
                payslips.push({
                    ...
                    bankDetailsSnapshot: emp.bankDetails
                });
            }
            // Create Run
    ...
    // 2. Approve Payroll
    async approvePayroll(runId, tenantId, userId) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            ...
            if (run.status !== 'DRAFT')
                throw new AppError("Run is not in Draft state", 400);
            run.status = 'APPROVED';
            ...
            for (const slip of payslips) {
                ...
                if (advanceRecovery && advanceRecovery.amount > 0) {
                    ...
                    await SalaryAdvance.updateMany({ employeeId: slip.employeeId, tenantId, status: 'PENDING' }, { $set: { status: 'RECOVERED', payrollRunId: runId } }, { session });
                }
            }
    ...
    async payPayroll(runId, accountId, paymentMode, tenantId, userId) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            const run = await PayrollRun.findOne({ _id: runId, tenantId }).session(session);
            ...
            if (run.status !== 'APPROVED')
                throw new AppError("Run must be Approved first", 400);
            ...
            await run.save({ session });
            // Update Payslips
            await Payslip.updateMany({ payrollRunId: run._id }, {
                $set: {
                    paymentStatus: 'PAID',
    ...
    async updatePayslip(payslipId, tenantId, updates) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            ...
            if (!run)
                throw new AppError("Run not found", 404);
            if (run.status !== 'DRAFT')
                throw new AppError("Cannot edit payslip. Run is locked.", 400);
            // Apply updates
            if (updates.basicSalary !== undefined)
                payslip.basicSalary = updates.basicSalary;
            if (updates.earnings)
                payslip.earnings = updates.earnings;
            if (updates.deductions)
                payslip.deductions = updates.deductions;
            // Recalculate Totals
            const totalEarnings = payslip.earnings.reduce((sum, e) => sum + Number(e.amount), 0) + payslip.basicSalary;
    ...
    async deleteRun(runId, tenantId) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            const run = await PayrollRun.findOne({ _id: runId, tenantId }).session(session);
            if (!run)
                throw new AppError("Run not found", 404);
            if (run.status !== 'DRAFT')
                throw new AppError("Cannot delete processed run", 400);
    ...
    async sendPayslip(payslipId, tenantId, channels) {
    ...
    async bulkUpdateStructure(tenantId, componentId, amount) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            ...
            for (const emp of employees) {
                ...
                const existingCompIndex = structure.components.findIndex((c) => c.componentId.toString() === componentId);
    ...
    async processIndividualPayout(tenantId, employeeId, month, year, userId, paymentMode, accountId, overrideWorkedDays, force = false) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            const periodStart = new Date(year, month, 1);
            ...
            if (emp.wageType === 'MONTHLY')
                ...
                basic = emp.hourlyRate * (attendance?.regularHours || 0); // Note: Hourly override not fully handled here, assuming Daily logic for overrides
            else
            ...
            if (!hasTea) {
                ...
                const teaAllowance = teaRate * workedDays;
                if (teaAllowance > 0)
                    earnings.push({ name: 'Tea Allowance', amount: Number(teaAllowance.toFixed(2)) });
            }
            if (advanceRecovery > 0)
                deductions.push({ name: 'Salary Advance Recovery', amount: advanceRecovery });
            const totalEarnings = earnings.reduce((sum, e) => sum + e.amount, 0) + basic;
            ...
            await JournalEntry.create([{
                    tenantId,
                    branchId: emp.branchId || 'HEAD_OFFICE',
                    date: new Date(),
            ...
            // Update Advances
            if (advanceRecovery > 0) {
                await SalaryAdvance.updateMany({ employeeId: emp._id, tenantId, status: 'PENDING' }, { $set: { status: 'RECOVERED', payrollRunId: run[0]._id } }, { session });
            }
            await session.commitTransaction();
