...
export const updateSalaryComponent = asyncHandler(async (req, res) => {
    ...
    const component = await SalaryComponent.findOneAndUpdate({ _id: id, tenantId }, updates, { new: true });
    if (!component) {
...
export const getAllSalaryStructures = asyncHandler(async (req, res) => {
    const user = req.user;
    const tenantId = user.tenantId;
...
export const bulkUpdateSalaryStructure = asyncHandler(async (req, res) => {
    const user = req.user;
    const tenantId = user.tenantId;
...
export const logAttendance = asyncHandler(async (req, res) => {
    const user = req.user;
    const tenantId = user.tenantId;
    const { employeeId, month, year, workedDays, notes, overtimeHours, regularHours, incentiveAmount, commissionAmount, bonusAmount, advanceRecoveryAmount, holidays, weeklyOffs, paidLeaves, lateDays, halfDays } = req.body;
    ...
    const attendance = await AttendanceSummary.findOneAndUpdate({ tenantId, employeeId, month, year }, {
        workedDays,
        notes,
        overtimeHours,
        regularHours,
...
export const getRunDetails = asyncHandler(async (req, res) => {
    const user = req.user;
    const tenantId = user.tenantId;
    const { id } = req.params;
...
export const processIndividualPayout = asyncHandler(async (req, res) => {
    const user = req.user;
    const tenantId = user.tenantId;
    const userId = user._id;
