...
export const createPaymentIn = asyncHandler(async (req, res) => {
    const { customerId, paymentMethods, allocatedInvoices = [], creditApplied = 0, depositAccount, notes = '', } = req.body;
    // ===== VALIDATION =====
    // 1. Validate customer
    if (!customerId) {
    ...
    for (const allocation of allocatedInvoices) {
        ...
        const invoiceBalance = invoice.totalAmount - invoice.paidAmount - invoice.returnedAmount;
    ...
    for (const allocation of validatedAllocations) {
        ...
        const newBalance = invoice.totalAmount - newPaidAmount - invoice.returnedAmount;
    ...
    if (depositAccount !== 'cash') {
        await BankAccount.findByIdAndUpdate(depositAccount, {
            $inc: { currentBalance: totalPayment },
        });
    }
    info(`Payment In created by ${req.user.name}: Receipt ${receiptNumber} - ₹${totalPayment} from ${customer.name}`);
    // Populate and return
 ...
 */
export const getPaymentInRecords = asyncHandler(async (req, res) => {
 ...
 */
export const getPaymentInById = asyncHandler(async (req, res) => {
 ...
 */
export const getCustomerOutstandingInvoices = asyncHandler(async (req, res) => {
 ...
 */
export const getCustomerPaymentInfo = asyncHandler(async (req, res) => {
    const { customerId } = req.params;
    if (!mongoose.Types.ObjectId.isValid(customerId)) {
        res.status(400).json({ message: 'Invalid customer ID format' });
        return;
