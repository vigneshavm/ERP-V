...
import { validateStockLevels } from '@smarterp/shared/utils/inventoryValidator.js';
...
export const createDeliveryChallan = asyncHandler(async (req, res) => {
    ...
    const { customerId, challanDate, deliveryDate, salesOrderId, items, vehicleNo, driverName, transportMode, notes, } = req.body;
    // Validate customer
    ...
    if (salesOrderId && mongoose.Types.ObjectId.isValid(salesOrderId)) {
        ...
        if (salesOrder) {
            ...
            for (const challanItem of validatedItems) {
                const soItem = salesOrder.items.find((i) => i.item.toString() === challanItem.item.toString());
                if (soItem) {
                ...
                await logStockMovement(item, 'DELIVER', challanItem.deliveredQty, challan._id, 'DeliveryChallan', req.user._id, previousState, newState);
                await logStockMovement(item, 'IN_TRANSIT', challanItem.deliveredQty, challan._id, 'DeliveryChallan', req.user._id, previousState, newState);
            }
 ...
 */
export const getAllDeliveryChallans = asyncHandler(async (req, res) => {
 ...
 */
export const updateDeliveryChallan = asyncHandler(async (req, res) => {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
        res.status(400).json({ message: 'Invalid challan ID' });
        return;
    }
    ...
    if (challanDate)
        challan.challanDate = challanDate;
    if (deliveryDate)
        challan.deliveryDate = deliveryDate;
    if (vehicleNo !== undefined)
        challan.vehicleNo = vehicleNo;
...
export const convertToInvoice = asyncHandler(async (req, res) => {
    ...
    const invoiceItems = challan.items.map((challanItem) => {
        ...
        if (salesOrder) {
            const soItem = salesOrder.items.find((i) => i.item.toString() === challanItem.item._id.toString());
            if (soItem) {
    ...
    if (challan.salesOrder) {
        ...
        if (salesOrderForUpdate) {
            ...
            for (const challanItem of challan.items) {
                const soItem = salesOrderForUpdate.items.find((i) => i.item.toString() === challanItem.item._id.toString());
                if (soItem) {
                    ...
                    if (soItem.invoicedQty > soItem.deliveredQty) {
                        throw new Error(`Invoiced quantity cannot exceed delivered quantity for item ${challanItem.item._id}`);
                    }
                    ...
                    if (item.reservedStock < 0) {
                        throw new Error(`Reserved stock cannot be negative for item ${item.name}`);
                    }
                    ...
                    await logStockMovement(item, 'INVOICE', challanItem.deliveredQty, invoice._id, 'Invoice', req.user._id, previousState, newState);
            ...
            // Update sales order status
            const totalInvoiced = salesOrderForUpdate.items.reduce((sum, item) => sum + item.invoicedQty, 0);
            const totalDelivered = salesOrderForUpdate.items.reduce((sum, item) => sum + item.deliveredQty, 0);
...
export const deleteDeliveryChallan = asyncHandler(async (req, res) => {
    ...
    if (challan.salesOrder) {
        ...
        if (salesOrder) {
            ...
            salesOrder.deliveryChallans = salesOrder.deliveryChallans.filter((dc) => dc.toString() !== challan._id.toString());
            // Update sales order status
            const totalOrdered = salesOrder.items.reduce((sum, item) => sum + item.quantity, 0);
            const totalDelivered = salesOrder.items.reduce((sum, item) => sum + item.deliveredQty, 0);
            if (totalDelivered === 0) {
