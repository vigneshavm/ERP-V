...
export const createReturn = async (req, res) => {
    try {
        const { invoiceId, items, refundMethod = 'credit', discountAmount = 0, notes = '', } = req.body;
        ...
        for (const returnItem of items) {
            ...
            const invoiceItem = invoice.items.find((invItem) => invItem.item.toString() === returnItem.productId);
            if (!invoiceItem) {
                res.status(400).json({
        ...
        for (const returnItem of processedItems) {
            if (returnItem.condition === 'not_damaged') {
                ...
                await Return.findOneAndUpdate({ _id: returnRecord._id, 'items.product': returnItem.product }, { $set: { 'items.$.inventoryAdjusted': true } });
            }
        }
        // Update invoice
        ...
        if (actualRefundMethod === 'bank_transfer' && refundBankAccount) {
            ...
            try {
                ...
                if (bankAcc) {
                    ...
                    const updateResult = await BankAccount.updateOne({ _id: refundBankAccount, userId: req.user?._id }, {
        ...
        info(`Return created by ${req.user?.name}: ${returnId} for invoice ${invoice.invoiceNo}`);
...
export const deleteReturn = async (req, res) => {
    try {
        ...
        if (returnRecord.refundProcessed && returnRecord.bankAccount) {
            ...
            if (bankAcc) {
                ...
                if (cashbankTxn) {
                    ...
                    await BankAccount.updateOne({ _id: returnRecord.bankAccount, userId: req.user?._id }, {
        ...
        info(`Return deleted by ${req.user?.name}: ${returnRecord.returnId}`);
