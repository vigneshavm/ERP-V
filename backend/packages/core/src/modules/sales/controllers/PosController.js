 ...
 * @access  Private
 */
export const getPosProducts = asyncHandler(async (_req, res) => {
 ...
 */
export const getPosSummary = asyncHandler(async (_req, res) => {
 ...
 */
export const processPosSale = asyncHandler(async (_req, res) => {
...
export const createInvoice = asyncHandler(async (req, res) => {
    ...
    const { customerId, items, discount = 0, paymentMethod, bankAccount, paidAmount = 0, changeReturned = 0, splitPaymentDetails = [], creditApplied = 0, previousDueAmount = 0 } = req.body;
    // 1. Basic Validation
    ...
    if (lastInvoice && lastInvoice.invoiceNo) {
        ...
        if (match)
            nextInvoiceNum = parseInt(match[1]) + 1;
    }
    ...
    if (actualReceived > 0) {
        if (paymentMethod === 'split') {
            ...
            if (paymentMethod === 'bank_transfer' && bankAccount) {
                ...
                await BankAccount.findOneAndUpdate({ _id: bankAccount, tenantId: req.tenantId }, { $inc: { currentBalance: actualReceived } }, { session });
            }
            await CashbankTransaction.create([{
                    type: 'in',
