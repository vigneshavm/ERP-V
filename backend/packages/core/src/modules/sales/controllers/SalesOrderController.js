...
const processSalesOrderItems = async (items, tenantId) => {
    ...
    for (const it of items) {
        ...
        if (!item)
            throw new AppError(`Item not found or unauthorized: ${it.item}`, 400);
...
export const createSalesOrder = asyncHandler(async (req, res) => {
    ...
    if (!customer)
        throw new AppError('Customer not found or unauthorized', 400);
...
export const updateSalesOrder = asyncHandler(async (req, res) => {
    ...
    if (!salesOrder)
        throw new AppError('Sales Order not found or unauthorized', 404);
    if (salesOrder.pricesLocked)
        throw new AppError('Cannot modify confirmed order. Prices and items are locked.', 400);
    if (salesOrder.isCancelled)
        throw new AppError('Cannot modify cancelled order', 400);
    const { customerId, items, orderDate, expectedDeliveryDate, discount = 0, notes = '' } = req.body;
    if (customerId) {
        validId(customerId, 'customer ID');
...
export const cancelSalesOrder = asyncHandler(async (req, res) => {
    ...
    if (!salesOrder)
        throw new AppError('Sales Order not found or unauthorized', 404);
    if (salesOrder.isCancelled)
        throw new AppError('Sales Order is already cancelled', 400);
    if (salesOrder.status === 'Invoiced')
        throw new AppError('Cannot cancel fully invoiced order', 400);
    if (salesOrder.isConfirmed) {
        for (const item of salesOrder.items) {
            const qty = item.reservedQty - item.deliveredQty;
            if (qty > 0)
                await releaseStock(item.item.toString(), qty, salesOrder._id.toString(), 'SalesOrder', userId);
        }
    }
...
export const convertToDeliveryChallan = asyncHandler(async (req, res) => {
    const tenantId = requireTenantId(req);
    const userId = requireUserId(req);
    ...
    if (!salesOrder)
        throw new AppError('Sales Order not found or unauthorized', 404);
    if (!salesOrder.isConfirmed)
        throw new AppError('Only confirmed orders can be converted to Delivery Challan', 400);
    if (salesOrder.isCancelled)
        throw new AppError('Cannot convert cancelled order', 400);
    if (salesOrder.status === 'Invoiced')
        throw new AppError('Order is already fully invoiced', 400);
    ...
    for (const dcItem of items) {
        ...
        if (!soItem)
            throw new AppError(`Item ${dcItem.item} not found in sales order`, 400);
        ...
        if (!item)
            throw new AppError(`Item not found: ${dcItem.item}`, 404);
    ...
    const totalDelivered = salesOrder.items.reduce((s, i) => s + i.deliveredQty, 0);
    const totalReserved = salesOrder.items.reduce((s, i) => s + i.reservedQty, 0);
...
export const convertToInvoice = asyncHandler(async (req, res) => {
    ...
    for (const invItem of items) {
        ...
        if (!soItem)
            throw new AppError(`Item ${invItem.item} not found in sales order`, 400);
        ...
        if (!item)
            throw new AppError(`Item not found: ${invItem.item}`, 404);
        const prev = { stockQty: item.stockQty, reservedStock: item.reservedStock, inTransitStock: item.inTransitStock || 0 };
    ...
    const totalInvoiced = salesOrder.items.reduce((s, i) => s + i.invoicedQty, 0);
    const totalDelivered = salesOrder.items.reduce((s, i) => s + i.deliveredQty, 0);
...
export const updateOrderStatus = asyncHandler(async (req, res) => {
    ...
    const order = await SalesOrder.findOneAndUpdate({ _id: req.params.id, tenantId }, { status }, { new: true });
    if (!order)
        throw new AppError('Sales Order not found', 404);
