    ...
    try {
        ...
        if (salesOrder.isConfirmed) {
            for (const item of salesOrder.items) {
                ...
                if (unreservedQty > 0) {
                    await releaseStock(item.item.toString(), unreservedQty, salesOrder._id.toString(), 'SalesOrder', req.user?._id);
                }
            }
        }
        // Update sales order status
...
export const convertToDeliveryChallan = async (req, res) => {
    try {
        ...
        for (const dcItem of items) {
            ...
            await logStockMovement(item, 'DELIVER', dcItem.quantity, salesOrder._id, 'SalesOrder', req.user?._id, previousState, { ...previousState, stockQty: item.stockQty });
            await logStockMovement(item, 'IN_TRANSIT', dcItem.quantity, salesOrder._id, 'SalesOrder', req.user?._id, { ...previousState, stockQty: item.stockQty }, newState);
        }
...
export const convertToInvoice = async (req, res) => {
    try {
        ...
        for (const invItem of items) {
            ...
            await logStockMovement(item, 'INVOICE', invItem.quantity, salesOrder._id, 'SalesOrder', req.user?._id, previousState, newState);
        }
...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        const order = await SalesOrder.findOneAndUpdate({ _id: id, tenantId: req.tenantId }, { status: status }, { new: true });
        if (!order) {
            res.status(404).json({ message: 'Sales Order not found' });
            return;
        }
        ...
        error(`Update Order Status failed: ${err.message}`);
        res.status(500).json({ message: 'Server Error', error: err.message });
    }
};
export default {
    ...
    convertToDeliveryChallan: convertToDeliveryChallan,
