...
import AuditLog from '@smarterp/core/modules/core/models/AuditLog.js';
import { error as logError } from '@smarterp/shared/utils/logger.js';
 ...
 */
export const mandatoryAudit = (entityType: string, action: string) => {
        ...
        res.json = function (data): Response {
            ...
            if (res.statusCode >= 200 && res.statusCode < 300) {
                try {
                    ...
                    const beforeSnapshot = (req as any).deletedEntity || (req as any).originalEntity || null;
 ...
 */
export const validateAuditCoverage = (app: any) => {
    const results = {
        valid: true,
        totalDestructive: 0,
        audited: 0,
 ...
 */
export const startAuditIntegrityVerification = (intervalMinutes: number = 60) => {
    ...
    const verify = async () => {
        try {
            ...
            // Explicitly cast AuditLog to any to access the static method if TS doesn't see it (or update model definition)
            const result = await (AuditLog as any).verifyIntegrity();
