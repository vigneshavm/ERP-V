...
import { warn, error as logError } from '@smarterp/shared/utils/logger.js';
 ...
 */
const extractRoutes = (app: any) => {
    const routes: any[] = [];

    const extractFromStack = (stack: any[], basePath: string = '') => {
        stack.forEach((layer) => {
            if (layer.route) {
                // Direct route
 ...
 */
const isProtected = (middlewares: string[]) => {
 ...
 */
export const validateAuthorizationCoverage = (app: any) => {
 ...
 */
export const enforceAuthorization = (req: Request, res: Response, next: NextFunction) => {
 ...
 * @param {Object} app - Express app instance
 */
export const printAuthorizationReport = (app: any) => {
