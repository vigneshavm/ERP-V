...
import { withTransaction } from '@smarterp/shared/utils/transaction.js';
 ...
 */
export const enforceTransaction = (operationName: string) => {
    return async (req: Request, res: Response, next: NextFunction) => {
        // Mark request as transaction-enforced
        (req as any).transactionEnforced = true;
        (req as any).operationName = operationName;
 ...
 */
export const validateTransactionEnforcement = (app: any) => {
    const results = {
        valid: true,
        missing: [] as string[],
        protected: [] as string[],
...
export const enableTransactionGuard = () => {
    ...
    mongoose.Model.prototype.save = async function (...args: any[]) {
        ...
        // Type guard to ensure this.constructor has modelName
        const modelName = (this.constructor as any).modelName;

        if (!session && modelName && CRITICAL_OPERATIONS.has(modelName)) {
            console.warn(
                `⚠️  TRANSACTION GUARD: ${modelName}.save() called without session. ` +
