...
export const runStartupChecks = async () => {
    ...
    try {
        ...
        logError('Environment check failed:', { error: err.message });
    }
    ...
    try {
        if (mongoose.connection.readyState === 1) {
            ...
            try {
                ...
                const { validateDatabaseSafety, validateIndexes } = await import('../utils/transaction.js');
        ...
        logError('Database check failed:', { error: err.message });
    }
    ...
    try {
        ...
        warn(`Email check failed: ${err.message}`);
    }
...
export const setupGracefulShutdown = (server) => {
    ...
    const shutdown = async (signal) => {
        ...
        try {
            ...
            logError('Error closing database:', { error: err.message });
        }
