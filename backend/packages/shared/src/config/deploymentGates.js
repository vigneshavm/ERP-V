...
export const runPreDeployChecks = async (app) => {
    ...
    try {
        ...
        try {
            const { validateDatabaseSafety } = await import('../utils/transaction.js');
    ...
    try {
        const { validateAuthorizationCoverage } = await import('../middlewares/authorizationEnforcement.js');
    ...
    // 4. Audit logging
    try {
        const { validateAuditLoggingEnabled } = await import('../middlewares/auditEnforcement.js');
    ...
    try {
        ...
        const { validateLoggingConfiguration } = await import('../middlewares/observabilityEnforcement.js');
