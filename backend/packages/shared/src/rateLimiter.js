...
export const handleLoginAttempt = async (req, success) => {
    ...
    if (success) {
        ...
        if (deviceKey)
            await resetRateLimit(deviceKey);
        ...
        if (accountKey)
            await incrementRateLimit(accountKey, LIMITS.ACCOUNT.windowMs);
        if (deviceKey)
            await incrementRateLimit(deviceKey, LIMITS.DEVICE.windowMs);
        // Log failed attempt
...
export const importLimiter = async (req, res, next) => {
