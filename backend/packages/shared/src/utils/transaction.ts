 ...
 */
export const withTransaction = async <T>(callback: (session: ClientSession) => Promise<T>, options: TransactionOptions = {}): Promise<T> => {
    const {
        maxRetries = 3,
        retryDelay = 1000,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            session.startTransaction({
                readConcern: { level: readConcern as any },
                writeConcern: { w: writeConcern as any },
 ...
 */
export const requireTransaction = (session: ClientSession | null, operationName: string) => {
 ...
 */
export const updateWithOptimisticLock = async <T>(Model: Model<T>, id: string, expectedVersion: number, update: any, session: ClientSession) => {
 ...
 */
export const withIdempotency = async <T>(key: string | undefined, operation: () => Promise<T>): Promise<T> => {
    if (!key) {
 ...
 */
export const validateIndexes = async (models: Model<any>[]) => {
