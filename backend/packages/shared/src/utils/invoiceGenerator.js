...
const buildPaymentMethodDisplay = (invoiceData) => {
    ...
    if (credit > 0 && paid > 0)
        return `Split (${primaryMethod} + Customer Credit)`;
...
export const generateInvoicePDF = async (invoiceData) => {
    ...
    doc.text(invoiceDate.toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }), pageWidth - margin, 25, { align: "right" });
    doc.setFontSize(9);
    doc.setTextColor(...grayColor);
    doc.text(invoiceDate.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: true }).toLowerCase(), pageWidth - margin, 30, { align: "right" });
    // Horizontal line
