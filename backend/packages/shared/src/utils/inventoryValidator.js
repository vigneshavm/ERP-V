...
export const validateStockLevels = (item) => {
    ...
    if (item.reservedStock > (item.stockQty + (item.inTransitStock || 0))) {
        throw new Error(`reservedStock (${item.reservedStock}) cannot exceed stockQty + inTransitStock (${item.stockQty + (item.inTransitStock || 0)}) for item: ${item.name}`);
    }
...
export const validateSalesOrderQuantities = (salesOrder) => {
    for (const item of salesOrder.items) {
        if (item.deliveredQty > item.reservedQty) {
            throw new Error(`deliveredQty (${item.deliveredQty}) cannot exceed reservedQty (${item.reservedQty})`);
        }
        if (item.invoicedQty > item.deliveredQty) {
            throw new Error(`invoicedQty (${item.invoicedQty}) cannot exceed deliveredQty (${item.deliveredQty})`);
        }
