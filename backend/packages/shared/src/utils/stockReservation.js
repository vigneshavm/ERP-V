...
export const reserveStock = async (itemId, quantity, sourceId = null, sourceType = "SalesOrder", userId = null) => {
    ...
    if (availableStock < quantity) {
        throw new Error(`Insufficient available stock for ${item.name}. Available: ${availableStock}, Requested: ${quantity}`);
    }
    ...
    if (sourceId && userId) {
        await logStockMovement(item, "RESERVE", quantity, sourceId, sourceType, userId, previousState, newState);
    }
};
/**
...
export const releaseStock = async (itemId, quantity, sourceId = null, sourceType = "SalesOrder", userId = null) => {
    ...
    if (item.reservedStock < quantity) {
        throw new Error(`Cannot release more stock than reserved for ${item.name}. Reserved: ${item.reservedStock}, Requested: ${quantity}`);
    }
    // Capture previous state
    const previousState = {
        stockQty: item.stockQty,
    ...
    // Log stock movement if source info provided
    if (sourceId && userId) {
        await logStockMovement(item, "RELEASE", quantity, sourceId, sourceType, userId, previousState, newState);
    }
};
/**
