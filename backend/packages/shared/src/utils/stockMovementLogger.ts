import StockMovement from '@smarterp/core/modules/core/models/StockMovement.js';
...
export const logStockMovement = async (
    ...
    previousState: { stockQty: number; reservedStock: number; inTransitStock: number },
    newState: { stockQty: number; reservedStock: number; inTransitStock: number }
) => {
    try {
 ...
 */
export const getStockMovementHistory = async (itemId: string, limit = 50) => {
    try {
