...
export const withTransaction = async (callback, options = {}) => {
    const { maxRetries = 3, retryDelay = 1000, writeConcern = 'majority', // Enterprise default
...
export const requireTransaction = (session, operationName) => {
    if (!session || !session.inTransaction()) {
        throw new Error(`TRANSACTION_REQUIRED: ${operationName} must be executed within a transaction. ` +
...
export const updateWithOptimisticLock = async (Model, id, expectedVersion, update, session) => {
    const result = await Model.findOneAndUpdate({ _id: id, __v: expectedVersion }, { ...update, $inc: { __v: 1 } }, { new: true, session });
    if (!result) {
        throw new Error(`OPTIMISTIC_LOCK_FAILED: Document ${id} was modified by another operation. ` +
...
export const validateDatabaseSafety = async () => {
    try {
        ...
        if (!serverStatus.repl) {
            warn('DATABASE SAFETY WARNING: Not running in replica set mode. ' +
