...
export class InventoryService {
    ...
    async updateItem(itemId: string, userId: string, updateData: any, userName: string, originalEntity?: any): Promise<{ updated: IItem, alerts: any[] }> {
        const item = await this.inventoryRepository.findById(itemId, userId);
        if (!item) {
            throw new AppError("Item not found or unauthorized", 404);
        }
    ...
    async importItems(items: any[], userId: string, userName: string): Promise<any> {
