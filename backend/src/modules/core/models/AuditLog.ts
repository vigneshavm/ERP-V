...
interface IAuditLogModel extends Model<IAuditLog> {
    verifyIntegrity(): Promise<{ verified: boolean; totalLogs: number; errors: any[] }>;
    exportLogs(filter?: any, options?: any): Promise<any[]>;
}
...
auditLogSchema.pre('save', async function (next) {
    if (this.isNew) {
        ...
        const previousLog = await (this.constructor as any).findOne().sort({ createdAt: -1 });
 ...
 */
auditLogSchema.statics.exportLogs = async function (filter = {}, options: any = {}) {
    const {
        startDate,
        endDate,
        userId,
