...
import { validateStockLevels, validateSalesOrderQuantities as _validateSalesOrderQuantities } from '../../../utils/inventoryValidator.js';
 ...
 */
export const createDeliveryChallan = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (!customerId || !mongoose.Types.ObjectId.isValid(customerId)) {
            res.status(400).json({ message: 'Valid customer is required' });
            return;
        }
        ...
        const validatedItems: ChallanItem[] = [];
        for (const item of items) {
            ...
            if (deliveredQty > inventoryItem.stockQty) {
                res.status(400).json({
                    message: `Insufficient stock for ${inventoryItem.name}. Available: ${inventoryItem.stockQty}, Requested: ${deliveredQty}`,
                });
                return;
            }
            ...
            if (salesOrder) {
                ...
                for (const challanItem of validatedItems) {
                    const soItem = salesOrder.items.find(
                        (i: any) => i.item.toString() === challanItem.item.toString()
                    );
                    if (soItem) {
                    ...
                    if (!item) {
                        throw new Error(`Item not found: ${challanItem.item}`);
                    }
                ...
                const totalOrdered = salesOrder.items.reduce((sum: number, item: any) => sum + item.quantity, 0);
        ...
        error(`Create Delivery Challan Error: ${(err as Error).message}`);
 ...
 * /api/delivery-challan:
 *   get:
 *     summary: Get all delivery challans
 *     tags: [Sales - Deliveries]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: List of delivery challans retrieved
 */
export const getAllDeliveryChallans = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        error(`Get All Delivery Challans Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};
...
/**
 * @swagger
 * /api/delivery-challan/{id}:
 *   get:
 *     summary: Get single delivery challan
 *     tags: [Sales - Deliveries]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Delivery challan details retrieved
 */
export const getDeliveryChallanById = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        error(`Get Delivery Challan By ID Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};
 ...
 */
export const updateDeliveryChallan = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        if (!mongoose.Types.ObjectId.isValid(req.params.id as string)) {
            res.status(400).json({ message: 'Invalid challan ID' });
            return;
        ...
        error(`Update Delivery Challan Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};
...
export const convertToInvoice = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        const invoiceItems = challan.items.map((challanItem: any) => {
            ...
            if (salesOrder) {
                const soItem = salesOrder.items.find(
                    (i: any) => i.item.toString() === challanItem.item._id.toString()
                );
                if (soItem) {
        ...
        const subtotal = invoiceItems.reduce((sum: number, item: any) => sum + (item.quantity * item.price), 0);
        const taxTotal = invoiceItems.reduce((sum: number, item: any) => sum + (item.tax || 0), 0);
        const discountTotal = invoiceItems.reduce((sum: number, item: any) => sum + (item.discount || 0), 0);
        ...
        // Update Sales Order if challan was created from one
        if (challan.salesOrder) {
            const salesOrderForUpdate = await SalesOrder.findById(challan.salesOrder);
            if (salesOrderForUpdate) {
                // Update invoicedQty and release reserved stock for each item
                for (const challanItem of challan.items) {
                    const soItem = salesOrderForUpdate.items.find(
                        (i: any) => i.item.toString() === challanItem.item._id.toString()
                    );
                    if (soItem) {
                        ...
                        if (!item) {
                            throw new Error(`Item not found: ${challanItem.item._id}`);
                        }
                ...
                salesOrderForUpdate.invoices.push(invoice._id);

                // Update sales order status
                const totalInvoiced = salesOrderForUpdate.items.reduce((sum: number, item: any) => sum + item.invoicedQty, 0);
                const totalDelivered = salesOrderForUpdate.items.reduce((sum: number, item: any) => sum + item.deliveredQty, 0);

                if (totalInvoiced >= totalDelivered) {
                    salesOrderForUpdate.status = 'Invoiced';
                } else if (totalInvoiced > 0) {
                    salesOrderForUpdate.status = 'Partially Invoiced';
                }

                await salesOrderForUpdate.save();
            }
        }
        ...
        error(`Convert to Invoice Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};
 ...
 */
export const deleteDeliveryChallan = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        if (!mongoose.Types.ObjectId.isValid(req.params.id as string)) {
            res.status(400).json({ message: 'Invalid challan ID' });
            return;
        ...
        if (challan.salesOrder) {
            ...
            if (salesOrder) {
                ...
                salesOrder.deliveryChallans = salesOrder.deliveryChallans.filter(
                    (dc: any) => dc.toString() !== challan._id.toString()
                );
        ...
        error(`Delete Delivery Challan Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};
