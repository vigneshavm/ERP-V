...
export const receiveSms = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        error(`Receive SMS Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};

export const getSmsTransactions = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        error(`Get SMS Transactions Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};

export const convertToExpense = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        smsTxn.expenseId = expense[0]._id as mongoose.Types.ObjectId;
        ...
        error(`Convert SMS to Expense Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    } finally {
        session.endSession();
    }
...
export const ignoreSms = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const smsTxn = await SmsTransaction.findOneAndUpdate(
        ...
        error(`Ignore SMS Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};
