import { Request, Response } from 'express';
import mongoose from 'mongoose';

import DueAdjustment from '../models/DueAdjustment.js';

import Customer from '../../crm/models/Customer.js';
import Transaction from '../../sales/models/Transaction.js';
import { info, error } from '../../../config/logger.js';

/**
 * Request interface with authenticated user
 */
interface AuthenticatedRequest extends Request {
    user?: {
        _id: string;
        name?: string;
        [key: string]: any;
    };
}

/**
 * @swagger
 * /api/due/adjust:
 *   post:
 *     summary: Create a new due adjustment
 *     tags: [Finance - Dues]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             type: object
 *             required: [customerId, adjustmentAmount, adjustmentMethod]
 *             properties:
 *               customerId: { type: string }
 *               adjustmentAmount: { type: number }
 *               adjustmentMethod: { type: string, enum: [cash, bank, credit] }
 *               notes: { type: string }
 *     responses:
 *       201:
 *         description: Due adjustment created successfully
 */
export const createDueAdjustment = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { customerId, adjustmentAmount, adjustmentMethod, notes = '' } = req.body;

        // Validate input
        if (!customerId || !adjustmentAmount || !adjustmentMethod) {
            res.status(400).json({
                message: 'Customer ID, adjustment amount, and adjustment method are required',
            });
            return;
        }

        // Validate ObjectId format
        if (!mongoose.Types.ObjectId.isValid(customerId as string)) {
            res.status(400).json({ message: 'Invalid customer ID format' });
            return;
        }

        // Validate adjustment amount
        if (adjustmentAmount <= 0) {
            res.status(400).json({
                message: 'Adjustment amount must be greater than zero',
            });
            return;
        }

        // Validate adjustment method
        const validMethods = ['cash', 'bank', 'credit', 'original_payment'];
        if (!validMethods.includes(adjustmentMethod)) {
            res.status(400).json({
                message: 'Invalid adjustment method',
            });
            return;
        }

        // Fetch customer and verify ownership
        const customer = await Customer.findOne({
            _id: customerId,
            owner: req.user?._id,
        });

        if (!customer) {
            res.status(404).json({
                message: 'Customer not found or unauthorized',
            });
            return;
        }

        // Validate adjustment amount doesn't exceed outstanding due
        if (adjustmentAmount > customer.dues) {
            res.status(400).json({
                message: `Adjustment amount (₹${adjustmentAmount}) cannot exceed outstanding due (₹${customer.dues})`,
            });
            return;
        }

        // Calculate updated due
        const previousDue = customer.dues;
        const updatedDue = previousDue - adjustmentAmount;

        // Ensure no negative balance
        if (updatedDue < 0) {
            res.status(400).json({
                message: 'Adjustment would result in negative balance',
            });
            return;
        }

        // Create due adjustment record
        const dueAdjustment = await DueAdjustment.create({
            customer: customerId,
            relatedInvoice: null,
            adjustmentAmount,
            adjustmentMethod,
            previousDue,
            updatedDue,
            notes,
            createdBy: req.user?._id,
        });

        // Update customer dues
        await Customer.findByIdAndUpdate(customerId, {
            $inc: { dues: -adjustmentAmount },
        });

        // Create transaction record
        await Transaction.create({
            type: 'due_adjustment',
            customer: customerId,
            dueAdjustment: dueAdjustment._id,
            amount: adjustmentAmount,
            paymentMethod: adjustmentMethod,
            description: `Due adjustment of ₹${adjustmentAmount} via ${adjustmentMethod}${notes ? ` - ${notes}` : ''}`,
        });

        info(
            `Due adjustment created by ${req.user?.name}: ₹${adjustmentAmount} for customer ${customer.name}`
        );

        // Populate and return the created adjustment
        const populatedAdjustment = await DueAdjustment.findById(dueAdjustment._id)
            .populate('customer', 'name phone email dues')
            .populate('createdBy', 'name email');

        res.status(201).json({
            message: 'Due adjustment created successfully',
            adjustment: populatedAdjustment,
        });
    } catch (err) {
        error(`Create Due Adjustment Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};

/**
 * @swagger
 * /api/due/adjustments:
 *   get:
 *     summary: Get all due adjustments
 *     tags: [Finance - Dues]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: List of due adjustments retrieved
 */
export const getDueAdjustments = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const adjustments = await DueAdjustment.find({ createdBy: req.user?._id })
            .populate('customer', 'name phone email')
            .populate('createdBy', 'name email')
            .sort({ createdAt: -1 });

        res.status(200).json(adjustments);
    } catch (err) {
        error(`Get All Due Adjustments Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};

/**
 * @desc Get due adjustments for a specific customer
 * @route GET /api/due/customer/:customerId/adjustments
 */
export const getCustomerDueAdjustments = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { customerId } = req.params;

        // Validate ObjectId format
        if (!mongoose.Types.ObjectId.isValid(customerId as string)) {
            res.status(400).json({ message: 'Invalid customer ID format' });
            return;
        }

        // Verify customer belongs to current user
        const customer = await Customer.findOne({
            _id: customerId,
            owner: req.user?._id,
        });

        if (!customer) {
            res.status(404).json({
                message: 'Customer not found or unauthorized',
            });
            return;
        }

        const adjustments = await DueAdjustment.find({
            customer: customerId,
            createdBy: req.user?._id,
        })
            .populate('customer', 'name phone email')
            .populate('createdBy', 'name email')
            .sort({ createdAt: -1 });

        res.status(200).json({
            customer: { name: customer.name, phone: customer.phone },
            adjustments,
        });
    } catch (err) {
        error(`Get Customer Due Adjustments Error: ${(err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};

// Legacy functions (keeping for backward compatibility)
export const addDue = async (_req: Request, res: Response): Promise<void> => {
    res.status(501).json({ message: 'Not implemented - use createDueAdjustment instead' });
};

export const getAllDues = async (_req: Request, res: Response): Promise<void> => {
    res.status(501).json({ message: 'Not implemented - use getDueAdjustments instead' });
};

export const getCustomerDues = async (_req: Request, res: Response): Promise<void> => {
    res.status(501).json({ message: 'Not implemented - use getCustomerDueAdjustments instead' });
};

export const updateDue = async (_req: Request, res: Response): Promise<void> => {
    res.status(501).json({ message: 'Not implemented' });
};

export const clearDue = async (_req: Request, res: Response): Promise<void> => {
    res.status(501).json({ message: 'Not implemented' });
};

export default {
    createDueAdjustment,
    getDueAdjustments,
    getCustomerDueAdjustments,
    addDue,
    getAllDues,
    getCustomerDues,
    updateDue,
    clearDue,
};
