...
export interface IGSTReconciliation extends Document {
    tenantId: string;
    branchId?: string;
    ...
    importedRecords: {
        ...
        taxAmount: number;
        source: 'GSTR-2B' | 'MANUAL';
    }[];

    // Reconciliation Results
    results: {
        billId?: mongoose.Types.ObjectId; // System Bill
        gstrReference?: string; // Invoice Number from GSTR
        status: 'MATCHED' | 'MISMATCH' | 'MISSING_IN_GSTR2B' | 'MISSING_IN_SYSTEM';
        remarks?: string;
        reconciledAt: Date;
    }[];
    ...
    stats: {
        ...
        totalMissingSystem: number;
        totalMissingGSTR: number;
    };
...
const gstReconciliationSchema = new Schema<IGSTReconciliation>(
    {
...
const GSTReconciliation = mongoose.model<IGSTReconciliation>("GSTReconciliation", gstReconciliationSchema);
