...
const buildPaymentMethodDisplay = (invoiceData: any): string => {
    ...
    if (splitDetails.length > 1) {
        const methods = splitDetails.map((d: any) => formatPaymentMethodLabel(d.method)).join(" + ");
        if (credit > 0) {
    ...
    if (hasCreditPayment) {
        ...
        if ((invoiceData.paidAmount || 0) > 0) {
            ...
            if (splitDetails.length > 0) {
                ...
                splitDetails.forEach((split: any) => {
                    yPos += 6;
