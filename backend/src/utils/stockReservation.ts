 ...
 */
export const reserveStock = async (itemId: string, quantity: number, sourceId: string | null = null, sourceType: string = "SalesOrder", userId: any = null) => {
 ...
 * @param {ObjectId} userId - User performing the action
 */
export const releaseStock = async (itemId: string, quantity: number, sourceId: string | null = null, sourceType: string = "SalesOrder", userId: any = null) => {
    const item = await Item.findById(itemId);
    if (!item) {
        throw new Error(`Item not found: ${itemId}`);
    }
 ...
 */
export const checkAvailableStock = async (itemId: string, quantity: number) => {
    const item = await Item.findById(itemId);
    if (!item) {
        throw new Error(`Item not found: ${itemId}`);
    }
 ...
 */
export const getAvailableStock = async (itemId: string) => {
    const item = await Item.findById(itemId);
    if (!item) {
        throw new Error(`Item not found: ${itemId}`);
    }
