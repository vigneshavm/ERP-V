...
// Variable to extract bankAccount from scope
...
export const createInvoice = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        if (!items || items.length === 0) {
            res.status(400).json({ message: 'No items in invoice' });
            return;
        }
        ...
        for (const it of items as InvoiceItem[]) {
            ...
            if (!item) {
                error(`Item ${it.item} not found during stock update`);
                continue;
            }
        ...
        if (actualPaidAmount > 0) {
            ...
            if (paymentMethod === 'bank_transfer' && bankAccount) {
                ...
                if (!bankAcc) {
                    ...
                    res.status(400).json({ message: 'Selected bank account not found' });
                    return;
                }
        ...
        const customerData = populatedInvoice.customer as any;
        ...
        error(`Invoice creation failed: ${(err as Error).message}`);
 ...
 */
export const markInvoiceAsPaid = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        ...
        let paymentStatus: "partial" | "paid" | "unpaid" = 'unpaid';
        ...
        if (invoice.customer) {
            ...
            await Transaction.create({
                type: 'payment',
                customer: (invoice.customer as any)._id,
                invoice: invoice._id,
                amount: paidAmount,
                paymentMethod,
        ...
        error(`Mark invoice as paid failed: ${(err as Error).stack || (err as Error).message}`);
        res.status(500).json({ message: 'Server Error', error: (err as Error).message });
    }
};
