...
export const getExpenseReport = async (req, res) => {
    try {
        ...
        for (const exp of expenses) {
            ...
            categoryMap.get(cat).amount += exp.amount || 0;
        ...
        for (const exp of expenses) {
            ...
            paymentMap.set(method, paymentMap.get(method) + (exp.amount || 0));
        }
        ...
        // Branch analysis (placeholder - would need branch data in production)
        const by_branch = [
        ...
        error(`Get expense report failed: ${err.message}`);
