 ...
 */
const calculateIntelligence = async (recurringExpenses, userId) => {
    const now = new Date();
    ...
    upcomingDues.sort((a, b) => new Date(a.next_due_date) - new Date(b.next_due_date));
...
export const createRecurringExpense = async (req, res) => {
    try {
        ...
        if (!category || !amount || !next_due) {
            return res.status(400).json({ message: "Category, amount, and next due date are required" });
        }
...
export const updateRecurringExpense = async (req, res) => {
    try {
        ...
        if (!expense) {
            return res.status(404).json({ message: "Recurring expense not found or unauthorized" });
        }
