...
const FundTransferIntelligence: React.FC = () => {
                ...
                <div className="lg:col-span-2 space-y-4">
                                    ...
                                    <div key={branch.id} className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-3xl p-6 shadow-sm group hover:border-primary/30 transition-all hover:scale-[1.01]">
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-neutral-400">
                                                ...
                                                <span className={branch.health === 'CRITICAL' ? 'text-error' : 'text-neutral-700'}>
                                        ...
                                        <div className="mt-6 pt-4 border-t border-neutral-100 dark:border-neutral-800 flex items-center gap-2">
                                            <button className="flex-1 py-2 rounded-xl bg-neutral-100 dark:bg-neutral-900 text-[10px] font-black uppercase tracking-widest hover:bg-primary hover:text-white transition-all">
                                                Initiate Outflow
                                            </button>
                                            <button className="p-2 rounded-xl border border-neutral-100 dark:border-neutral-800 hover:bg-neutral-50">
                    ...
                    {viewMode === 'TRANSFERS' && (
                        <div className="bg-white dark:bg-neutral-800 rounded-3xl border border-neutral-200 dark:border-neutral-700 overflow-hidden shadow-sm">
                                        ...
                                        <tr key={tx.id} className="hover:bg-neutral-50 transition-colors group">
                                            <td className="p-5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-[10px] text-neutral-400 mt-0.5">ID: {tx.id} • {tx.initiated_by}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-5">
                                            ...
                                            <td className="p-5">
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${tx.status === 'COMPLETED' ? 'bg-success/10 text-success' :
                                                        tx.status === 'INITIATED' ? 'bg-primary/10 text-primary' : 'bg-error text-white'
                                                    }`}>
                                                    {tx.status}
                                                </span>
                                            </td>
                            ...
                            <button className="w-full py-4 bg-neutral-50 text-[10px] font-black text-neutral-400 uppercase tracking-widest hover:text-primary border-t border-neutral-100">
                    ...
                    {viewMode === 'ADVISOR' && (
                        <div className="bg-neutral-900 text-white rounded-[2.5rem] p-10 border border-neutral-800 shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10 max-w-xl">
                                <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/20 border border-primary/30 rounded-full text-primary-light text-[10px] font-black uppercase tracking-[0.2em] mb-8 animate-pulse">
                                ...
                                <div className="space-y-6 mb-10">
                                    <div className="flex gap-5 p-6 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-md">
                                        <div className="p-4 bg-error/20 text-error rounded-2xl h-fit">
                                            <AlertCircle className="w-6 h-6" />
                                        </div>
                                        <div>
                                            <h4 className="font-black text-lg text-white mb-2">Critical Node: Coimbatore Store</h4>
                                            <p className="text-sm text-neutral-400 leading-relaxed font-medium">
                                                Available funds (₹80k) will only last <span className="text-error font-black italic">2 more days</span> based on avg daily overhead.
                                    ...
                                    <div className="flex gap-5 p-6 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-md">
                                        <div className="p-4 bg-success/20 text-success rounded-2xl h-fit">
                                            <TrendingUp className="w-6 h-6" />
                                        </div>
                                        <div>
                                            <h4 className="font-black text-lg text-white mb-2">Optimal Source: Chennai Main</h4>
                                            <p className="text-sm text-neutral-400 leading-relaxed font-medium">
                                                चेन्नई Main holds ₹30.5L (Excess Liquidity). Moving ₹5L to Coimbatore and ₹2L to Trichy will normalize network stress without impacting Chennai’s performance.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <button className="w-full py-4 bg-primary text-white rounded-2xl font-black text-sm uppercase tracking-[0.1em] shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3">
                                    Apply Multi-Node Rebalancing <Zap className="w-4 h-4 fill-current" />
                                </button>
                            </div>
                        </div>
                    )}
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-neutral-800 rounded-3xl border border-neutral-200 dark:border-neutral-700 p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <span className="px-2 py-0.5 bg-success/10 text-success rounded-full text-[9px] font-black uppercase">Active</span>
                        </div>
                        ...
                        <div className="space-y-3">
                            {[
                                ...
                                <div key={i} className="flex items-center justify-between p-3 bg-neutral-50 dark:bg-neutral-900 rounded-2xl">
                                    <span className="text-xs font-bold text-neutral-700 dark:text-neutral-300">{item.name}</span>
                                    {item.status ? <Unlock className="w-3.5 h-3.5 text-success" /> : <Lock className="w-3.5 h-3.5 text-neutral-300" />}
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-neutral-100 dark:bg-neutral-900 p-6 rounded-3xl border border-neutral-200 dark:border-neutral-800 relative overflow-hidden group">
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-white dark:bg-neutral-800 rounded-2xl shadow-sm border border-error/20">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-[9px] font-black uppercase text-error tracking-widest">Abuse Detected</span>
                                    <Clock className="w-3 h-3 text-neutral-400" />
                                </div>
                                <p className="text-[11px] font-bold leading-relaxed">
                                    Possible <span className="text-error">Circular Transfer</span> at Chennai Main (FT-105). System blocked movement automatically.
                                </p>
                            </div>

                            <div className="p-4 bg-white dark:bg-neutral-800 rounded-2xl shadow-sm border border-amber-200">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-[9px] font-black uppercase text-amber-600 tracking-widest">Anomalous Time</span>
                                    <Clock className="w-3 h-3 text-neutral-400" />
                                </div>
                                <p className="text-[11px] font-bold leading-relaxed">
                                    High value transfer initiated at <span className="text-amber-600">3:00 AM IST</span>. Manual intervention required.
                                </p>
                            </div>
                        </div>
                    </div>
...
const Smartphone = (props: React.ComponentProps<typeof Info>) => (
...
export default FundTransferIntelligence;
