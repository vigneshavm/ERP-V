...
const PettyCashIntelligence: React.FC = () => {
        ...
        <div className="space-y-6 animate-fade-in text-neutral-900 dark:text-neutral-100 pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-neutral-500 mt-0.5">
                        Operational spend control and leak detection for <span className="font-bold text-primary">{tenant_id}</span>
                    </p>
                </div>
                <div className="flex gap-2">
                    <button className="px-4 py-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-neutral-50">
                        <Download className="w-4 h-4" /> Export Audit
                    </button>
                    <button className="px-4 py-2 bg-primary text-white rounded-xl text-sm font-black shadow-lg shadow-primary/20 flex items-center gap-2 hover:bg-primary/90">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white dark:bg-neutral-800 p-5 rounded-2xl border border-neutral-200 dark:border-neutral-700 shadow-sm relative overflow-hidden group">
                    ...
                    <h3 className="text-2xl font-black">{metrics.activeFunds} <span className="text-xs text-neutral-400 font-bold">across branches</span></h3>
                    <p className="text-xs text-neutral-500 mt-2 font-medium">All custodians active</p>
                </div>
                ...
                <div className="bg-white dark:bg-neutral-800 p-5 rounded-2xl border border-neutral-200 dark:border-neutral-700 shadow-sm relative overflow-hidden group">
                    <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-1">Agent Strategy</p>
            ...
            <div className="flex border-b border-neutral-100 dark:border-neutral-800 pb-0.5 mt-8 gap-8">
                ...
                >
                    ...
                    <span className="ml-2 px-1.5 py-0.5 bg-error text-white text-[8px] rounded uppercase">2 High Risk</span>
                ...
                <div className="lg:col-span-2 space-y-4">
                        ...
                        <>
                            ...
                            {filteredFunds.map(fund => (
                                <div key={fund.id} className={`group bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-2xl p-5 shadow-sm transition-all hover:scale-[1.005] hover:shadow-md ${fund.status === 'LOW_BALANCE' ? 'border-l-4 border-l-amber-500' :
                                        fund.status === 'AUDIT_REQUIRED' ? 'border-l-4 border-l-error' : ''
                                    }`}>
                                        ...
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-[10px] font-black text-neutral-400 tracking-widest">{fund.branch}</span>
                                                    <span className="w-1 h-1 bg-neutral-300 rounded-full" />
                                                    <span className="text-[10px] font-bold text-primary uppercase">ID: {fund.id}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-neutral-400 text-[10px] font-black uppercase tracking-widest mb-1">Fund Liquidity</div>
                                            <div className={`text-xl font-black ${fund.status === 'LOW_BALANCE' ? 'text-amber-600' : 'text-neutral-900 dark:text-white'}`}>
                                                ₹{fund.current_balance.toLocaleString()}
                                            </div>
                                            <div className="flex items-center justify-end gap-1.5 mt-1">
                                                <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${fund.status === 'HEALTHY' ? 'bg-success/10 text-success' :
                                    ...
                                    <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-neutral-50 dark:bg-neutral-900/50 rounded-xl group/inner">
                                        <div>
                                            <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-0.5">Burn Rate</p>
                                            <p className="text-sm font-bold">₹{fund.burn_rate_daily}/day</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-0.5">Est. Runway</p>
                                            <p className="text-sm font-bold text-amber-600">{Math.floor(fund.current_balance / fund.burn_rate_daily)} Days left</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-0.5">Daily Limit</p>
                                            <p className="text-sm font-bold">₹{fund.daily_limit.toLocaleString()}</p>
                                        </div>
                                        <div>
                                    ...
                                    {fund.status === 'LOW_BALANCE' && (
                                        <div className="mt-4 p-3 bg-primary/5 border border-primary/10 rounded-xl flex items-center justify-between animate-pulse">
                                            ...
                                            <button className="px-3 py-1 bg-primary text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-lg shadow-primary/20">
                                                Trigger Now
                                            </button>
                                        </div>
                                    )}
                    ...
                    {viewMode === 'LEDGER' && (
                        <div className="bg-white dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 overflow-hidden shadow-sm">
                                ...
                                <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                                    {recentTransactions.map(tx => (
                                        <tr key={tx.id} className="hover:bg-neutral-50 transition-colors">
                                            <td className="p-4">
                                                <div className="font-bold text-neutral-900 dark:text-white">{tx.date.split(' ')[1]}</div>
                                                <div className="text-neutral-400">{tx.date.split(' ')[0]}</div>
                                            </td>
                                            <td className="p-4">
                                                <div className="font-medium text-neutral-700 dark:text-neutral-300">{tx.description}</div>
                                                <div className="text-[9px] text-neutral-400 mt-0.5">ID: {tx.id} • {tx.custodian}</div>
                                            </td>
                                            <td className="p-4">
                                                <span className="px-2 py-0.5 bg-neutral-100 dark:bg-neutral-900 rounded-full font-bold">
                                                    {tx.category}
                                                </span>
                                            </td>
                                            <td className="p-4">
                                                <div className={`text-sm font-black ${tx.type === 'IN' ? 'text-success' : 'text-error'}`}>
                                            ...
                                            <td className="p-4">
                                                {tx.violation ? (
                                                    <div className="flex items-center gap-1.5 text-error font-bold">
                                                        ...
                                                        <span className="text-[10px] break-all max-w-[100px] leading-tight underline decoration-error/30">
                            ...
                            <button className="w-full py-3 bg-neutral-50 border-t border-neutral-100 text-[10px] font-black text-neutral-500 uppercase tracking-widest hover:text-primary">
                    ...
                    {viewMode === 'AUDIT' && (
                        <div className="bg-neutral-900 text-white rounded-2xl p-8 border border-neutral-800 shadow-2xl relative overflow-hidden group">
                            <ShieldAlert className="absolute -bottom-10 -right-10 w-48 h-48 text-primary/10" />
                            <div className="relative z-10">
                                <div className="inline-flex items-center gap-2 px-3 py-1 bg-error/20 border border-error/30 rounded-full text-error text-[10px] font-black uppercase tracking-[0.2em] mb-6 animate-pulse">
                                ...
                                <div className="space-y-4">
                                    {alerts.map((alert, i) => (
                                        <div key={i} className="flex gap-4 p-4 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-sm group/alert hover:bg-white/10 transition-all cursor-pointer">
                                            <div className={`p-3 rounded-xl h-fit ${alert.risk_level === 'HIGH' ? 'bg-error/20 text-error' : 'bg-warning/20 text-warning'
                                                }`}>
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${alert.risk_level === 'HIGH' ? 'bg-error text-white' : 'bg-warning text-neutral-900'
                                                        }`}>
                                                        {alert.risk_level} Risk
                                                    </span>
                                                </div>
                                                <p className="font-bold text-sm mt-1">{alert.issue}</p>
                                                <div className="mt-3 flex items-center justify-between">
                                                    <span className="text-[9px] text-neutral-400 flex items-center gap-1">
                                                        <Clock className="w-3 h-3" /> Detected 2h ago
                                                    </span>
                                                    <button className="text-[10px] font-black text-primary uppercase group-hover/alert:translate-x-1 transition-transform">
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-white dark:bg-neutral-800 rounded-2xl border border-neutral-200 dark:border-neutral-700 p-6 shadow-sm">
                        ...
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-bold">Receipts Attached</span>
                            <span className="text-xs font-black text-primary">82%</span>
                        </div>
                        <div className="w-full bg-neutral-100 dark:bg-neutral-900 h-2 rounded-full overflow-hidden mb-8">
                        ...
                        <div className="space-y-3">
                            {[
                                ...
                                { name: 'Office Repairs', spend: 4500, limit: 4000, color: 'bg-error' }
                            ].map((cat, i) => (
                                <div key={i}>
                                    <div className="flex justify-between text-[10px] mb-1">
                                        <span className="font-bold">{cat.name}</span>
                                        <span className="text-neutral-400">₹{cat.spend}/₹{cat.limit}</span>
                                    </div>
                                    <div className="w-full bg-neutral-100 dark:bg-neutral-900 h-1.5 rounded-full overflow-hidden">
                                        <div className={`${cat.color} h-full rounded-full`} style={{ width: `${Math.min((cat.spend / cat.limit) * 100, 100)}%` }} />
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-primary text-white p-6 rounded-3xl shadow-xl shadow-primary/20 relative overflow-hidden group">
                        <Zap className="absolute -top-6 -right-6 w-32 h-32 opacity-10 group-hover:scale-110 transition-transform duration-700" />
                        <div className="relative z-10">
                            <h4 className="font-black text-lg mb-2">Automate Cash-to-Petty?</h4>
                            <p className="text-xs opacity-90 leading-relaxed mb-6">
                                The Agent predicts that Chennai fund will be exhausted in <span className="font-black underline">4 days</span>.
                                Set up auto-replenishment to avoid operational delays.
                            </p>
                            <button className="w-full py-3 bg-white text-primary font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-neutral-50 transition-colors shadow-lg shadow-black/5">
                                Enable Autoflow Rebuild
                            </button>
                        </div>
                    </div>

                    <div className="p-4 rounded-2xl border border-neutral-100 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50 flex items-start gap-3">
...
export default PettyCashIntelligence;
