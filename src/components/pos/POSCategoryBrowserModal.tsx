                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                            {filteredProducts.map(product => (
                                <button
                                    ...
                                    className={`text-left p-4 bg-neutral-50 dark:bg-neutral-800/50 hover:bg-white dark:hover:bg-neutral-800 border-2 border-transparent hover:border-primary rounded-2xl transition-all flex flex-col gap-2 ${product.stock <= 0 ? 'opacity-50 grayscale' : ''}`}
                                >
                                    <div className="flex justify-between items-start">
                                        <h4 className="font-bold text-neutral-800 dark:text-neutral-100 line-clamp-1">{product.name}</h4>
                                        <span className="bg-primary/10 dark:bg-primary/20 text-primary dark:text-primary-light px-2 py-0.5 rounded text-[10px] font-black">₹{product.price}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-[11px] font-medium text-neutral-500">
                                        <span>SKU: {product.sku}</span>
                                        <span className={product.stock > 10 ? 'text-success' : 'text-warning'}>Stock: {product.stock}</span>
                                    </div>
                                </button>
                            ))}
