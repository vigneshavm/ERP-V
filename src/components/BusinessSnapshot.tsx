...
const BusinessSnapshot: React.FC = () => {
    ...
    const currentMonthTransactions = sectorTransactions.filter(t => new Date(t.date) >= startOfMonth);
    const currentMonthDailyRecords = (dailyFinanceRecords || []).filter(r => new Date(r.date) >= startOfMonth);

    const monthIncome = currentMonthTransactions
        .filter(t => t.type === 'INCOME' || t.type === 'Sale')
        .reduce((acc, curr) => acc + curr.amount, 0) +
        currentMonthDailyRecords.reduce((acc, curr) => acc + (curr.totalSales || 0), 0);
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        const periodTrans = sectorTransactions.filter(t => {
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-10">
            ...
            {/* Recent Transactions / Profit Trend */}
            <div className="bg-white dark:bg-neutral-800 p-6 rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-sm">
