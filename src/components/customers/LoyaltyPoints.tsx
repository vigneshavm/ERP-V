...
const LoyaltyPoints: React.FC = () => {
    ...
    const loyaltyData = useMemo(() => {
        ...
        salesHistory
            ...
            .forEach(sale => {
                if (sale.customerId) {
                    if (!customerPoints[sale.customerId]) {
                        customerPoints[sale.customerId] = { earned: 0, redeemed: 0, balance: 0 };
                    }
                    const pointsEarned = Math.floor(sale.total / 100);
                    customerPoints[sale.customerId].earned += pointsEarned;
                    customerPoints[sale.customerId].balance += pointsEarned;
                    ...
                    transactions.push({
                        ...
                        type: 'EARNED',
                        points: pointsEarned,
                        invoiceRef: `INV-${sale.id.substring(0, 8)}`,
