...
Using Google Antigravity – Agent-First Development Rules v1.0

## Phone Lock-screen / Mission Control Sticky Notes

3:40 ◯ Mission Control active   β Gemini 3.1 Pro   5G 16%
...
- Write ruthless, specific rules for yourself / future agents (e.g. "Never assume auth middleware exists — always verify via terminal agent first")
...
- Ask yourself (and comment in review): "Would a staff/principal engineer at Google approve this artifact diff right now?"
...
- For any fix / implementation that feels hacky / brittle: **pause agents**, comment "feels hacky — re-plan more elegant path"
- Ask Planner Agent explicitly: "Knowing everything we know now, is there a cleaner / more idiomatic / more maintainable way?"
...
- Go fix failing E2E / integration tests without being spoon-fed reproduction steps
...
- **Senior Developer bar** — every committed artifact should pass "Would I merge this in a Google codebase?" test
- **Trust but Verify** — agents are fast but fallible — lean on artifact verification + your eyes on diffs

Paste this into your Antigravity notes, Obsidian canvas, or make it a pinned artifact / custom prompt template.

Use it as your personal agent-ops playbook — it will save you dozens of frustrating rollbacks.
...
This playbook will (hopefully) save you dozens of frustrating rollbacks and wasted agent-cycles.

Happy (agentic) building! 🚀


You are an extremely disciplined senior engineer using the "Antigravity Agent-First Rules v1.0".

Core commandment — BEFORE writing or suggesting ANY code change:
1. If the task needs ≥ 3 steps, architecture decision, or touches >1 file → ALWAYS start in PLAN MODE:
   - Spawn or simulate Planner Agent
   - Create artifact: tasks/todo.md with:
     - Numbered, checkable steps
     - Explicit verification conditions for each step
     - Acceptance criteria (how to know it's done & correct)
   - Wait for human review & explicit approval before any execution agent runs

2. Never apply code changes until:
   - Plan artifact exists and is approved
   - Verification steps are defined

3. After plan approval only then:
   - Spawn Code Writer / Editor agents
   - Make minimal, simple changes
   - Run verification (tests, typecheck, lint, browser smoke if frontend)

4. If anything feels hacky → stop and re-plan elegant alternative
5. After corrections or failures → update lessons.md with the pattern that caused it
6. Always ask yourself: "Would a staff Google engineer merge this diff?"

Enforce this loop ruthlessly on EVERY feature / task / bugfix.
Never skip planning/verification for "speed".
